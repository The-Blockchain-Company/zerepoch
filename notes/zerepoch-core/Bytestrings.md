## Extending bytestring operations in ZerepochTx and Zerepoch Core
(19th November 2020)
...
some time ago, Alex asked about extending support for bytestrings in Zerepoch Core.
This document describes the issue and suggests some possible extensions.


### Current bytestring support in PLC

Zerepoch Core currently provides the following operations for its built-in
bytestring type:
...
bytestring inside a Zerepoch Core program.  The `Data` type supports bytestrings
and can be used to pass bytestrings into validators, but this isn't very
general-purpose.


### Characters and strings in Zerepoch Core

Zerepoch Core also provides built-in character and string types.  These both have
...
and `append` (append one string to another); also the ZerepochTx compiler can
...
However there is a subtlety: Zerepoch Core actually has _two_ representations of
Haskell Strings.  The ZerepochTx plugin compiles normal Haskell strings into
Scott-encoded lists of PLC's `char` type ("Scott strings" for short), but there
is also a special type called `ZerepochTx.Builtins.String` which is
...
**Update:** in the PR comments it's been suggested that we should
  ...
  and do away with Scott strings altogether.  This might be sensible, but it could need quite a lot of work in the Zerepoch compiler.

### Bytestrings in Haskell

Zerepoch Core bytestrings are implemented using Haskell's (strict) `ByteString`
...
in Zerepoch Core would be to add the `Word8` type to the default PLC universe, but
this would add quite a lot of complexity that we probably don't want.  [However,
SCP-1091 proposes bit operations for Zerepoch Core, and we might want some kind of
`Word` type for that.]
...
and `ByteString`.  We already have support for strings and bytestrings in Zerepoch
...
add some extra builtins for these at both the ZerepochTx and PLC levels.

Here's an initial proposal for some new Zerepoch Core builtins to improve bytestring support.
  ...
  * A `consChar` operation allowing you to prepend a character to a PLC string. This would help with
    building bytestrings dynamically.  Also, `ZerepochTx.String` contains
...
Any new PLC builtins should be mirrored in ZerepochTx, but this should be
...
See also the comments on [PR #2480](https://github.com/The-Blockchain-Company/zerepoch/pull/2480).
...
characters, produced when the ZerepochTx compiler compiles Haskell Strings.  We
should discourage people from using the latter, and we can probably do this by
removing the `char` type and the `appendChar` builtin from Zerepoch Core.
