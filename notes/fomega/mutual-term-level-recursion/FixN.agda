...
module Classic where
  ...
  -- In general, that's a good approach for a lazy language, but Zerepoch Core is a strict one (so far),
...
module StrictPolymorphic where
  ...
  -- So in Zerepoch Core we can just generate appropriate `by*` functions on the fly depending on
  -- the type schema of each of the functions that we take the fixed point of, while compiling
  -- those functions from Zerepoch IR.
