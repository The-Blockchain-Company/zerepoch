...
-- | The CEK machine.
-- Rules are the same as for the CK machine from "ZerepochCore.Evaluation.CkMachine",
...
module ZerepochCore.Interpreter.CekMachine
    ( EvaluationResult (..)
    , evaluateCek
    , runCek
    ) where
...
import           ZerepochCore.Evaluation.MachineException (MachineError (..), MachineException (..))
import           ZerepochCore.Evaluation.Result           (EvaluationResult (..))
import           ZerepochCore.View
