...
We have implemented a blockchain emulator (mockchain) as a Haskell library. An important feature of the mockchain is that it faithfully emulates the behaviour of Zerepoch smart contracts in a UTXO-style ledger, by storing, deserialising and running Zerepoch scripts the same way they will be run when Zerepoch is integrated with the Bcc blockchain.
...
Note the `txValidRange` field. This field contains an interval of slot numbers during which the transaction may be validated. The value of a transaction's `txValidRange` is passed to any validator scripts that are run during validation of that transaction's inputs, in order to make Zerepoch scripts deterministic. See [here](https://github.com/The-Blockchain-Company/fm-ledger-rules/issues/139) for the planned work to integrate this into the formal ledger rules for the Sophie release. The `SlotRange` type is defined in [`Ledger.Interval`](../../wallet-api/src/Ledger/Interval.hs).

The `Value` and `Bcc` types represent currency in the mockchain. `Bcc` is the designated currency in which fees are paid, and `Value` a map of currency identifiers to quantities, following Def. 1 in [2].
...
The reason why the data and redeemer scripts used in steps 2-5 are of the form `(state, input)` is that this is currently the only way we have of [validating the next data script](https://github.com/The-Blockchain-Company/zerepoch/issues/426). The linked github issue explains the encoding under the heading "Possible Solutions", third item.
...
Care must be taken to ensure that we don't make transactions which only use Bcc
