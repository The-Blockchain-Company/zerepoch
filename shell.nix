...
let
  inherit (packages) pkgs zerepoch zerepoch-playground simeon-playground zerepoch-pab simeon-dashboard fake-pab deployment docs;
  inherit (pkgs) stdenv lib utillinux python3 nixpkgs-fmt;
  inherit (zerepoch) haskell agdaPackages stylish-haskell sphinxcontrib-haddock sphinx-markdown-tables sphinxemoji nix-pre-commit-hooks bcc-cli bcc-node;
  inherit (zerepoch) agdaWithStdlib;
  inherit (zerepoch) purty purty-pre-commit purs spargo;
  ...
  localInputs = (with zerepoch; [
    ...
    hie-bios
    hlint
    simeon-dashboard.generate-purescript
    simeon-dashboard.start-backend
    simeon-playground.generate-purescript
    simeon-playground.start-backend
    zerepoch-playground.generate-purescript
    zerepoch-playground.start-backend
    zerepoch-pab.generate-purescript
    zerepoch-pab.migrate
    zerepoch-pab.start-backend
    zerepoch-pab.start-all-servers
    zerepoch-pab.start-all-servers-m
    purs
    purty
    spago
    stylish-haskell
