...
import           Zerepoch.Trace                   (EmulatorTrace, PrintEffect (..))
...
import qualified ZerepochTx.Numeric               as P
...
txnValidFrom =
    let five = Bcc.entropicValueOf 5
...
invalidTrace = property $ do
    (Mockchain m _, txn) <- forAll genChainTxn
    let invalidTxn = txn { txMint = Bcc.bccValueOf 1 }
...
invalidScript = property $ do
    ...
    let totalVal = Bcc.fromValue $ txOutValue (fst outToSpend)
    ...
    where
        ...
        failValidator = mkValidatorScript $$(ZerepochTx.compile [|| wrapValidator validator ||])
        validator :: () -> () -> ScriptContext -> Bool
        validator _ _ _ = ZerepochTx.traceError "I always fail everything"
...
pubKeyTransactions2 = do
    let [w1, w2, w3] = [wallet1, wallet2, wallet3]
        payment1 = initialBalance P.- Bcc.entropicValueOf 100
        payment2 = initialBalance P.+ Bcc.entropicValueOf 100
    ...
    _ <- Trace.nextSlot
    Trace.liftWallet wallet1 $ payToPublicKey_ W.always (Bcc.entropicValueOf 200) pubKey2
    void Trace.nextSlot
...
initialBalance = Bcc.entropicValueOf 100000
