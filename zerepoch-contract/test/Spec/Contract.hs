...
import           Zerepoch.Contract                      as Con
import qualified Zerepoch.Contract.State                as State
import           Zerepoch.Contract.Test
import           Zerepoch.Contract.Types                (ResumableResult (..), responses)
import           Zerepoch.Contract.Util                 (loopM)
import qualified Zerepoch.Trace                         as Trace
import           Zerepoch.Trace.Emulator                (ContractInstanceTag, Emulator, EmulatorTrace, activateContract,
...
import qualified Zerepoch.Contract.Request              as Endpoint
import           Zerepoch.Contract.Resumable            (IterationID, Response (..))
import           Zerepoch.Contract.Trace.RequestHandler (maybeToHandler)
...
tests =
    ...
    testGroup "contracts"
        ...
        , let smallTx = Constraints.mustPayToPubKey (Crypto.pubKeyHash $ walletPubKey w2) (Bcc.entropicValueOf 10)
          ...
          in run 3 "handle several blockchain events"
                (walletFundsChange w1 (Bcc.entropicValueOf (-20))
        ...
        , run 2 "pay to wallet"
            (walletFundsChange w1 (Bcc.entropicValueOf (-200))
                .&&. walletFundsChange w2 (Bcc.entropicValueOf 200)
                .&&. assertNoFailedTransactions)
            (void $ Trace.payToWallet w1 w2 (Bcc.entropicValueOf 200))
        ...
        , let theContract :: Contract () Schema ContractError () = void $ awaitUtxoProduced (walletAddress w2)
          in run 2 "await utxo produced"
            ...
            (void $ do
                ...
                Trace.payToWallet w1 w2 (Bcc.entropicValueOf 200)
                Trace.waitNSlots 1
            )
        ...
        , let payment = Constraints.mustPayToPubKey (Crypto.pubKeyHash $ walletPubKey w2) (Bcc.entropicValueOf 10)
...
someAddress = Ledger.scriptAddress $
    Ledger.mkValidatorScript $$(ZerepochTx.compile [|| \(_ :: ZerepochTx.BuiltinData) (_ :: ZerepochTx.BuiltinData) (_ :: ZerepochTx.BuiltinData) -> () ||])
