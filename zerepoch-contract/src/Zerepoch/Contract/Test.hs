...
module Zerepoch.Contract.Test(
      module X
    , TracePredicate
    , ContractConstraints
    , Zerepoch.Contract.Test.not
    , (.&&.)
...
import           Ledger.Tx                             (Tx)
import qualified Zerepoch.Contract.Effects               as Requests
...
import           Zerepoch.Contract.Types                 (Contract (..), IsContract (..), ResumableResult,
                                                        shrinkResumableResult)
import           ZerepochTx                              (CompiledCode, FromData (..), getPir)
...
import           Zerepoch.Contract.Trace                 as X
import           Zerepoch.Trace.Emulator                 (EmulatorConfig (..), EmulatorTrace, runEmulatorStream)
import           Zerepoch.Trace.Emulator.Types           (ContractConstraints, ContractInstanceLog,
    ...
    flip postMapM (L.generalize $ (,) <$> Folds.walletFunds w <*> Folds.walletFees w) $ \(finalValue', fees) -> do
        ...
        unless result $ do
            tell @(Doc Void) $ vsep $
                ...
                , " " <+> viaShow dlt] ++
                (if exact then [] else ["  (excluding" <+> viaShow (Bcc.getEntropic (Bcc.fromValue fees)) <+> "entropic in fees)" ]) ++
    ...
    flip postMapM (L.generalize $ Folds.walletFees w) $ \fees -> do
        ...
        unless result $ do
            tell @(Doc Void) $ vsep
                ...
                , " " <+> viaShow val
                , "entropic in fees, but they paid"
                , " " <+> viaShow fees ]
        pure result
