...
module Zerepoch.Contract.State(
    -- * Contract state
    -- $contractstate
    Contract
    , State(..)
...
import           Zerepoch.Contract.Checkpoint       (CheckpointKey, CheckpointStore)
import           Zerepoch.Contract.Effects          (PABReq, PABResp)
import           Zerepoch.Contract.Resumable
import           Zerepoch.Contract.Types            hiding (lastLogs, lastState, logs, observableState)
...
-- from 'Zerepoch.Contract.Types', exposing an interface that is suitable
