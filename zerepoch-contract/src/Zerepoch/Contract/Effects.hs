...
module Zerepoch.Contract.Effects( -- TODO: Move to Requests.Internal
    -- * Zerepoch application backend request effect types
    PABReq(..),
    _AwaitSlotReq,
    _AwaitTimeReq,
    _AwaitUtxoSpentReq,
    ...
    _UtxoSetAtAddress,
    _GetTip,
    -- * Zerepoch application backend response effect types
    PABResp(..),
    _AwaitSlotResp,
    _AwaitTimeResp,
    _AwaitUtxoSpentResp,
...
import           Zerepoch.ChainIndex.Tx        (ChainIndexTx (_citxTxId))
import           Zerepoch.ChainIndex.Types     (Page (pageItems), Tip (..), TxStatus (..))
...
-- constructors of the data type 'Zerepoch.ChainIndex.Effects.ChainIndexQueryEffect'.
...
-- responses come from the data type 'Zerepoch.ChainIndex.Effects.ChainIndexQueryEffect'.
