...
--   traces.
module Zerepoch.Contract.Trace
    ( TraceError(..)
    , EndpointError(..)
    , AsTraceError(..)
    , toNotifyError
...
import           Zerepoch.Contract.Trace.RequestHandler (RequestHandler (..), RequestHandlerLogMsg, generalise)
import qualified Zerepoch.Contract.Trace.RequestHandler as RequestHandler

import qualified Ledger.Bcc                           as Bcc
import           Ledger.Value                         (Value)

import           Zerepoch.ChainIndex                    (ChainIndexQueryEffect)
import           Zerepoch.Trace.Emulator.Types          (EmulatedWalletEffects)
...
defaultDistFor wallets = Map.fromList $ zip wallets (repeat (Bcc.entropicValueOf 100_000_000))
