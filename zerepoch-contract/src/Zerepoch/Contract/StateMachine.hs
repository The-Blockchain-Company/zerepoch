...
module Zerepoch.Contract.StateMachine(
    -- $statemachine
    StateMachineClient(..)
    , TxConstraints
    , SMContractError(..)
...
import           Zerepoch.ChainIndex                            (ChainIndexTx (..))
import           Zerepoch.Contract
import           Zerepoch.Contract.StateMachine.MintingPolarity (MintingPolarity (..))
import           Zerepoch.Contract.StateMachine.OnChain         (State (..), StateMachine (..), StateMachineInstance (..))
import qualified Zerepoch.Contract.StateMachine.OnChain         as SM
import           Zerepoch.Contract.StateMachine.ThreadToken     (ThreadToken (..), curPolicy, ttOutRef)
import           Zerepoch.Contract.Wallet                       (getUnspentOutput)
...
getInput outRef tx = do
    ...
    ZerepochTx.fromBuiltinData r

getStates
    :: forall s i
    . (ZerepochTx.FromData s, ZerepochTx.ToData s)
...
getOnChainState ::
    ( AsSMContractError e
    , ZerepochTx.FromData state
    , ZerepochTx.ToData state
    )
...
runInitialiseWith customLookups customConstraints StateMachineClient{scInstance} initialState initialValue = mapError (review _SMContractError) $ do
    ...
    let StateMachineInstance{stateMachine, typedValidator} = scInstance
        constraints = mustPayToTheScript initialState (initialValue <> SM.threadTokenValueOrZero scInstance)
            ...
            <> customConstraints
        red = Ledger.Redeemer (ZerepochTx.toBuiltinData (Scripts.validatorHash typedValidator, Mint))
            ...
            case smTransition oldState input of
                Just (newConstraints, newState)  ->
                    let isFinal = smFinal stateMachine (stateData newState)
                        ...
                        red = Ledger.Redeemer (ZerepochTx.toBuiltinData (Scripts.validatorHash typedValidator, Burn))
