...
module Zerepoch.Contract.Types(
    ContractEffs
    , handleContractEffs
    , Contract(..)
    , IsContract(..)
...
import           Zerepoch.Contract.Checkpoint        (AsCheckpointError (..), Checkpoint (..), CheckpointError (..),
                                                    CheckpointKey, CheckpointLogMsg, CheckpointStore,
                                                    completedIntervals, handleCheckpoint, jsonCheckpoint,
                                                    jsonCheckpointLoop)
import           Zerepoch.Contract.Resumable         hiding (never, responses, select)
import qualified Zerepoch.Contract.Resumable         as Resumable

import           Zerepoch.Contract.Effects           (PABReq, PABResp)
import qualified ZerepochTx.Applicative              as ZerepochTx
import qualified ZerepochTx.Functor                  as ZerepochTx
...
instance ZerepochTx.Functor (Contract w s e) where
  fmap = Haskell.fmap

instance ZerepochTx.Applicative (Contract w s e) where
  (<*>) = (Haskell.<*>)
  pure  = Haskell.pure
