...
module Zerepoch.Contract.StateMachine.ThreadToken where

import           ZerepochTx.Prelude                             hiding (Monoid (..), Semigroup (..))
...
ZerepochTx.makeIsDataIndexed ''ThreadToken [('ThreadToken,0)]
ZerepochTx.makeLift ''ThreadToken
...
curPolicy outRef = mkMintingPolicyScript $
    $$(ZerepochTx.compile [|| \r -> Scripts.wrapMintingPolicy (checkPolicy r) ||])
        `ZerepochTx.applyCode`
            ZerepochTx.liftCode outRef
