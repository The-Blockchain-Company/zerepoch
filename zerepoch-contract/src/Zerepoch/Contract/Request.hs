...
module Zerepoch.Contract.Request(
    -- * PAB requests
    -- ** Waiting
    awaitSlot
    , isSlot
...
import           Zerepoch.ChainIndex           (ChainIndexTx, Page (pageItems), txOutRefs)
import           Zerepoch.ChainIndex.Types     (Tip, TxStatus (..))
import           Zerepoch.Contract.Resumable
import           Zerepoch.Contract.Types
...
submitTxConstraints
  :: forall a w s e.
  ( ZerepochTx.ToData (RedeemerType a)
  , ZerepochTx.FromData (DatumType a)
  , ZerepochTx.ToData (DatumType a)
  , AsContractError e
  )
  => TypedValidator a
