...
--   wallet API.
module Zerepoch.Contract.Wallet(
      balanceTx
    , handleTx
    , getUnspentOutput
...
import qualified Zerepoch.Contract.BccAPI  as BccAPI
import qualified Zerepoch.Contract.Request     as Contract
import           Zerepoch.Contract.Types       (Contract (..))
...
{- Note [Submitting transactions from Zerepoch contracts]
...
getUnspentOutput = do
    ...
    let constraints = mustPayToPubKey (pubKeyHash ownPK) (Bcc.entropicValueOf 1)
...
data ExportTx =
        ExportTx
            { partialTx   :: C.Tx C.AurumEra -- ^ The transaction itself
...
data ExportTxInput = ExportTxInput{txIn :: C.TxIn, txOut :: C.TxOut C.AurumEra}
    deriving stock (Generic, Typeable)
...
instance FromJSON ExportTx where
    parseJSON (Object v) =
        ExportTx
            ...
            <*> pure mempty -- FIXME: How to deserialise Utxo / [(TxIn, TxOut)] ) see https://github.com/The-Blockchain-Company/bcc-node/issues/3051
            <*> v .: "signatories"
    parseJSON _ = fail "Expexted Object"

export :: C.ProtocolParameters -> C.NetworkId -> UnbalancedTx -> Either BccAPI.ToBccError ExportTx
...
mkPartialTx :: C.ProtocolParameters -> C.NetworkId -> Zerepoch.Tx -> Either BccAPI.ToBccError (C.Tx C.AurumEra)
mkPartialTx params networkId = fmap (C.makeSignedTransaction []) . BccAPI.toBccTxBody (Just params) networkId

mkLookups :: C.NetworkId -> Map Zerepoch.TxOutRef Zerepoch.TxOut -> Either BccAPI.ToBccError [ExportTxInput]
mkLookups networkId = fmap (fmap $ uncurry ExportTxInput) . traverse (bitraverse BccAPI.toBccTxIn (BccAPI.toBccTxOut networkId)) . Map.toList

mkSignatories :: Map Zerepoch.PubKeyHash (Maybe Zerepoch.PubKey) -> Either BccAPI.ToBccError [Text]
mkSignatories =
