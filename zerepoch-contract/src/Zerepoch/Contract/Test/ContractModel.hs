-- | This module provides a framework for testing Zerepoch contracts built on "Test.QuickCheck". The
...
--   the model state, and how to execute them in the blockchain emulator ("Zerepoch.Trace.Emulator").
...
module Zerepoch.Contract.Test.ContractModel
    ( -- * Contract models
      --
      -- $contractModel
      ContractModel(..)
...
import           Zerepoch.Trace.Emulator                 as Trace (ContractHandle, ContractInstanceTag, EmulatorTrace,
                                                                 activateContract, walletInstanceTag)
import           ZerepochTx.Monoid                       (inv)
...
-- $walletHandles
--
-- In order to call contract endpoints using `Zerepoch.Trace.Emulator.callEndpoint`, a `ContractHandle`
...
--   endpoints (`Zerepoch.Trace.Emulator.callEndpoint`).
...
--   `Zerepoch.Trace.Emulator.callEndpoint`.
...
class ( Typeable state
    ...
    --   `Zerepoch.Trace.Emulator.callEndpoint` or a transfer of tokens, but it can be anything
    ...
    --   running the actions in the emulator (see "Zerepoch.Trace.Emulator"). It gets access to the
...
-- blockchain emulator ("Zerepoch.Trace.Emulator") and checks that the model and the emulator agree
...
--     `defaultCheckOptions` `&` `emulatorConfig` . `Zerepoch.Trace.Emulator.initialChainState` `.~` `Left` dist
--                         `&` `maxSlot`                            `.~` slot
