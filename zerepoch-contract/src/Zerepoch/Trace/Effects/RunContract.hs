...
-}
module Zerepoch.Trace.Effects.RunContract(
    RunContract(..)
    , ContractConstraints
    , ContractInstanceTag
    , activateContract
...
import           Zerepoch.Contract                         (Contract, HasEndpoint)
import           Zerepoch.Contract.Effects                 (ActiveEndpoint, PABResp (ExposeEndpointResp),
                                                          _ExposeEndpointReq)
import           Zerepoch.Contract.Resumable               (Request (rqRequest), Requests (..))
import           Zerepoch.Contract.Schema                  (Input, Output)
import           Zerepoch.Contract.Types                   (IsContract (..), ResumableResult (..))
import           Zerepoch.Trace.Effects.ContractInstanceId (ContractInstanceIdEff, nextId)
import           Zerepoch.Trace.Emulator.ContractInstance  (contractThread, getThread)
import           Zerepoch.Trace.Emulator.Types             (ContractHandle (..), ContractInstanceState (..),
                                                          ContractInstanceTag,
...
import           Zerepoch.Trace.Scheduler                  (AgentSystemCall, EmSystemCall, MessageCall (Message),
...
-- | Run a Zerepoch contract (client side)
