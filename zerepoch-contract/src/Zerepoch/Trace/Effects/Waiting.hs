...
module Zerepoch.Trace.Effects.Waiting(
    Waiting(..)
    , waitUntilSlot
    , waitUntilTime
    , nextSlot
...
import           Zerepoch.Trace.Emulator.Types   (EmulatorMessage (NewSlot))
import           Zerepoch.Trace.Scheduler        (EmSystemCall, Priority (Sleeping), sleep)
