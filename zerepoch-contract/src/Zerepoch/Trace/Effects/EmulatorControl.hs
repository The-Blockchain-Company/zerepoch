...
-}
module Zerepoch.Trace.Effects.EmulatorControl(
    EmulatorControl(..)
    , setSigningProcess
    , agentState
...
import           Zerepoch.Trace.Emulator.Types            (EmulatorMessage (Freeze), EmulatorThreads)
import           Zerepoch.Trace.Scheduler                 (EmSystemCall, MessageCall (Message), Priority (Normal),
...
internals of the Zerepoch emulator, such as messing with the clock and dis-
connecting agents from the network.

All other effects defined under @Zerepoch.Trace.Effects@ can, in theory, be run
against a live system, ie. one running in real time with a real Charles node and
wallet(s).
