...
An emulator trace is a contract trace that can be run in the Zerepoch emulator.

-}
module Zerepoch.Trace.Emulator(
    Emulator
    , EmulatorTrace
    , EmulatorErr(..)
    , ContractHandle(..)
...
import           Zerepoch.Trace.Effects.EmulatorControl    (EmulatorControl, handleEmulatorControl)
import qualified Zerepoch.Trace.Effects.EmulatorControl    as EmulatorControl
import           Zerepoch.Trace.Effects.RunContract        (RunContract, handleRunContract)
import qualified Zerepoch.Trace.Effects.RunContract        as RunContract
...
import           Zerepoch.V1.Ledger.Slot                   (getSlot)
import           Zerepoch.V1.Ledger.Value                  (Value (..), flattenValue)
