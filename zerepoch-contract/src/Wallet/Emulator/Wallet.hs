...
import           Zerepoch.ChainIndex              (ChainIndexEmulatorState, ChainIndexQueryEffect)
import qualified Zerepoch.ChainIndex              as ChainIndex
import           Zerepoch.Contract.Checkpoint     (CheckpointLogMsg)
...
validateTxAndAddFees feeCfg ownTxOuts utx = do
    ...
    let scriptsSize = getSum $ foldMap (Sum . scriptSize . Ledger.sveScript) events
        theFee = Bcc.toValue $ calcFees feeCfg scriptsSize -- TODO: use protocol parameters
...
handleBalanceTx utxo UnbalancedTx{unBalancedTxTx} = do
    ...
    let fees = txFee filteredUnbalancedTxTx
        ...
        remainingFees = fees ZerepochTx.- fold collateral -- TODO: add collateralPercent
        balance = left ZerepochTx.- right
    ...
    if remainingFees `Value.leq` ZerepochTx.zero
    then do
            ...
            then err
                ...
                let
                    ...
                    totalSpent     = case reverse fundsToSpend of
                                        []            -> ZerepochTx.zero
                                        (_, total'):_ -> total'
                    change         = totalSpent ZerepochTx.- vl
