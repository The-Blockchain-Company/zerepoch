...
import           Ledger.Bcc                            (Bcc (Entropic))
...
instance Render CurrencySymbol where
    render (CurrencySymbol "")    = pure "Bcc"
    render (CurrencySymbol other) = render other

instance Render TokenName where
    render (TokenName "") = pure "Entropic"
    render t              = pure $ pretty $ Value.toString t

instance Render ZerepochTx.BuiltinByteString where
    render = pure . pretty . JSON.encodeByteString . ZerepochTx.fromBuiltin

deriving via RenderPretty ZerepochTx.Data instance
         Render ZerepochTx.Data

instance Render ZerepochTx.BuiltinData where
    render d = render $ ZerepochTx.builtinDataToData d
...
instance Render Bcc where
    render bcc@(Entropic l)
        | Bcc.isZero bcc = pure "-"
