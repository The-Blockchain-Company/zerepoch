...
-- | Functions for compiling GHC Core expressions into Zerepoch Core terms.
module ZerepochTx.Compiler.Expr (compileExpr, compileExprWithDefs, compileDataConRef) where

import           ZerepochTx.Compiler.Binders
import           ZerepochTx.Compiler.Builtins
import           ZerepochTx.Compiler.Error
import           ZerepochTx.Compiler.Laziness
import           ZerepochTx.Compiler.Names
import           ZerepochTx.Compiler.Type
import           ZerepochTx.Compiler.Types
import           ZerepochTx.Compiler.Utils
...
import qualified ZerepochTx.Builtins.Class       as Builtins
...
import qualified ZerepochIR.Compiler.Definitions as PIR
import           ZerepochIR.Compiler.Names
import qualified ZerepochIR.MkPir                as PIR
import qualified ZerepochIR.Purity               as PIR
...
As Zerepoch has no different runtime representations, the overall strategy is consider `Type rep` to always be `Type LiftedRep`, which becomes `Type` on the Zerepoch side.
on all levels and match any 'TYPE rep' to the usual Zerepoch type.
