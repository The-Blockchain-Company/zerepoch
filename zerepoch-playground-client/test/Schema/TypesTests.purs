          ...
          { getValue:
              AssocMap.fromTuples
                [ ( Tuple (CurrencySymbol { unCurrencySymbol: "12345" })
                      (AssocMap.fromTuples [ Tuple (TokenName { unTokenName: "BCC" }) (BigInteger.fromInt 100) ])
                  )
                ]
          }
    test "FormIntF" do
    ...
    test "balance" do
      equalGenericShow
        ( Value
            { getValue:
                AssocMap.fromTuples
                  [ bcc /\ AssocMap.fromTuples [ bccToken /\ BigInteger.fromInt 10 ]
                  , currencies
                      /\ AssocMap.fromTuples
        ...
        ( mkInitialValue
            [ KnownCurrency { hash: "", friendlyName: "Bcc", knownTokens: (JsonNonEmptyList (pure (TokenName { unTokenName: "" }))) }
