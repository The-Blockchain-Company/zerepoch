...
import           ZerepochTx                    (CompiledCode, applyCode, liftCode)
...
tests = testGroup "all tests" [
    ...
    testGroup "Value" ([
        ...
        testProperty "CurrencySymbol IsString/Show" currencySymbolIsStringShow
          ...
          ++ (let   vlJson :: BSL.ByteString
                    ...
                    vlValue = Bcc.entropicValueOf 50
                in byteStringJson vlJson vlValue)),
    ...
    testGroup "SomeBccApiTx" [
        testProperty "Value ToJSON/FromJSON" (jsonRoundTrip Gen.genSomeBccApiTx)
        ]
    ]
...
valueAddIdentity = property $ do
    ...
    Hedgehog.assert $ vl1 == (vl1 ZerepochTx.+ ZerepochTx.zero)
    Hedgehog.assert $ vl1 == (ZerepochTx.zero ZerepochTx.+ vl1)

valueAddInverse :: Property
valueAddInverse = property $ do
    vl1 <- forAll Gen.genValue
    let vl1' = ZerepochTx.negate vl1
    Hedgehog.assert $ ZerepochTx.zero == (vl1 ZerepochTx.+ vl1')

valueScalarIdentity :: Property
valueScalarIdentity = property $ do
    vl1 <- forAll Gen.genValue
    Hedgehog.assert $ vl1 == ZerepochTx.scale 1 vl1
...
valueScalarDistrib = property $ do
    ...
    let r1 = ZerepochTx.scale scalar (vl1 ZerepochTx.+ vl2)
        r2 = ZerepochTx.scale scalar vl1 ZerepochTx.+ ZerepochTx.scale scalar vl2
...
ledgerBytesShowFromHexProp = property $ do
    bts <- forAll $ LedgerBytes . ZerepochTx.toBuiltin <$> Gen.genSizedByteString 32
...
calcFeesTest = property $ do
    ...
    Hedgehog.assert $ calcFees feeCfg 11 == Bcc.entropicOf 13
...
slotToTimeRangeBoundsInverseProp = property $ do
    sc <- forAll Gen.genSlotConfig
    slot <- forAll Gen.genSlot
    let slotRange = ZerepochTx.fmap (TimeSlot.posixTimeToEnclosingSlot sc)
