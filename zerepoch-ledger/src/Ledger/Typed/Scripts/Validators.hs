...
import           ZerepochCore.Default                    (DefaultUni)
import           ZerepochTx
import           ZerepochTx.Prelude                      (check)

import qualified Ledger.Scripts                        as Scripts
import           Zerepoch.V1.Ledger.Address              (Address (..), scriptHashAddress)
...
mkTypedValidatorParam vc wrapper param =
    mkTypedValidator (vc `ZerepochTx.applyCode` ZerepochTx.liftCode param) wrapper
