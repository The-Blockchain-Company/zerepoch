...
import           ZerepochTx                         (FromData (..), ToData (..))
import           ZerepochTx.Lattice
import qualified ZerepochTx.Numeric                 as N
...
import           Zerepoch.V1.Ledger.Crypto          (PubKey, PubKeyHash)
...
--   Zerepoch contracts] in 'Zerepoch.Contract.Wallet'.
data UnbalancedTx =
    UnbalancedTx
        { unBalancedTxTx                  :: Tx
...
[1] See 'Zerepoch.V1.Ledger.Value.split'
...
--   'Zerepoch.Contract.submitTxConstraints'
