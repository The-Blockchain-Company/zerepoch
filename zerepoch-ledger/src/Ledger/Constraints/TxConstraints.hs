...
import qualified Zerepoch.V1.Ledger.Interval as I
import           Zerepoch.V1.Ledger.Scripts  (Datum (..), DatumHash, MintingPolicyHash, Redeemer (..), ValidatorHash)
import           Zerepoch.V1.Ledger.Time     (POSIXTimeRange)
import           Zerepoch.V1.Ledger.Tx       (TxOutRef)
import           Zerepoch.V1.Ledger.Value    (TokenName, Value, isZero)
...
type UntypedConstraints = TxConstraints ZerepochTx.BuiltinData ZerepochTx.BuiltinData
...
instance Haskell.Semigroup (TxConstraints i o) where
    (<>) = (<>) -- uses ZerepochTx.Semigroup instance
...
mustPayToTheScript :: forall i o. ZerepochTx.ToData o => o -> Value -> TxConstraints i o
mustPayToTheScript dt vl =
    TxConstraints
        { txConstraints = [MustIncludeDatum (Datum $ ZerepochTx.toBuiltinData dt)]
        , txOwnInputs = []
...
mustMintValue = mustMintValueWithRedeemer (Redeemer $ ZerepochTx.toBuiltinData ())
...
mustMintCurrency mps = mustMintCurrencyWithRedeemer mps (Redeemer $ ZerepochTx.toBuiltinData ())
