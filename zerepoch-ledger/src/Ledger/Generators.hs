...
import qualified Gen.Bcc.Api.Typed      as Gen
import           Hedgehog
...
import qualified Zerepoch.V1.Ledger.Contexts  as Contexts
import qualified Zerepoch.V1.Ledger.Interval  as Interval
...
generatorModel =
    let vl = Bcc.entropicValueOf 100000
...
--   zerepoch-ledger (in particular, 'Ledger.Tx.unspentOutputs') we note the
...
genValidTransaction' g feeCfg (Mockchain _ ops) = do
    ...
    let ins = Set.fromList $ pubKeyTxIn . fst <$> inUTXO
        ...
        totalVal = foldl' (+) 0 $ map (Bcc.fromValue . txOutValue . snd) inUTXO
...
genValidTransactionSpending' g feeCfg ins totalVal = do
    ...
    if fee' < totalVal
        then do
            outVals <- fmap Bcc.toValue <$> splitVal numOut (totalVal - fee')
            let tx = mempty
                        { txInputs = ins
                        ...
                        , txRedeemers = Map.singleton (RedeemerPtr Mint 0) (Redeemer $ ZerepochTx.toBuiltinData ())
                        , txFee = Bcc.toValue fee'
                        }
...
alwaysSucceedPolicy = mkMintingPolicyScript $$(ZerepochTx.compile [|| \_ _ -> () ||])
...
genSomeBccApiTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genSomeBccApiTx = Gen.choice [ genColeEraInBccModeTx
                                 , genSophieEraInBccModeTx
                                 , genEvieEraInBccModeTx
                                 , genJenEraInBccModeTx
                                 , genAurumEraInBccModeTx
                                 ]

genColeEraInBccModeTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genColeEraInBccModeTx = do
  tx <- fromGenT $ Gen.genTx C.ColeEra
  pure $ SomeTx tx C.ColeEraInBccMode

genSophieEraInBccModeTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genSophieEraInBccModeTx = do
  tx <- fromGenT $ Gen.genTx C.SophieEra
  pure $ SomeTx tx C.SophieEraInBccMode

genEvieEraInBccModeTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genEvieEraInBccModeTx = do
  tx <- fromGenT $ Gen.genTx C.EvieEra
  pure $ SomeTx tx C.EvieEraInBccMode

genJenEraInBccModeTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genJenEraInBccModeTx = do
  tx <- fromGenT $ Gen.genTx C.JenEra
  pure $ SomeTx tx C.JenEraInBccMode

genAurumEraInBccModeTx :: (GenBase m ~ Identity, MonadGen m) => m SomeBccApiTx
genAurumEraInBccModeTx = do
  tx <- fromGenT $ Gen.genTx C.AurumEra
  pure $ SomeTx tx C.AurumEraInBccMode

genBcc :: MonadGen m => m Bcc
genBcc = Bcc.entropicOf <$> Gen.integral (Range.linear 0 (100000 :: Integer))
...
-- | A TokenName is either an arbitrary bytestring or the bcc token name
...
-- or the bcc symbol (empty bytestring).
