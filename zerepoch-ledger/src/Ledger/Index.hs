...
import qualified Zerepoch.V1.Ledger.Slot            as Slot
import           Zerepoch.V1.Ledger.Tx
...
data ValidationError =
    ...
    | NonBccFees Tx
    -- ^ The fee is not denominated entirely in Bcc.
...
'checkMintingAuthorised' will never allow a transaction that mints Bcc.
Bcc's currency symbol is the empty bytestring, and it can never be matched by a
...
-- | Check if the fees are paid exclusively in Bcc.
checkFeeIsBcc :: ValidationMonad m => Tx -> m ()
checkFeeIsBcc t =
    if (Bcc.toValue $ Bcc.fromValue $ txFee t) == txFee t
    then pure ()
    else throwError $ NonBccFees t

-- | Minimum transaction fee.
minFee :: Tx -> V.Value
minFee = const (Bcc.entropicValueOf 10)
