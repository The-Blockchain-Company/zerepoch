...
compiled to a language called Zerepoch Core.  Any change to the core
...
best avoided, so the goal with Zerepoch Core was to make it so simple
...
against the evaluator for Zerepoch Core that is used in Bcc.
...
contract core language Zerepoch Core has only β-equality.
...
system (Zerepoch Core) on which this paper is based is also available as
...
used in Zerepoch Core but this is something that we would like to
...
calculus. We omit the $\eta$-rule as Zerepoch Core does not have it. The
...
normalisation oriented approach is that in Zerepoch, contracts are
...
\section{Scaling up from System $F_{\omega\mu}$ to Zerepoch Core}
\label{sec:real-world}

This formalisation forms the basis of a formalisation of Zerepoch
...
In Zerepoch Core we also extend System $F_{\omega\mu}$ with integers and
...
Haskell implementation of Zerepoch Core which uses the Haskell
bytestring library. We chose to model the Zerepoch Core implementation
...
As we can compile our Agda Zerepoch Core interpreter to Haskell we can
test the production Haskell Zerepoch Core interpreter against it. We
...
%presented here Zerepoch Core also has intrinsically sized integer and
