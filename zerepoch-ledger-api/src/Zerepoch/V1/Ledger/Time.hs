...
module Zerepoch.V1.Ledger.Time(
      POSIXTime(..)
    , POSIXTimeRange
    , DiffMilliSeconds(..)
    , fromMilliSeconds
...
newtype DiffMilliSeconds = DiffMilliSeconds Integer
  ...
  deriving newtype (Haskell.Num, AdditiveSemigroup, AdditiveMonoid, AdditiveGroup, Haskell.Enum, Eq, Ord, Haskell.Real, Haskell.Integral, Serialise, Hashable, ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)
