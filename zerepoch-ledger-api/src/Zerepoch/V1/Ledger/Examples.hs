...
-}
module Zerepoch.V1.Ledger.Examples (alwaysSucceedingNAryFunction, alwaysFailingNAryFunction) where

import           Codec.Serialise
import           Data.ByteString.Lazy     (toStrict)
import           Data.ByteString.Short
import           Numeric.Natural
import           Zerepoch.V1.Ledger.Api
...
The scripts we provide here are *manually* constructed Zerepoch Core. Why not use our great machinery for easily creating
scripts with Zerepoch Tx? Because Zerepoch Tx relies on a compiler plugin, and so is always going to be a bit finicky to user.
It seems better therefore to avoid depending on Zerepoch Tx in any "core" projects like the ledger.
-}
