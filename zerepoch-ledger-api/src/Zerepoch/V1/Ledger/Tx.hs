...
import           Zerepoch.V1.Ledger.Slot
import           Zerepoch.V1.Ledger.TxId
import           Zerepoch.V1.Ledger.Value
import qualified Zerepoch.V1.Ledger.Value    as V
...
(1) is used in the bcc-sl repository, and (2) is used in the
`zerepoch-core` project in this repository.

In this module we use (2) because of the precedent. This means however that we
may generate different hashes for the same transactions compared to bcc-sl.
...
instance ZerepochTx.Eq TxOutRef where
    {-# INLINABLE (==) #-}
    l == r =
        txOutRefId l ZerepochTx.== txOutRefId r
        ZerepochTx.&& txOutRefIdx l ZerepochTx.== txOutRefIdx r
...
instance ZerepochTx.Eq TxOut where
    {-# INLINABLE (==) #-}
    l == r =
        txOutAddress l ZerepochTx.== txOutAddress r
        ZerepochTx.&& txOutValue l ZerepochTx.== txOutValue r
        ZerepochTx.&& txOutDatumHash l ZerepochTx.== txOutDatumHash r
...
ZerepochTx.makeIsDataIndexed ''TxOut [('TxOut,0)]
ZerepochTx.makeLift ''TxOut

ZerepochTx.makeIsDataIndexed ''TxOutRef [('TxOutRef,0)]
ZerepochTx.makeLift ''TxOutRef
