...
module Zerepoch.V1.Ledger.Scripts(
    -- * Scripts
    Script (..),
    scriptSize,
    fromCompiledCode,
...
import           ZerepochTx                                 (CompiledCode, FromData (..), ToData (..),
                                                           UnsafeFromData (..), getPlc, makeLift)
import           ZerepochTx.Builtins                        as Builtins
import           ZerepochTx.Builtins.Internal               as BI
import           ZerepochTx.Evaluation                      (ErrorWithCause (..), EvaluationError (..), evaluateCekTrace)
...
`zerepoch-ledger` uses CBOR for data serialisation and `zerepoch-core` uses Flat. The
...
To make `zerepoch-ledger` work with scripts serialised with Flat, and keep the CBOR
