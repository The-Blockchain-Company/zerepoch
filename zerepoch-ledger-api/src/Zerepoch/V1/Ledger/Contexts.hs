...
module Zerepoch.V1.Ledger.Contexts
    (
...
import           Zerepoch.V1.Ledger.Address    (Address (..), toPubKeyHash)
import           Zerepoch.V1.Ledger.Bytes      (LedgerBytes (..))
import           Zerepoch.V1.Ledger.Credential (Credential (..), StakingCredential)
import           Zerepoch.V1.Ledger.Crypto     (PubKey (..), PubKeyHash (..), Signature (..))
import           Zerepoch.V1.Ledger.DCert      (DCert (..))
import           Zerepoch.V1.Ledger.Scripts
import           Zerepoch.V1.Ledger.Time       (POSIXTimeRange)
import           Zerepoch.V1.Ledger.Tx         (TxOut (..), TxOutRef (..))
import           Zerepoch.V1.Ledger.TxId
import           Zerepoch.V1.Ledger.Value      (CurrencySymbol (..), Value)
...
{-# INLINABLE bccLockedBy #-}
-- | Get the total amount of 'Bcc' locked by the given validator in this transaction.
bccLockedBy :: TxInfo -> ValidatorHash -> Bcc
bccLockedBy ptx h = Bcc.fromValue (valueLockedBy ptx h)
