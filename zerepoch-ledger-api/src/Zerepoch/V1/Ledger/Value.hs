...
module Zerepoch.V1.Ledger.Value(
    -- ** Currency symbols
      CurrencySymbol(..)
    , currencySymbol
    , mpsSymbol
...
import           Zerepoch.V1.Ledger.Bytes           (LedgerBytes (LedgerBytes))
import           Zerepoch.V1.Ledger.Orphans         ()
import           Zerepoch.V1.Ledger.Scripts
import qualified ZerepochTx                         as ZerepochTx
import qualified ZerepochTx.AssocMap                as Map
import           ZerepochTx.Lift                    (makeLift)
import qualified ZerepochTx.Ord                     as Ord
import           ZerepochTx.Prelude                 as ZerepochTx
import           ZerepochTx.These

newtype CurrencySymbol = CurrencySymbol { unCurrencySymbol :: ZerepochTx.BuiltinByteString }
    deriving (IsString, Haskell.Show, Serialise, Pretty) via LedgerBytes
    deriving stock (Generic)
    deriving newtype (Haskell.Eq, Haskell.Ord, Eq, Ord, ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)
...
instance FromJSON CurrencySymbol where
  parseJSON =
    JSON.withObject "CurrencySymbol" $ \object -> do
      ...
      Haskell.pure $ CurrencySymbol $ ZerepochTx.toBuiltin bytes
...
currencySymbol = CurrencySymbol . ZerepochTx.toBuiltin

-- | ByteString of a name of a token, shown as UTF-8 string when possible
newtype TokenName = TokenName { unTokenName :: ZerepochTx.BuiltinByteString }
...
tokenName = TokenName . ZerepochTx.toBuiltin

fromText :: Text -> TokenName
fromText = tokenName . E.encodeUtf8

fromTokenName :: (BS.ByteString -> r) -> (Text -> r) -> TokenName -> r
fromTokenName handleBytestring handleText (TokenName bs) = either (\_ -> handleBytestring $ ZerepochTx.fromBuiltin bs) handleText $ E.decodeUtf8' (ZerepochTx.fromBuiltin bs)
...
newtype AssetClass = AssetClass { unAssetClass :: (CurrencySymbol, TokenName) }
    deriving stock (Generic)
    deriving newtype (Haskell.Eq, Haskell.Ord, Haskell.Show, Eq, Ord, Serialise, ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)
...
newtype Value = Value { getValue :: Map.Map CurrencySymbol (Map.Map TokenName Integer) }
    ...
    deriving newtype (Serialise, ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)
...
currencies. At the moment there is only a single currency (Bcc), so 'Value'
...
using 'Ledger.Bcc.bccValueOf'. To get the bcc dimension of 'Value' we use
'Ledger.Bcc.fromValue'. Zerepoch contract authors will be able to define modules
similar to 'Ledger.Bcc' for their own currencies.
