...
{- Note [Error Codes of zerepoch errors]

Our goal is to assign a project-wide unique error number (errorCode) to all errors
that might occur during any phase of compilation --- lifting values, compiling Zerepoch Tx,
compiling PIR, executing PLC, "off-chain" Zerepoch code ---, so
...
left to be implemented for each error by the Zerepoch developers.
It is the responsibility of the  Zerepoch developer to make sure that

1) the assigned errorcode (Natural) is unique among the whole Zerepoch-project codebase,
...
A Zerepoch sub-project that wants to throw an error, must depend on this package `zerepoch-core`.
To aid in defining an instance for a brand-new (uncategorized) error or added error-dataconstructors,
the Zerepoch developer can make use (but not cabal-depend upon) of the
mega-package zerepoch-errors to "guess-pick" an error-code that is not currently in use
by the whole codebase, by running

```
> cabal run zerepoch-errors-next
An error code that is not currently used is: 9998
```

After defining/extending this errorcode instance, the Zerepoch developer must navigate to the megapackage and
confirm these new errorcodes by adding all newly-introduced base-error constructors
to the list of all-project errors `zerepoch-errors/src/Errors.hs`. The TH code of `zerepoch-errors`
will make sure there are not duplicates, and running the `cabal haddock zerepoch-errors`
...
be "moved" and listed/errorCoded under the umbrella datatype `zerepoch-errors:Errors.DeprecatedErrors`.
