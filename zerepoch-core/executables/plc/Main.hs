...
import qualified ZerepochCore.Evaluation.Machine.Ck as Ck
import qualified ZerepochCore.Pretty                as PP

import qualified UntypedZerepochCore                as UPLC (eraseProgram)
...
plcHelpText :: String
plcHelpText = helpText "Typed Zerepoch Core"

plcInfoCommand :: ParserInfo Command
plcInfoCommand = zerepoch plcHelpText
...
zerepoch ::
  ...
  String ->
  ParserInfo Command
zerepoch langHelpText =
    info
      (zerepochOpts <**> helper)
      (fullDesc <> header "Typed Zerepoch Core Tool" <> progDesc langHelpText)

zerepochOpts :: Parser Command
zerepochOpts = hsubparser (
       command "apply"
    ...
    <> command "typecheck"
           (info (Typecheck <$> typecheckOpts)
            (progDesc "Typecheck a typed Zerepoch Core program."))
    <> command "erase"
           (info (Erase <$> eraseOpts)
            (progDesc "Convert a typed Zerepoch Core program to an untyped one."))
    <> command "evaluate"
           (info (Eval <$> evalOpts)
            (progDesc "Evaluate a typed Zerepoch Core program using the CK machine."))
  )
