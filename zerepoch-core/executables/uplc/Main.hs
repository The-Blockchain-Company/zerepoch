...
uplcHelpText :: String
uplcHelpText = helpText "Untyped Zerepoch Core"

uplcInfoCommand :: ParserInfo Command
uplcInfoCommand = zerepoch uplcHelpText
...
zerepoch langHelpText =
    info
      (zerepochOpts <**> helper)
      (fullDesc <> header "Untyped Zerepoch Core Tool" <> progDesc langHelpText)
...
zerepochOpts = hsubparser (
    ...
    <> command "evaluate"
           (info (Eval <$> evalOpts)
            (progDesc "Evaluate an untyped Zerepoch Core program using the CEK machine."))
    <> command "dump-model"
           (info (pure DumpModel)
