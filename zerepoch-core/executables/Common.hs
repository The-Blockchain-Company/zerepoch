...
import           ZerepochCore.Check.Uniques                 as PLC (checkProgram)
import           ZerepochCore.Error                         (AsParseError, AsUniqueError, UniqueError)
import           ZerepochCore.Evaluation.Machine.ExBudget   (ExBudget (..), ExRestrictingBudget (..))
import           ZerepochCore.Evaluation.Machine.ExMemory   (ExCPU (..), ExMemory (..))
import qualified ZerepochCore.Generators                    as Gen
import qualified ZerepochCore.Generators.Interesting        as Gen
import qualified ZerepochCore.Generators.Test               as Gen
import qualified ZerepochCore.Pretty                        as PP
import           ZerepochCore.Rename                        (rename)
import qualified ZerepochCore.StdLib.Data.Bool              as StdLib
import qualified ZerepochCore.StdLib.Data.ChurchNat         as StdLib
import qualified ZerepochCore.StdLib.Data.Integer           as StdLib
import qualified ZerepochCore.StdLib.Data.Unit              as StdLib

import qualified UntypedZerepochCore                        as UPLC
import qualified UntypedZerepochCore.Check.Uniques          as UPLC (checkProgram)
import qualified UntypedZerepochCore.Evaluation.Machine.Cek as Cek
import qualified UntypedZerepochCore.Parser                 as UPLC (parseProgram)
...
typedDeBruijnNotSupportedError =
    errorWithoutStackTrace "De-Bruijn-named ASTs are not supported for typed Zerepoch Core"
...
helpText ::
  -- | Either "Untyped Zerepoch Core" or "Typed Zerepoch Core"
  String -> String
helpText lang =
    ...
    ++ "programs.  Some commands read or write Zerepoch Core abstract "
