...
module ZerepochCore.StdLib.Type
    ( RecursiveType (..)
    , makeRecursiveType
    ) where
...
@zerepoch/zerepoch-core/docs/Holed types.md@.
...
@zerepoch/docs/fomega/mutual-type-level-recursion/IFixIsEnough.agda@ describes the encoding trick
...
Note [Packing n-ary functors]) on the Zerepoch Core side and this resulted in huge unreadable types
...
but in the end types are not normalized due to how the encoding works. In Zerepoch Core we have two
...
our types are in head-spine form. See @zerepoch/docs/fomega/deep-isorecursive/alternatives.md@ for
...
and we require the user (where "the user" means someone generating Zerepoch Core or writing it directly,
i.e. either someone writing a compiler to Zerepoch Core or one of the creators of the language) to split
...
Cons:
    ...
    5. someone who generates Zerepoch Core does not care much about whether types are normalized,
...
-- This can only happen if someone writing/generating Zerepoch Core made a mistake.
