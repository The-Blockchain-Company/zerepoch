...
When running Zerepoch code on the chain, you're running code on other peoples
machines, so you'll have to pay for it. And it has to be possible to determine
how much money that should be before sending the transaction with the Zerepoch
...
locally and determine the cost. The functional nature of Zerepoch allows for the
...
- The memory cost of the initial AST is added to the budget. See Note [Memory
  Usage for Zerepoch]. This operation currently does not cost any CPU. It
  currently costs as much memory as the AST itself, before aborting. See
  https://github.com/The-Blockchain-Company/zerepoch/issues/1799 for more discussion.
 ...
 * The basic data for models of execution time is produced by Criterion
   benchmarks (run via zerepoch-core:cost-model-budgeting-bench) and saved in
   'benching.csv'.  At this point the time units are seconds.

 * The data in 'benching.csv' is used by zerepoch-core:update-cost-model to create
 ...
 * When the Zerepoch Core evaluator is compiled, the JSON data in
 ...
 * When the Zerepoch Core evaluator is run, the code in
   ZerepochCore.Evaluation.Machine.BuiltinCostModel uses the data in
 ...
 shows that the difference is very small.  The tests in zerepoch-core:
...
module ZerepochCore.Evaluation.Machine.ExBudget
    ( ExBudget(..)
    , ExBudgetBuiltin(..)
    , ExRestrictingBudget(..)
...
import           ZerepochPrelude                          hiding (toList)
