...
module ZerepochCore.Evaluation.Machine.ExBudgetingDefaults
...
import           ZerepochCore.Constant

import qualified ZerepochCore.DataFilePaths                                 as DFP
import           ZerepochCore.Default
import           ZerepochCore.Evaluation.Machine.BuiltinCostModel
import           ZerepochCore.Evaluation.Machine.CostModelInterface
...
import           UntypedZerepochCore.Evaluation.Machine.Cek.CekMachineCosts
...
{- Note [Modifying the cost model]
   ...
   comment out the three above) then rerun
      ...
      cabal bench zerepoch-core:update-cost-model

   (You may also need to add 'data-default' to the 'build-depends' for the
   library in zerepoch-core.cabal). This will generate a new JSON file filled with
