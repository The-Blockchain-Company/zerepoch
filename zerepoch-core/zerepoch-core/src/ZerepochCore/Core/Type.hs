...
-- | Version of Zerepoch Core to be used for the program.
data Version ann
...
-- We need this for type checking Zerepoch, however we get Zerepoch types/terms/programs by either
...
-- (instead of throwing it away like we do now) and compute the Zerepoch kind directly from that.
-- That might be less efficient and probably requires updating the Zerepoch Tx compiler, so we went
-- with the simplest option for now and it's to have a class. Providing an instance per universe is
-- no big deal.
-- | For getting the Zerepoch kind of a type from the universe.
...
-- | The default version of Zerepoch Core supported by this library.
...
-- iff 'name' is DeBruijn-encoded (level or index). See for example the instance of  'UntypedZerepochCore.Core.Instance.Flat'
