...
module ZerepochCore.Constant.Meaning where

import           ZerepochPrelude

import           ZerepochCore.Constant.Dynamic.Emit
import           ZerepochCore.Constant.Function
...
-- | Try to specialize @a@ as a type representing a Zerepoch type variable.
...
-- and specializing them as types representing Zerepoch type variables.
...
-- Take an argument of a built-in type and try to specialize it as a type representing a Zerepoch
...
-- | Specialize each Haskell type variable in @a@ as a type representing a Zerepoch Core type variable
...
-- see https://github.com/The-Blockchain-Company/zerepoch/pull/2521#issuecomment-759522445
