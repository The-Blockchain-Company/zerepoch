...
-- See the @zerepoch/zerepoch-core/docs/Constant application.md@
...
module ZerepochCore.Constant.Typed
    ( TypeScheme (..)
    , FoldArgs
    , FoldArgsEx
    , TyNameRep (..)
...
import           ZerepochCore.Constant.Dynamic.Emit
import           ZerepochCore.Core
import           ZerepochCore.Data
import           ZerepochCore.Evaluation.Machine.ExBudget
import           ZerepochCore.Evaluation.Machine.ExMemory
import           ZerepochCore.Evaluation.Machine.Exception
import           ZerepochCore.Evaluation.Result
...
1. @ifThenElse@ for 'Bool' (being a built-in type rather than a Scott-encoded one) has to be
   ...
   thing, see https://github.com/The-Blockchain-Company/zerepoch/blob/e222466e6d46bbca9f76243bb496b3c88ed02ca1/language-zerepoch-core/src/ZerepochCore/Constant/Typed.hs#L165-L252
...
require applying an arbitrary Zerepoch Core function in the denotation of a builtin, which would
...
the first-order case. In a Zerepoch program we always pair constants of built-in types with their
tags from the universe, which means that in order to produce a constant embedded into a program
we need the tag of the type of that constant. We can't get that tag from a Zerepoch type -- those
...
and then the Zerepoch Tx compiler can provide a type class or something for constructing singletons
for built-in types.
-}
...
-- type is, say, a bound variable, so we never need to convert such a term from Zerepoch Core to
...
we always have concrete type tags, but at Zerepoch compile time an argument to @f@ can be
a Zerepoch type variable and so we can't establish any connection between the type tag that
we'll end up having at runtime and a Zerepoch type variable that we have at compile time.
...
(boolean vs integer) and there won't be any Haskell type error for that, let alone a Zerepoch type
