...
import           ZerepochCore
import           ZerepochCore.Generators
import           ZerepochCore.Generators.AST       as AST
import qualified ZerepochCore.Generators.NEAT.Spec as NEAT
...
main :: IO ()
main = do
    plcFiles <- findByExtension [".plc"] "zerepoch-core/test/data"
    rwFiles <- findByExtension [".plc"] "zerepoch-core/test/scopes"
    typeFiles <- findByExtension [".plc"] "zerepoch-core/test/types"
    typeErrorFiles <- findByExtension [".plc"] "zerepoch-core/test/type-errors"
...
-- TODO: replace ZerepochCore.Generators.AST.genConstant with this.  We
-- can't do that at the moment because genConstant is used by the tests for the
-- zerepoch-ir parser, and that can't handle the full range of constants at the
-- moment.
