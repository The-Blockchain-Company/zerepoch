...
-- | Parser for untyped Zerepoch Core using megaparsec, as in Zerepoch IR.
-- Will replace UntypedZerepochCore.Parser.hs.
-- Parser.y and Lexer.x, which currently generate Parser.hs, will be removed.

module UntypedZerepochCore.NewParser
    ( topSourcePos
    , parse
    , parseQuoted
    , term
...
import qualified NewUntypedZerepochCore            as UPLC
import qualified ZerepochCore                      as PLC
import qualified ZerepochCore.Parsable             as PLC
import           ZerepochPrelude                   (Pretty, through)
import           Text.Megaparsec                 hiding (ParseError, State, parse)
import           UntypedZerepochCore.Check.Uniques (checkProgram)
import           UntypedZerepochCore.Rename        (Rename (rename))

import           Data.ByteString.Lazy            (ByteString)
import           Data.ByteString.Lazy.Internal   (unpackChars)
import qualified Data.Text                       as T
import           ZerepochCore.ParserCommon
