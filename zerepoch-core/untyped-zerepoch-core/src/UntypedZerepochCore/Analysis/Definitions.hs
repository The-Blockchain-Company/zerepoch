{-# LANGUAGE LambdaCase #-}
-- | Definition analysis for untyped Zerepoch Core.
-- This just adapts term-related code from ZerepochCore.Analysis.Definitions;
-- we just re-use the typed machinery to do the hard work here.
module UntypedZerepochCore.Analysis.Definitions
    ( termDefs
    , runTermDefs
    ) where
...
import           ZerepochCore.Analysis.Definitions (ScopeType (TermScope), UniqueInfos, addDef, addUsage)
