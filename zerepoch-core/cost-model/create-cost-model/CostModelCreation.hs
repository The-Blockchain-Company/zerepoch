...
import qualified Data.ByteString.Hash                           as ZerepochHash
...
sha2_256 cpuModelR = do
  ...
  let memModel = ModelOneArgumentConstantCost (memoryUsageAsCostingInteger $ ZerepochHash.sha2 "")
...
sha3_256 cpuModelR = do
  cpuModel <- readModelLinear cpuModelR
  let memModel = ModelOneArgumentConstantCost (memoryUsageAsCostingInteger $ ZerepochHash.sha3 "")
  pure $ CostingFun (ModelOneArgumentLinearCost cpuModel) memModel

blake2b :: MonadR m => (SomeSEXP (Region m)) -> m (CostingFun ModelOneArgument)
blake2b cpuModelR = do
  cpuModel <- readModelLinear cpuModelR
  let memModel = ModelOneArgumentConstantCost (memoryUsageAsCostingInteger $ ZerepochHash.blake2b "")
  pure $ CostingFun (ModelOneArgumentLinearCost cpuModel) memModel
