...
import           ZerepochIR             as PIR
import           ZerepochIR.Compiler    as PIR
import           ZerepochIR.Parser      as Parser
import           ZerepochIR.TypeCheck
...
-- | Bccpt an computation that keeps its errors in an 'Except' into one that looks as if it caught them in 'IO'.
asIfThrown
    :: Exception e
    => Except e a
