...
module ZerepochIR.Compiler.Datatype
    ( compileDatatype
    , compileDatatypeDefs
    , compileRecDatatypes
    , funTyArgs
...
import           ZerepochPrelude                 (showText)
...
import           ZerepochIR.Transform.Substitute

import qualified ZerepochCore.MkPlc              as PLC
import           ZerepochCore.Quote
import qualified ZerepochCore.StdLib.Type        as Types
...
zerepoch-tx plugin is unlikely to generate PIR code with unnormalized types inside
...
Consider a datatype as defined in Zerepoch IR:
(datatype
  (tyvardecl T (type))
  (tyvardecl a_1 t_1)
  ..
