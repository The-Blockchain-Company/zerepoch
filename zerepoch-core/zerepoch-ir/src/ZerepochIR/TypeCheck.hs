...
-- | Kind/type inference/checking, mirroring ZerepochCore.TypeCheck
module ZerepochIR.TypeCheck
    (
    -- * Configuration.
      BuiltinTypes (..)
    , PirTCConfig (..)
...
import           ZerepochCore                  (ToKind)
import           ZerepochCore.Quote
import           ZerepochCore.Rename
import qualified ZerepochCore.TypeCheck        as PLC
import           ZerepochIR
...
- Our deadcode eliminator which works on PIR (in `ZerepochIR.Optimizer.Deadcode`) may eliminate ill-typed code, which
