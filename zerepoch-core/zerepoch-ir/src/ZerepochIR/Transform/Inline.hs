...
In particular, we want to get rid of "trivial" let bindings which the Zerepoch Tx compiler sometimes creates.
-}
module ZerepochIR.Transform.Inline (inline) where

import           ZerepochIR
import qualified ZerepochIR.Analysis.Dependencies as Deps
import           ZerepochIR.MkPir
import           ZerepochIR.Purity
import           ZerepochIR.Transform.Rename      ()
import           ZerepochPrelude

import qualified ZerepochCore                     as PLC
import qualified ZerepochCore.Constant.Meaning    as PLC
import           ZerepochCore.Name
import           ZerepochCore.Quote
import           ZerepochCore.Subst               (typeSubstTyNamesM)
...
We inline type bindings unconditionally as it is safe to do so, because ZerepochIR
...
UntypedZerepochCore, hence inlining type bindings will not increase the code size
