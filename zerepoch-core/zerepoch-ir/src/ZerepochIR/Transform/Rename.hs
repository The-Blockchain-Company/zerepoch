...
-- instance in scope.
module ZerepochIR.Transform.Rename
    ( renameTermM
    , renameProgramM
    ) where
...
import qualified ZerepochCore.Rename.Internal as PLC
...
The AST of Zerepoch IR allows us to represent some weird data types like
...
is only unambiguous if we stipulate that GADTs are not allowed in Zerepoch IR (otherwise the last
