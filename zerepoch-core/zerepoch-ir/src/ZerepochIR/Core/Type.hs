...
import           ZerepochCore          (Kind, Name, TyName, Type (..))
import qualified ZerepochCore          as PLC
import           ZerepochCore.Constant (AsConstant (..), FromConstant (..), throwNotAConstant)
import           ZerepochCore.Core     (UniOf)
import           ZerepochCore.Flat     ()
import           ZerepochCore.MkPlc    (Def (..), TermLike (..), TyVarDecl (..), VarDecl (..))
...
{- Note [Declarations in Zerepoch Core]
In Zerepoch Core declarations are usually *flattened*, i.e. `(lam x ty t)`, whereas
in Zerepoch IR they are *reified*, i.e. `(termBind (vardecl x ty) t)`.

Zerepoch IR needs reified declarations to make it easier to parse *lists* of declarations,
which occur in a few places.

However, we also include all of Zerepoch Core's term syntax in our term syntax (and we also use
its types). We therefore have somewhat inconsistent syntax since declarations in Zerepoch
Core terms will be flattened. This makes the embedding obvious and makes life easier
if we were ever to use the Zerepoch Core AST "for real".

It would be nice to resolve the inconsistency, but this would probably require changing
Zerepoch Core to use reified declarations.
-}

-- See note [PIR as a PLC extension]
data Term tyname name uni fun a =
                        -- Zerepoch Core (ish) forms, see note [Declarations in Zerepoch Core]
