module Zerepoch.Contracts() where
...
construct a PLC script using the core-to-zerepoch plugin (with Template Haskell
...
So, in the future, the Zerepoch coordinating code has to translate the `a` value
to PLC and apply it to the function. This could be done with a type class
(similar to aeson's ToJSON).

In order to validate transactions, bcc nodes have to do the same with
