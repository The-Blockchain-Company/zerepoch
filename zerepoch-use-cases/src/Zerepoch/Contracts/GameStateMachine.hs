...
module Zerepoch.Contracts.GameStateMachine(
    contract
    , typedValidator
    , GameToken
    , mkValidator
...
import           ZerepochTx.Prelude             hiding (Applicative (..), check)
import           Schema                       (ToArgument, ToSchema)

import qualified Data.ByteString.Char8        as C

import           Zerepoch.Contract.StateMachine (State (..), Void)
...
newtype HashedString = HashedString BuiltinByteString
    deriving newtype (Eq, ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)
...
ZerepochTx.unstableMakeIsData ''GameState
ZerepochTx.makeLift ''GameState
ZerepochTx.unstableMakeIsData ''GameInput
ZerepochTx.makeLift ''GameInput
ZerepochTx.makeLift ''GameToken
ZerepochTx.unstableMakeIsData ''GameToken
