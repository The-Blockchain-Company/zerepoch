...
-- | Zerepoch implementation of an account that can be unlocked with a token.
--   Whoever owns the token can spend the outputs locked by the contract.
--   (A suitable token can be created with the 'Zerepoch.Contracts.Currency'
--   contract, or with 'newAccount' in this module)
module Zerepoch.Contracts.TokenAccount(
  Account(..)
...
import           Zerepoch.Contract.Constraints
...
import qualified Zerepoch.Contract.Typed.Tx         as TypedTx
...
ZerepochTx.makeLift ''Account
ZerepochTx.unstableMakeIsData ''Account
