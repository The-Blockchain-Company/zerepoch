...
module Zerepoch.Contracts.Currency(
      OneShotCurrency(..)
    , CurrencySchema
    , CurrencyError(..)
    , AsCurrencyError(..)
...
ZerepochTx.makeLift ''OneShotCurrency
...
curPolicy cur = mkMintingPolicyScript $
    $$(ZerepochTx.compile [|| \c -> Scripts.wrapMintingPolicy (checkPolicy c) ||])
        `ZerepochTx.applyCode`
            ZerepochTx.liftCode cur
