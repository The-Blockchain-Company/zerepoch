...
module Zerepoch.Contracts.Future(
    -- $future
      Future(..)
    , FutureAccounts(..)
    , mkAccounts
...
import           Zerepoch.Contract.Util             (loopM)
import qualified ZerepochTx
import           ZerepochTx.Prelude

import           Zerepoch.Contract.StateMachine     (AsSMContractError, State (..), StateMachine (..), Void)
import qualified Zerepoch.Contract.StateMachine     as SM
import qualified Zerepoch.Contracts.Currency        as Currency
import           Zerepoch.Contracts.Escrow          (AsEscrowError (..), EscrowError, EscrowParams (..), RefundSuccess)
import qualified Zerepoch.Contracts.Escrow          as Escrow
import           Zerepoch.Contracts.TokenAccount    (Account (..))
...
-- $future
-- A futures contract in Zerepoch. This example illustrates a number of concepts.
--
--   1. Maintaining a margin (a kind of deposit) during the duration of the contract to protect against breach of contract (see note [Futures in Zerepoch])
--   2. Using oracle values to obtain current pricing information (see note [Oracles] in Zerepoch.Contracts)
...
In Zerepoch we could do physical settlement for assets that exist on the
...
[Oracles] in Zerepoch.Contracts. Also note that we
...
typedValidator future ftos =
    let val = $$(ZerepochTx.compile [|| validatorParam ||])
            `ZerepochTx.applyCode`
                ZerepochTx.liftCode future
                `ZerepochTx.applyCode`
                    ZerepochTx.liftCode ftos
...
verifyOracle :: ZerepochTx.FromData a => PubKey -> SignedMessage a -> Maybe (a, TxConstraints Void Void)
verifyOracle pubKey sm =
    either (const Nothing) pure
    $ Oracle.verifySignedMessageConstraints pubKey sm

verifyOracleOffChain :: ZerepochTx.FromData a => Future -> SignedMessage (Observation a) -> Maybe (POSIXTime, a)
...
escrowParams client future ftos FutureSetup{longPK, shortPK, contractStart} =
    let
        ...
        dataScript  = Ledger.Datum $ ZerepochTx.toBuiltinData $ initialState future
        targets =
...
ZerepochTx.makeLift ''Future
ZerepochTx.makeLift ''FutureAccounts
ZerepochTx.makeLift ''Margins
ZerepochTx.unstableMakeIsData ''Margins
ZerepochTx.makeLift ''Role
ZerepochTx.unstableMakeIsData ''Role
ZerepochTx.makeLift ''FutureState
ZerepochTx.unstableMakeIsData ''FutureState
ZerepochTx.makeLift ''FutureAction
ZerepochTx.unstableMakeIsData ''FutureAction
