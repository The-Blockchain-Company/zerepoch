...
The two coins (stablecoin and reservecoin) are Zerepoch native token whose minting
...
-}
module Zerepoch.Contracts.Stablecoin(
  SC(..)
  , RC(..)
  , BC(..)
  , PC(..)
...
import           Zerepoch.Contract.StateMachine (AsSMContractError, OnChainState (..), SMContractError, State (..),
...
import           ZerepochTx.Ratio               as R
import qualified Prelude                      as Haskell

-- | Conversion rate from peg currency (eg. USD) to base currency (eg. Bcc)
...
-- | An amount of base currency coins (eg. Bcc or some native currency)
...
typedValidator stablecoin =
    let val = $$(ZerepochTx.compile [|| validator ||]) `ZerepochTx.applyCode` ZerepochTx.liftCode stablecoin
        validator d = SM.mkValidator (stablecoinStateMachine d)
        wrap = Scripts.wrapValidator @BankState @Input
    in Scripts.mkTypedValidator @(StateMachine BankState Input) val $$(ZerepochTx.compile [|| wrap ||])
...
ZerepochTx.makeLift ''SC
ZerepochTx.makeLift ''RC
ZerepochTx.makeLift ''BC
ZerepochTx.makeLift ''PC
ZerepochTx.makeLift ''BankState
ZerepochTx.makeLift ''Stablecoin
ZerepochTx.unstableMakeIsData ''BC
ZerepochTx.unstableMakeIsData ''SC
ZerepochTx.unstableMakeIsData ''RC
ZerepochTx.unstableMakeIsData ''PC
ZerepochTx.unstableMakeIsData ''BankState
ZerepochTx.unstableMakeIsData ''Stablecoin
ZerepochTx.unstableMakeIsData ''SCAction
