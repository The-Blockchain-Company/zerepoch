...
-- | A general-purpose escrow contract in Zerepoch
module Zerepoch.Contracts.Escrow(
    -- $escrow
    Escrow
    , EscrowError(..)
    , AsEscrowError(..)
...
import           ZerepochTx.Prelude         hiding (Applicative (..), Semigroup (..), check, foldMap)
...
ZerepochTx.makeLift ''EscrowTarget
...
ZerepochTx.makeLift ''EscrowParams
...
ZerepochTx.unstableMakeIsData ''Action
ZerepochTx.makeLift ''Action
...
refund inst escrow = do
    ...
    let flt _ ciTxOut = either id Ledger.datumHash (Tx._ciTxOutDatum ciTxOut) == Ledger.datumHash (Datum (ZerepochTx.toBuiltinData $ Ledger.pubKeyHash pk))
