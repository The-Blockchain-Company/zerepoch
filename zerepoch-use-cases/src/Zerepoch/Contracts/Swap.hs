...
module Zerepoch.Contracts.Swap(
    Swap(..),
    -- * Script
    swapValidator
    ) where
...
data Swap = Swap
    ...
    , swapMargin          :: !Bcc -- ^ Margin deposited at the beginning of the contract to protect against default (one party failing to pay)
    , swapOracle          :: !PubKey -- ^ Public key of the oracle (see note [Oracles] in [[Zerepoch.Contracts]])
    }

ZerepochTx.makeLift ''Swap
...
ZerepochTx.unstableMakeIsData ''SwapOwners
ZerepochTx.makeLift ''SwapOwners
...
mkValidator Swap{..} SwapOwners{..} redeemer p@ScriptContext{scriptContextTxInfo=txInfo} =
    let
        ...
        bccValueIn :: Value -> Integer
        bccValueIn v = Bcc.getEntropic (Bcc.fromValue v)
        ...
        amt    = Bcc.getEntropic swapNotionalAmt
        margin = Bcc.getEntropic swapMargin
        ...
        -- fixed leg
        iP1 :: TxInInfo -> Bool
        iP1 TxInInfo{txInInfoResolved=TxOut{txOutValue}} = Validation.txSignedBy txInfo swapOwnersFixedLeg && bccValueIn txOutValue == margin

        -- True if the transaction input is the margin payment of the
        -- floating leg
        iP2 :: TxInInfo -> Bool
        iP2 TxInInfo{txInInfoResolved=TxOut{txOutValue}} = Validation.txSignedBy txInfo swapOwnersFloating && bccValueIn txOutValue == margin
        ...
        ol1 :: TxOut -> Bool
        ol1 o@TxOut{txOutValue} = isPubKeyOutput o swapOwnersFixedLeg && bccValueIn txOutValue <= fixedRemainder

        -- True if the output is the payment of the floating leg.
        ol2 :: TxOut -> Bool
        ol2 o@TxOut{txOutValue} = isPubKeyOutput o swapOwnersFloating && bccValueIn txOutValue <= floatRemainder
...
--       Language.Zerepoch.Coordination.Contracts
swapValidator :: Swap -> Validator
swapValidator swp = Ledger.mkValidatorScript $
    $$(ZerepochTx.compile [|| validatorParam ||])
        `ZerepochTx.applyCode`
            ZerepochTx.liftCode swp
...
Zerepoch should be able to handle variation margins in a series of validation
