...
-}
module Zerepoch.Contracts.Uniswap.Trace(
    uniswapTrace
    --
    , setupTokens
    , tokenNames
...
import           Zerepoch.Contract                   hiding (throwError)
...
import           Zerepoch.Trace.Emulator             (EmulatorRuntimeError (GenericError), EmulatorTrace)
import qualified Zerepoch.Trace.Emulator             as Emulator
...
uniswapTrace = do
    ...
    let coins = Map.fromList [(tn, Types.mkCoin cs tn) | tn <- tokenNames]
        bcc   = Types.mkCoin bccSymbol bccToken
    ...
    let cp = OffChain.CreateParams bcc (coins Map.! "A") 100000 500000

    Emulator.callEndpoint @"create" cid1 cp
    _ <- Emulator.waitNSlots 5

    let ap = AddParams{apCoinA = bcc, apCoinB = coins Map.! "A", apAmountA = 1000, apAmountB = 5000}
