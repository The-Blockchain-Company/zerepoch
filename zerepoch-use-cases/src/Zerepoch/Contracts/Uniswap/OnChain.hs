...
module Zerepoch.Contracts.Uniswap.OnChain
    ( mkUniswapValidator
...
import           Zerepoch.Contracts.Uniswap.Pool    (calculateAdditionalLiquidity, calculateInitialLiquidity,
...
-- | See 'Zerepoch.Contracts.Uniswap.OffChain.close'.
...
-- | See 'Zerepoch.Contracts.Uniswap.OffChain.remove'.
...
-- | See 'Zerepoch.Contracts.Uniswap.OffChain.add'.
...
findPoolDatum info h = case findDatum h info of
    Just (Datum d) -> case ZerepochTx.unsafeFromBuiltinData d of
        (Pool lp a) -> (lp, a)
