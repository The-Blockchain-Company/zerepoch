...
module Zerepoch.Contracts.Uniswap.OffChain
...
import           Zerepoch.Contracts.Uniswap.OnChain (mkUniswapValidator, validateLiquidityMinting)
import           Zerepoch.Contracts.Uniswap.Pool
import           Zerepoch.Contracts.Uniswap.Types
import qualified ZerepochTx
import           ZerepochTx.Prelude                 hiding (Semigroup (..), dropWhile, flip, unless)
...
uniswapInstance us = Scripts.mkTypedValidator @Uniswapping
    ($$(ZerepochTx.compile [|| mkUniswapValidator ||])
        `ZerepochTx.applyCode` ZerepochTx.liftCode us
        `ZerepochTx.applyCode` ZerepochTx.liftCode c)
...
liquidityPolicy us = mkMintingPolicyScript $
    $$(ZerepochTx.compile [|| \u t -> Scripts.wrapMintingPolicy (validateLiquidityMinting u t) ||])
        `ZerepochTx.applyCode` ZerepochTx.liftCode us
        `ZerepochTx.applyCode` ZerepochTx.liftCode poolStateTokenName
...
create us CreateParams{..} = do
    ...
    let usInst   = uniswapInstance us
        ...
        tx       = Constraints.mustPayToTheScript usDat1 usVal                                     <>
                   ...
                   Constraints.mustSpendScriptOutput oref (Redeemer $ ZerepochTx.toBuiltinData $ Create lp)
...
close us CloseParams{..} = do
    ...
    let usInst   = uniswapInstance us
        ...
        redeemer = Redeemer $ ZerepochTx.toBuiltinData Close
        ...
        tx       = Constraints.mustPayToTheScript usDat usVal          <>
                   ...
                   Constraints.mustIncludeDatum (Datum $ ZerepochTx.toBuiltinData $ Pool lp liquidity)
...
remove us RemoveParams{..} = do
    ...
    let usInst       = uniswapInstance us
        ...
        redeemer     = Redeemer $ ZerepochTx.toBuiltinData Remove
...
add us AddParams{..} = do
    ...
    let usInst       = uniswapInstance us
        ...
        redeemer     = Redeemer $ ZerepochTx.toBuiltinData Add
...
swap us SwapParams{..} = do
    ...
    let inst    = uniswapInstance us
        ...
        tx      = mustSpendScriptOutput oref (Redeemer $ ZerepochTx.toBuiltinData Swap) <>
