...
module Zerepoch.Contracts.Prism.Credential(
    CredentialAuthority(..)
    , Credential(..)
    , policy
    , token
...
policy credential = mkMintingPolicyScript $
    $$(ZerepochTx.compile [|| \c -> Scripts.wrapMintingPolicy (validateMint c) ||])
        `ZerepochTx.applyCode`
            ZerepochTx.liftCode credential
...
ZerepochTx.makeLift ''CredentialAuthority
ZerepochTx.unstableMakeIsData ''CredentialAuthority
ZerepochTx.makeLift ''Credential
ZerepochTx.unstableMakeIsData ''Credential
