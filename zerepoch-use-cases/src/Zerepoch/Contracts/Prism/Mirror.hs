...
module Zerepoch.Contracts.Prism.Mirror(
    MirrorSchema
...
import           Zerepoch.Contract.StateMachine        (AsSMContractError (..), SMContractError,
...
import           Zerepoch.Contracts.Prism.StateMachine as StateMachine
...
createTokens authority = endpoint @"issue" $ \CredentialOwnerReference{coTokenName, coOwner} -> do
    ...
    let pk      = Credential.unCredentialAuthority authority
        ...
        constraints =
            ...
            <> Constraints.mustPayToPubKey pk (Bcc.entropicValueOf 1)   -- Add self-spend to force an input
