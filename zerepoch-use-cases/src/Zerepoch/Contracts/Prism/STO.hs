...
* Pays 1 Entropic to a predefined public key address for every coin minted
...
-}
module Zerepoch.Contracts.Prism.STO(
    STOData(..)
    , policy
    , coins
    ) where
...
import           Ledger.Bcc           (Bcc (..), fromValue)
...
validateSTO STOData{stoIssuer,stoCredentialToken,stoTokenName} _ ScriptContext{scriptContextTxInfo=txInfo,scriptContextPurpose=Minting ownHash} =
    let tokenOK = stoCredentialToken `Value.leq` Validation.valueSpent txInfo
        Entropic paidToIssuer = fromValue (Validation.valuePaidTo txInfo stoIssuer)
        mintOK =
...
policy stoData = mkMintingPolicyScript $
    $$(ZerepochTx.compile [|| \c -> Scripts.wrapMintingPolicy (validateSTO c) ||]) `ZerepochTx.applyCode` ZerepochTx.liftCode stoData
...
ZerepochTx.makeLift ''STOData
ZerepochTx.unstableMakeIsData ''STOData
