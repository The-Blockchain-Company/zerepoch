...
module Zerepoch.Contracts.Auction(
    AuctionState(..),
    AuctionInput(..),
    BuyerSchema,
    SellerSchema,
...
import           Ledger                           (Bcc, POSIXTime, PubKeyHash, Value)
...
import           Zerepoch.Contract.StateMachine     (State (..), StateMachine (..), StateMachineClient, ThreadToken, Void,
                                                   WaitingResult (..))
...
ZerepochTx.makeLift ''AuctionParams
...
ZerepochTx.unstableMakeIsData ''HighestBid
...
ZerepochTx.unstableMakeIsData ''AuctionState

-- | Transition between auction states
data AuctionInput
    = Bid { newBid :: Bcc, newBidder :: PubKeyHash } -- Increase the price
    | Payout
    deriving stock (Generic, Haskell.Show)
    deriving anyclass (ToJSON, FromJSON)

ZerepochTx.unstableMakeIsData ''AuctionInput
...
auctionTransition AuctionParams{apOwner, apAsset, apEndTime} State{stateData=oldState} input =
    case (oldState, input) of
        ...
        (Ongoing HighestBid{highestBid, highestBidder}, Bid{newBid, newBidder}) | newBid > highestBid -> -- if the new bid is higher,
            let constraints =
                    Constraints.mustPayToPubKey highestBidder (Bcc.toValue highestBid) -- we pay back the previous highest bid
                ...
                newState =
                    State
                        ...
                        , stateValue = apAsset <> Bcc.toValue newBid -- and lock the new bid in the script output
                        }
        ...
        (Ongoing h@HighestBid{highestBidder, highestBid}, Payout) ->
            let constraints =
                    ...
                    <> Constraints.mustPayToPubKey apOwner (Bcc.toValue highestBid) -- the owner receives the payment
...
--   machine compiled to a Zerepoch core validator script.
...
type BuyerSchema = Endpoint "bid" Bcc
...
data BuyerEvent =
        ...
        | SubmitOwnBid Bcc -- ^ We want to submit a new bid
...
handleEvent client lastHighestBid change =
    ...
    in case change of
        ...
        SubmitOwnBid bcc -> do
            ...
            r <- SM.runStep client Bid{newBid = bcc, newBidder = self}
