...
{-
This module and its submodules define the Zerepoch part of a credential
...
4. .CredentialManager: A Zerepoch application managing all the credentials
   that are available to the User.
5. .Mirror: A Zerepoch application that locks credentials in state machines
   and revokes them when necessary.
6. .Unlock: Two Zerepoch applications that each present a credential to unlock
   some funds.
...
-}
module Zerepoch.Contracts.Prism(
    -- * Unlock (STO)
    STOSubscriber(..)
    , STOSubscriberSchema
    , UnlockError(..)
...
import           Zerepoch.Contracts.Prism.Credential
import           Zerepoch.Contracts.Prism.Mirror
import           Zerepoch.Contracts.Prism.StateMachine
import           Zerepoch.Contracts.Prism.Unlock
