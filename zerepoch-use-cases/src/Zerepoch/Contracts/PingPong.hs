...
module Zerepoch.Contracts.PingPong(
    PingPongState(..),
    Input(..),
    PingPongError(..),
    PingPongSchema,
...
import           Zerepoch.Contract.StateMachine (AsSMContractError (..), OnChainState, State (..), Void)
...
initialise = endpoint @"initialise" $ \() -> void $ SM.runInitialise client Pinged (Bcc.entropicValueOf 1)
...
ZerepochTx.unstableMakeIsData ''PingPongState
ZerepochTx.makeLift ''PingPongState
