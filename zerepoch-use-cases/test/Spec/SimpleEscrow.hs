...
import           Zerepoch.Contracts.SimpleEscrow
...
tests = testGroup "simple-escrow"
    [ checkPredicate "can lock some value in the contract"
        ...
        $ do
            ...
            let params = mkEscrowParams startTime (Bcc.entropicValueOf 10) (Bcc.entropicValueOf 1)
    ...
    , checkPredicate "only locking wallet can request refund"
        ...
        $ do
            startTime <- TimeSlot.scSlotZeroTime <$> Trace.getSlotConfig
            let params = mkEscrowParams startTime (Bcc.entropicValueOf 100) (Bcc.entropicValueOf 1)
