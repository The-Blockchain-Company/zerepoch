...
import qualified Zerepoch.Contracts.MultiSigStateMachine as MS
...
tests =
    ...
    [ checkPredicate "lock, propose, sign 3x, pay - SUCCESS"
        ...
        .&&. walletFundsChange w2 (Bcc.entropicValueOf 5))
    ...
    , checkPredicate "lock, propose, sign 2x, pay - FAILURE"
        ...
        .&&. walletFundsChange w1 (Bcc.entropicValueOf (-10))
        .&&. walletFundsChange w2 (Bcc.entropicValueOf 0))
    ...
    , goldenPir "test/Spec/multisigStateMachine.pir" $$(ZerepochTx.compile [|| MS.mkValidator ||])
...
-- | A payment of 5 Bcc to the public key address of wallet 2
payment :: POSIXTime -> MS.Payment
payment startTime =
    MS.Payment
        { MS.paymentAmount    = Bcc.entropicValueOf 5
...
lockProposeSignPay signatures rounds = do
    ...
    _ <- Trace.callEndpoint @"lock" handle1 (Bcc.entropicValueOf 10)
