...
import           Zerepoch.Contract.Test               hiding (not)
import           Zerepoch.Contract.Test.ContractModel
import           Zerepoch.Contracts.GameStateMachine  as G
import           Zerepoch.Trace.Emulator              as Trace
...
instance ContractModel GameModel where
    ...
    nextState (Lock w secret val) = do
        ...
        withdraw w $ Bcc.entropicValueOf val
        wait 2
    ...
    nextState (Guess w old new val) = do
        ...
        when correct $ do
            ...
            deposit w $ Bcc.entropicValueOf val
        wait 1
...
tests =
    ...
    [ checkPredicate "run a successful game trace"
        (walletFundsChange w2 (Bcc.entropicValueOf 3 <> gameTokenVal)
        .&&. valueAtAddress (Scripts.validatorAddress G.typedValidator) (Bcc.entropicValueOf 5 ==)
        .&&. walletFundsChange w1 (Bcc.entropicValueOf (-8)))
        successTrace

    , checkPredicate "run a 2nd successful game trace"
        (walletFundsChange w2 (Bcc.entropicValueOf 3)
        .&&. valueAtAddress (Scripts.validatorAddress G.typedValidator) (Bcc.entropicValueOf 1 ==)
        .&&. walletFundsChange w1 (Bcc.entropicValueOf (-8))
        .&&. walletFundsChange w3 (Bcc.entropicValueOf 4 <> gameTokenVal))
        successTrace2

    , checkPredicate "run a failed trace"
        (walletFundsChange w2 gameTokenVal
        .&&. valueAtAddress (Scripts.validatorAddress G.typedValidator) (Bcc.entropicValueOf 8 ==)
        .&&. walletFundsChange w1 (Bcc.entropicValueOf (-8)))
        failTrace

    , goldenPir "test/Spec/gameStateMachine.pir" $$(ZerepochTx.compile [|| mkValidator ||])
...
successTrace = do
    ...
    Trace.callEndpoint @"lock" hdl LockArgs{lockArgsSecret="hello", lockArgsValue= Bcc.entropicValueOf 8}
    _ <- Trace.waitNSlots 2
    _ <- Trace.payToWallet w1 w2 gameTokenVal
    _ <- Trace.waitNSlots 1
    hdl2 <- Trace.activateContractWallet w2 G.contract
    Trace.callEndpoint @"guess" hdl2 GuessArgs{guessArgsOldSecret="hello", guessArgsNewSecret="new secret", guessArgsValueTakenOut=Bcc.entropicValueOf 3}
...
successTrace2 = do
    ...
    Trace.callEndpoint @"guess" hdl3 GuessArgs{guessArgsOldSecret="new secret", guessArgsNewSecret="hello", guessArgsValueTakenOut=Bcc.entropicValueOf 4}
    void $ Trace.waitNSlots 1
...
failTrace = do
    ...
    _ <- Trace.waitNSlots 1
    hdl2 <- Trace.activateContractWallet w2 G.contract
    _ <- Trace.callEndpoint @"guess" hdl2 GuessArgs{guessArgsOldSecret="hola", guessArgsNewSecret="new secret", guessArgsValueTakenOut=Bcc.entropicValueOf 3}
    void $ Trace.waitNSlots 1
