...
import           Zerepoch.Contracts.Future (Future (..), FutureAccounts (..), FutureError, FutureSchema, FutureSetup (..),
                                          Role (..))
import qualified Zerepoch.Contracts.Future as F
...
tests =
    ...
    , checkPredicate "can increase margin"
        (assertAccountBalance (ftoShort F.testAccounts) (== Bcc.entropicValueOf 1936)
        .&&. assertAccountBalance (ftoLong F.testAccounts) (== Bcc.entropicValueOf 2410))
        increaseMarginTrace

    , checkPredicate "can settle early"
        (assertAccountBalance (ftoShort F.testAccounts) (== Bcc.entropicValueOf 0)
        .&&. assertAccountBalance (ftoLong F.testAccounts) (== Bcc.entropicValueOf 4246))
        settleEarlyTrace

     , checkPredicate "can pay out"
        (assertAccountBalance (ftoShort F.testAccounts) (== Bcc.entropicValueOf 1936)
        .&&. assertAccountBalance (ftoLong F.testAccounts) (== Bcc.entropicValueOf 2310))
        payOutTrace

    , goldenPir "test/Spec/future.pir" $$(ZerepochTx.compile [|| F.futureStateMachine ||])
...
-- | A futures contract over 187 units with a forward price of 1233 Entropic,
--   due at slot #100.
...
theFuture startTime = Future {
    ...
    ftInitialMargin = Bcc.entropicValueOf 800,
...
payOut hdl = do
    startTime <- TimeSlot.scSlotZeroTime <$> Trace.getSlotConfig
    let
        spotPrice = Bcc.entropicValueOf 1124
...
-- | Margin penalty
penalty :: Value
penalty = Bcc.entropicValueOf 1000

-- | The forward price agreed at the beginning of the contract.
forwardPrice :: Value
forwardPrice = Bcc.entropicValueOf 1123
...
-- | Increase the margin of the 'Long' role by 100 entropic
increaseMargin :: ContractHandle () FutureSchema FutureError -> EmulatorTrace ()
increaseMargin hdl = do
    Trace.callEndpoint @"increase-margin" hdl (Bcc.entropicValueOf 100, Long)
    void $ Trace.waitNSlots 2

-- | Call 'settleEarly' with a high spot price (11240 entropic)
settleEarly :: ContractHandle () FutureSchema FutureError -> EmulatorTrace ()
settleEarly hdl = do
    startTime <- TimeSlot.scSlotZeroTime <$> Trace.getSlotConfig
    let
        spotPrice = Bcc.entropicValueOf 11240
