...
import           Zerepoch.Contracts.Escrow
...
tests = testGroup "escrow"
    [ let con = void $ payEp @() @EscrowSchema @EscrowError (escrowParams startTime) in
      checkPredicate "can pay"
        ...
        $ do
          ...
          Trace.callEndpoint @"pay-escrow" hdl (Bcc.entropicValueOf 10)
    ...
    , let con = void $ selectEither (payEp @()
      ...
      checkPredicate "can redeem"
        ( assertDone con (Trace.walletInstanceTag w3) (const True) "escrow redeem not done"
          .&&. walletFundsChange w1 (Bcc.entropicValueOf (-10))
          .&&. walletFundsChange w2 (Bcc.entropicValueOf 10)
    ...
    , checkPredicate "can redeem even if more money than required has been paid in"
          ...
          -- in this test case we pay in a total of 40 entropic (10 more than required), for
          -- the same contract as before, requiring 10 entropic to go to wallet 1 and 20 to
          -- wallet 2.
          --
          -- The scenario is
...
escrowParams startTime =
  EscrowParams
    ...
    , escrowTargets  =
        [ payToPubKeyTarget (pubKeyHash $ walletPubKey w1) (Bcc.entropicValueOf 10)
        , payToPubKeyTarget (pubKeyHash $ walletPubKey w2) (Bcc.entropicValueOf 20)
        ]
    }
...
redeemTrace = do
    ...
    Trace.callEndpoint @"pay-escrow" hdl1 (Bcc.entropicValueOf 20)
    Trace.callEndpoint @"pay-escrow" hdl2 (Bcc.entropicValueOf 10)
...
redeem2Trace = do
    ...
    Trace.callEndpoint @"pay-escrow" hdl1 (Bcc.entropicValueOf 20)
    Trace.callEndpoint @"pay-escrow" hdl2 (Bcc.entropicValueOf 10)
    Trace.callEndpoint @"pay-escrow" hdl3 (Bcc.entropicValueOf 10)
    _ <- Trace.waitNSlots 1
