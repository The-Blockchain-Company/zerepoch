...
import           Zerepoch.Contracts.TokenAccount (Account (..), TokenAccountError, TokenAccountSchema,
                                                tokenAccountContract)
import qualified Zerepoch.Contracts.TokenAccount as Accounts
...
tests = testGroup "token account"
    ...
    , checkPredicate "Pay into the account"
        (assertNoFailedTransactions
        .&&. assertNotDone contract (Trace.walletInstanceTag w1) "contract should not have any errors"
        .&&. walletFundsChange w1 (Bcc.entropicValueOf (-10) <> theToken))
        $ do
            hdl <- Trace.activateContractWallet w1 contract
            Trace.callEndpoint @"new-account" hdl (tokenName, Ledger.pubKeyHash $ walletPubKey w1)
            _ <- Trace.waitNSlots 3
            Trace.callEndpoint @"pay" hdl (account, Bcc.entropicValueOf 10)
    ...
    , checkPredicate "Transfer & redeem all funds"
        ...
        .&&. walletFundsChange w2 (theToken <> Bcc.entropicValueOf 10)
        )
        tokenAccountTrace
...
-- | Create a new token account for wallet 1, pay 100 entropic to the token
