...
import           Bcc.Crypto.Hash                as Crypto
...
import           Ledger                             (Bcc, Slot (..), Value, pubKeyHash)
import qualified Ledger.Bcc                         as Bcc
import           Zerepoch.Contract                    hiding (currentSlot)
...
import           Zerepoch.Contracts.Auction           hiding (Bid)
...
mpsHash = Value.CurrencySymbol $ ZerepochTx.toBuiltin $ Crypto.hashToBytes $ Crypto.hashWith @Crypto.Blake2b_224 id "ffff"
        ...
        case cmd of
            ...
            Bid w bid -> do
                ...
                when (bid > current && slot <= end) $ do
                    withdraw w $ Bcc.entropicValueOf bid
                    deposit leader $ Bcc.entropicValueOf current
                    currentBid $= bid
                    winner     $= w
        slot' <- viewModelState currentSlot
...
tests =
    testGroup "auction"
        [ checkPredicateOptions options "run an auction"
            ...
            .&&. walletFundsChange w1 (Bcc.toValue trace1WinningBid <> inv theToken)
            .&&. walletFundsChange w2 (inv (Bcc.toValue trace1WinningBid) <> theToken))
            auctionTrace1
        , checkPredicateOptions options "run an auction with multiple bids"
            ...
            .&&. walletFundsChange w1 (Bcc.toValue trace2WinningBid <> inv theToken)
            .&&. walletFundsChange w2 (inv (Bcc.toValue trace2WinningBid) <> theToken)
