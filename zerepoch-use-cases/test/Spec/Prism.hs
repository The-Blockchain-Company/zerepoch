...
import           Zerepoch.Contract.Test.ContractModel as ContractModel

import           Test.QuickCheck                    as QC hiding ((.&&.))
import           Test.Tasty
import           Test.Tasty.QuickCheck              (testProperty)

import           Zerepoch.Contracts.Prism             hiding (mirror)
import qualified Zerepoch.Contracts.Prism.Credential  as Credential
import qualified Zerepoch.Contracts.Prism.Mirror      as C
import           Zerepoch.Contracts.Prism.STO         (STOData (..))
import qualified Zerepoch.Contracts.Prism.STO         as STO
import qualified Zerepoch.Contracts.Prism.Unlock      as C
    ...
    nextState cmd = do
        ...
        case cmd of
            ...
            Call w    -> do
              ...
              when (iss && pend) $ do
                transfer w issuer (Bcc.entropicValueOf numTokens)
...
tests = testGroup "PRISM"
    [ checkPredicate "withdraw"
        ...
        .&&. walletFundsChange issuer (Bcc.entropicValueOf numTokens)
        .&&. walletFundsChange user (Bcc.entropicValueOf (negate numTokens) <> STO.coins stoData numTokens)
        )
        prismTrace
