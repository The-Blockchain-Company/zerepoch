...
import           Zerepoch.Contract.Wallet         (ExportTx (..))
import qualified Zerepoch.Contracts.Crowdfunding  as Crowdfunding
import qualified Zerepoch.Contracts.Uniswap.Trace as Uniswap
import           Zerepoch.Trace                   (Command (..), ScriptsConfig (..), showStats, writeScriptsTo)
...
networkIdParser =
    let p = C.Testnet . C.NetworkMagic <$> option auto (long "network-magic" <> short 'n' <> help "Bcc network magic. If none is specified, mainnet addresses are generated.")
    in p <|> pure C.Mainnet
...
progParser =
    ...
    in info
        ...
        <> header "zerepoch-use-cases-scripts - extract validators and partial transactions from emulator traces"
        )
