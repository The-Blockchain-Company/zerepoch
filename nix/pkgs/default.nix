...
let
  ...
  bcc-repo-tool = exeFromExtras "bcc-repo-tool";
  ...
  # These are needed to pull the bcc-cli and bcc-node in the nix-shell.
  inherit (haskell.project.hsPkgs.bcc-cli.components.exes) bcc-cli;
  inherit (haskell.project.hsPkgs.bcc-node.components.exes) bcc-node;
  ...
  updateMaterialized = pkgs.writeShellScriptBin "updateMaterialized" ''
    ...
    $(nix-build default.nix -A zerepoch.haskell.project.plan-nix.passthru.updateMaterialized --argstr system x86_64-linux)
    $(nix-build default.nix -A zerepoch.haskell.project.plan-nix.passthru.updateMaterialized --argstr system x86_64-darwin)
    $(nix-build default.nix -A zerepoch.haskell.project.plan-nix.passthru.updateMaterialized --argstr system windows)
    $(nix-build default.nix -A zerepoch.haskell.project.projectCross.mingwW64.plan-nix.passthru.updateMaterialized --argstr system x86_64-linux)

    # This updates the sha files for the extra packages
    $(nix-build default.nix -A zerepoch.haskell.extraPackages.updateAllShaFiles --argstr system x86_64-linux)
    $(nix-build default.nix -A zerepoch.haskell.extraPackages.updateAllShaFiles --argstr system x86_64-darwin)
  '';
  ...
  # * Run `nix-shell --run 'cd zerepoch-playground-client ; update-client-deps'
  #
  ...
  # combined haddock documentation for all public zerepoch libraries
  zerepoch-haddock-combined =
    let
    ...
    in
    pkgs.callPackage ./zerepoch-haddock-combined {
      inherit haskell haddock-combine;
      inherit (pkgs) haskell-nix;
    };

  # Collect everything to be exported under `zerepoch.lib`: builders/functions/utils
  lib = rec {
...
in
{
  ...
  inherit haskell agdaPackages cabal-install bcc-repo-tool stylish-haskell hlint haskell-language-server haskell-language-server-wrapper hie-bios bcc-cli bcc-node;
  ...
  inherit web-ghc;
  inherit easyPS zerepoch-haddock-combined;
