...
let
  ...
  bccRepoToolProject = haskell-nix.cabalProject' {
    src = sources.bcc-repo-tool;
    inherit compiler-nix-name index-state checkMaterialization;
    plan-sha256 = lib.removeSuffix "\n" (builtins.readFile ./bcc-repo-tool.sha);
    sha256map = {
      "https://github.com/The-Blockchain-Company/nix-archive"."7dcf21b2af54d0ab267f127b6bd8fa0b31cfa49d" = "0mhw896nfqbd2iwibzymydjlb3yivi9gm0v2g1nrjfdll4f7d8ly";
    };
  };
  ...
  updateAllShaFiles = writeShellScript "updateShaFiles" ''
    ...
    ${updateShaFile bccRepoToolProject ./bcc-repo-tool.sha}
  '';
in
{
  ...
  inherit (bccRepoToolProject.hsPkgs) bcc-repo-tool;
