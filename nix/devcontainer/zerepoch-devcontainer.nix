...
{ pkgs, zerepoch }:
let
  shell = (zerepoch.haskell.project.shellFor { withHoogle = false; });
...
pkgs.callPackage (import ./devcontainer.nix) {
  inherit pkgs;
  name = "zerepoch-devcontainer";
  tag = "latest";
  nonRootUser = "zerepoch";
  extraContents = [
    shell.ghc
    zerepoch.haskell-language-server
    zerepoch.cabal-install
    pkgs.binutils
    pkgs.pkg-config
  ];
  extraCommands = ''
