...
pkgs.recurseIntoAttrs {
  ...
  zerepoch-core-spec = import ../zerepoch-core-spec { inherit buildLatexDoc; };
  ...
  zerepoch-report = import ../zerepoch-report { inherit buildLatexDoc; };
  cost-model-notes = import ../notes/cost-model-notes { inherit buildLatexDoc; };

  site = pkgs.callPackage ../doc {
    inherit (zerepoch) sphinx-markdown-tables sphinxemoji;
    inherit (zerepoch.sphinxcontrib-haddock) sphinxcontrib-haddock sphinxcontrib-domaintools;
    combined-haddock = zerepoch.zerepoch-haddock-combined;
