...
import Capability.Simeon (class ManageSimeon, createContract, createPendingFollowerApp, followContract, followContractWithPendingFollowerApp, getFollowerApps, getRoleContracts, redeem, subscribeToZerepochApp)
import Capability.SimeonStorage (class ManageSimeonStorage, getWalletLibrary, insertIntoContractNicknames)
import Capability.Toast (class Toast, addToast)
import Clipboard (class MonadClipboard)
import Contract.Lenses (_mSimeonParams, _nickname, _selectedStep)
...
import Simeon.Client (ContractHistory, _chHistory, _chParams)
import Simeon.Deinstantiate (findTemplate)
import Simeon.Execution.State (getAllPayments)
import Simeon.Extended.Metadata (_metaData)
import Simeon.PAB (ZerepochAppId)
import Simeon.Semantics (SimeonData, SimeonParams, Party(..), Payee(..), Payment(..), Slot(..), _simeonContract)
...
mkInitialState :: WalletLibrary -> WalletDetails -> Map ZerepochAppId ContractHistory -> Map ZerepochAppId String -> Slot -> State
...
handleAction input@{ currentSlot } UpdateFromStorage = do
  ...
  when (dataProvider == LocalStorage) do
    ...
    for_ ajaxFollowerApps \followerApps ->
      let
        unfoldedFollowerApps :: Array (Tuple ZerepochAppId ContractHistory)
        unfoldedFollowerApps = toUnfoldable followerApps
      in
        void $ for unfoldedFollowerApps \(zerepochAppId /\ contractHistory) -> handleAction input $ UpdateContract zerepochAppId contractHistory

-- this handler takes the wallet companion app state (which contains SimeonParams of all the contracts)
...
handleAction input (UpdateFollowerApps companionAppState) = do
  ...
  let
    contractExists simeonParams = elem (Just simeonParams) (view _mSimeonParams <$> values existingContracts)

    newContracts = filterKeys (not contractExists) companionAppState

    newContractsArray :: Array (Tuple SimeonParams SimeonData)
    newContractsArray = toUnfoldable newContracts
  void
    $ for newContractsArray \(simeonParams /\ simeonData) -> do
        let
          mTemplate = findTemplate $ view _simeonContract simeonData

          hasRightMetaData :: Contract.State -> Boolean
          hasRightMetaData { mSimeonParams, metadata } = case mSimeonParams, mTemplate of
        ...
        -- we create a follower app immediately as a placeholder (and remember its ZerepochAppId), then we wait for
        -- the wallet companion app to tell us the SimeonParams of the contract we created, and pass those to the
        -- pending follower app. Fine. But when we're using the LocalStorage as our dataProvider, this workflow
        -- doesn't quite work. This is because the LocalStorage doesn't keep a record of ZerepochAppIds, but just
        -- derives them as a function of the SimeonParams (see note [SimeonParams] in Capability.Simeon). So
        -- when we create a pending follower app with LocalStorage, we get back what is ultimately the wrong
        -- ZerepochAppId (and we can't know the right one until we've generated the SimeonParams). At that point we
        -- could try to update the ZerepochAppId, but it's simpler to just delete the pending follower app and
        -- create a new one with the right ID (and the right key in the contracts map).
        --
        -- This solution is ugly for two reasons: (1) In general, the Simeon capability should be the only thing
        ...
        --
        -- (Note: There are sensible LocalStorage implementations of these useless functions in the Simeon capability.
        ...
        case dataProvider of
          SimeonPAB -> do
            ajaxFollowerApp <- case mPendingContract of
              Just { key: followerAppId } -> followContractWithPendingFollowerApp walletDetails simeonParams followerAppId
              Nothing -> followContract walletDetails simeonParams
            case ajaxFollowerApp of
              Left decodedAjaxError -> addToast $ decodedAjaxErrorToast "Failed to load new contract." decodedAjaxError
              Right (followerAppId /\ contractHistory) -> subscribeToZerepochApp dataProvider followerAppId
          LocalStorage -> do
            ajaxFollowerApp <- followContract walletDetails simeonParams
            case ajaxFollowerApp of
              Left decodedAjaxError -> addToast $ decodedAjaxErrorToast "Failed to load new contract." decodedAjaxError
...
handleAction input@{ currentSlot } (UpdateContract followerAppId contractHistory) =
  ...
  in
    ...
    for_ chParams \(simeonParams /\ simeonData) -> do
      walletDetails <- use _walletDetails
      contracts <- use _contracts
      case lookup followerAppId contracts of
        Just contractState -> do
          ...
          modifying _contracts $ insert followerAppId $ Contract.updateState walletDetails simeonParams currentSlot chHistory contractState
...
-- wallet holds the token, and then calls the "redeem" endpoint of the main simeon app for each
...
handleAction input (RedeemPayments followerAppId) = do
  walletDetails <- use _walletDetails
  contracts <- use _contracts
  for_ (lookup followerAppId contracts) \{ executionState, mSimeonParams, userParties } ->
    for_ mSimeonParams \simeonParams ->
      let
        ...
        for (List.fromFoldable userParties) \party ->
          ...
          in
            for paymentsToParty \payment -> case payment of
              Payment _ (Party (Role tokenName)) _ -> void $ redeem walletDetails simeonParams tokenName
              _ -> pure unit
    ...
    case instantiateExtendedContract currentSlot templateState of
      ...
      Just contract -> do
        ...
        case ajaxCreateContract of
          ...
          _ -> do
            -- We create a follower app now with no SimeonParams; when the next notification of a new contract
            -- notification comes in, a follower app with no SimeonParams but the right metadata will be used.
