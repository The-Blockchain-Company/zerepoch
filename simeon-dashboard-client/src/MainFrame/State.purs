...
import Capability.Simeon (class ManageSimeon, getFollowerApps, getRoleContracts, subscribeToZerepochApp, subscribeToWallet, unsubscribeFromZerepochApp, unsubscribeFromWallet)
import Capability.SimeonStorage (class ManageSimeonStorage, getContractNicknames, getWalletLibrary)
...
import Simeon.Client (LastResult(..), SimeonError(..))
import Simeon.PAB (ZerepochAppId)
import Zerepoch.PAB.Webserver.Types (CombinedWSStreamToClient(..), InstanceStatusToClient(..))
...
import WalletData.Lenses (_assets, _companionAppId, _simeonAppId, _previousCompanionAppState, _wallet, _walletInfo)
    ...
    (WS.ReceiveMessage (Right streamToClient)) -> case streamToClient of
      ...
      InstanceUpdate contractInstanceId instanceStatusToClient -> case instanceStatusToClient of
        NewObservableState rawJson -> do
          let
            zerepochAppId = toFront contractInstanceId
          mDashboardState <- peruse _dashboardState
          ...
          for_ mDashboardState \dashboardState -> do
            ...
            if (zerepochAppId == walletCompanionAppId) then case runExcept $ decodeJSON $ unwrap rawJson of
              ...
              let
                simeonAppId = view (_walletDetails <<< _simeonAppId) dashboardState
              -- if this is the wallet's SimeonApp...
              if (zerepochAppId == simeonAppId) then case runExcept $ decodeJSON $ unwrap rawJson of
                Left decodingError -> addToast $ decodingErrorToast "Failed to parse contract update." decodingError
                Right lastResult -> case lastResult of
                  ...
                  SomeError simeonError -> addToast $ errorToast "something went wrong" Nothing
                  Unknown -> pure unit
              -- otherwise this should be one of the wallet's WalletFollowerApps
                ...
                Right contractHistory -> do
                  handleAction $ DashboardAction $ Dashboard.UpdateContract zerepochAppId contractHistory
                  handleAction $ DashboardAction $ Dashboard.RedeemPayments zerepochAppId
        -- Zerepoch contracts in general can change in other ways, but the Simeon contracts don't, so
...
handleAction (EnterWelcomeState walletLibrary walletDetails followerApps) = do
  { dataProvider } <- ask
  let
    followerAppIds :: Array ZerepochAppId
  ...
  unsubscribeFromZerepochApp dataProvider $ view _companionAppId walletDetails
  unsubscribeFromZerepochApp dataProvider $ view _simeonAppId walletDetails
  for_ followerAppIds $ unsubscribeFromZerepochApp dataProvider
...
handleAction (EnterDashboardState walletLibrary walletDetails) = do
  ...
  case ajaxFollowerApps of
    ...
    Right followerApps -> do
      ...
      subscribeToZerepochApp dataProvider $ view _companionAppId walletDetails
      subscribeToZerepochApp dataProvider $ view _simeonAppId walletDetails
      for_ followerAppIds $ subscribeToZerepochApp dataProvider
