...
import Capability.Simeon (class ManageSimeon, lookupWalletDetails, lookupWalletInfo)
import Capability.SimeonStorage (class ManageSimeonStorage, insertIntoWalletLibrary)
...
import Simeon.Semantics (Assets, Token(..))
...
defaultWalletDetails =
  ...
  , companionAppId: ZerepochAppId emptyUUID
  , simeonAppId: ZerepochAppId emptyUUID
...
handleAction (SaveWallet mTokenName) = do
  ...
  let
    mWalletId = parseZerepochAppId walletIdString
...
handleAction (WalletIdInputAction inputFieldAction) = do
  case inputFieldAction of
    InputField.SetValue walletIdString -> do
      ...
      for_ (parseZerepochAppId walletIdString) \walletId -> do
...
getBcc :: Assets -> BigInteger
getBcc assets = fromMaybe zero $ lookup "" =<< lookup "" (unwrap assets)
...
walletIdError remoteDataWalletInfo walletLibrary walletIdString = case parseZerepochAppId walletIdString of
  Nothing -> Just InvalidWalletId
  Just zerepochAppId
    | not $ isEmpty $ filter (\walletDetails -> walletDetails.companionAppId == zerepochAppId) walletLibrary -> Just DuplicateWalletId
  _ -> case remoteDataWalletInfo of
    Success _ -> Nothing
    Failure _ -> Just NonexistentWalletId
    _ -> Just UnconfirmedWalletId

parseZerepochAppId :: String -> Maybe ZerepochAppId
parseZerepochAppId zerepochAppIdString = case parseUUID zerepochAppIdString of
  Just uuid -> Just $ ZerepochAppId uuid
  Nothing -> Nothing
