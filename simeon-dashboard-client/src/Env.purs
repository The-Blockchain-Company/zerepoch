...
-- The frontend app can be run with two different data providers: the Simeon PAB (the PAB bundled
-- up with the Simeon Zerepoch contracts in one executable), or with the browser's localStorage
-- giving the local illusion of persistent and shared data. How this env property is set determines
-- the implementation of the functions in the ManageSimeon capability monad.
data DataProvider
  = SimeonPAB
  | LocalStorage
