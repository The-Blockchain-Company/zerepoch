{-
FIXME: This module duplicates some necessary Haskell types from Simeon.Client. These should
preferably be generated by purescript-bridge with a PSGenerator module created specifically for use
by the Simeon Run frontend.
-}
module Simeon.Client where
...
import Simeon.Semantics (Input, SimeonData, SimeonParams, Slot, TransactionError, TransactionInput, aesonCompatibleOptions)
import Zerepoch.Contract.StateMachine (InvalidTransition, SMContractError)
import Wallet.Types (ContractError)

-- This is the state of the main simeon (control) contract. Its purpose is to provide feedback if
-- anything goes wrong when we try to create a contract or apply inputs.
data LastResult
  = OK
  | SomeError SimeonError
  | Unknown
...
data SimeonError
  ...
  --     (a) there's a duplicate simeon contract (which could theoretially happen if someone is deliberately trying to break things)
  --     (b) the contract doesn't exist or has already closed
  --     (c) you don't have enough money
  | TransitionError (InvalidTransition SimeonData SimeonInput)
  -- ^ can arise when applying inputs if:
...
derive instance eqSimeonError :: Eq SimeonError

derive instance genericSimeonError :: Generic SimeonError _

instance encodeSimeonError :: Encode SimeonError where
  encode a = genericEncode defaultOptions a

instance decodeSimeonError :: Decode SimeonError where
  decode a = genericDecode defaultOptions a

type SimeonInput
  = Tuple SimeonSlotRange (Array Input)

type SimeonSlotRange
  = Tuple Slot Slot

-- This is the state of the follower contract. Its purpose is to provide us with an up-to-date
-- transaction history for a Simeon contract running on the blockchain.
newtype ContractHistory
  = ContractHistory
  { chParams :: Maybe (Tuple SimeonParams SimeonData)
...
_chParams :: Lens' ContractHistory (Maybe (Tuple SimeonParams SimeonData))
