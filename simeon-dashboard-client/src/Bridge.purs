...
import Prelude
import Bcc.Wallet.Types (WalletInfo(..)) as Back
...
import Simeon.PAB (ZerepochAppId(..)) as Front
import Simeon.Semantics (Assets(..), Slot(..)) as Front
import Network.RemoteData (RemoteData)
import Zerepoch.V1.Ledger.Crypto (PubKey(..), PubKeyHash(..)) as Back
import Zerepoch.V1.Ledger.Slot (Slot(..)) as Back
import Zerepoch.V1.Ledger.Value (CurrencySymbol(..), TokenName(..), Value(..)) as Back
import ZerepochTx.AssocMap (Map, fromTuples, toTuples) as Back
...
Simeon.Contract. This is because we want custom encode/decode instances for Simeon contracts anyway
(making the JSON more readable makes the JavaScript implementation of Simeon nicer, since this works
by writing the contract as JSON directly.

There are two issues with method 1. First, the Haskell code uses a custom `ZerepochTx.AssocMap` instead
...
Simeon contracts, since for these we have the custom JSON encode/decode instances.
-}
...
-- TODO: Simeon.Semantics.PubKey is currently just an alias for String
...
-- TODO: Simeon.Semantics.TokenName is currently just an alias for String
...
-- TODO: Simeon.Semantics.CurrencySymbol is currently just an alias for String
...
instance contractInstanceIdBridge :: Bridge Back.ContractInstanceId Front.ZerepochAppId where
  toFront (Back.ContractInstanceId { unContractInstanceId: JsonUUID uuid }) = Front.ZerepochAppId uuid
  toBack (Front.ZerepochAppId uuid) = Back.ContractInstanceId { unContractInstanceId: JsonUUID uuid }
