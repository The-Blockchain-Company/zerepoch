...
import Simeon.Execution.Lenses (_semanticState, _mNextTimeout)
import Simeon.Execution.State (expandBalances)
import Simeon.Execution.Types (NamedAction(..))
import Simeon.Extended (contractTypeName)
import Simeon.Extended.Metadata (_contractType)
import Simeon.PAB (transactionFee)
import Simeon.Semantics (Assets, Bound(..), ChoiceId(..), Party(..), Payee(..), Payment(..), Slot, SlotInterval(..), Token, TransactionInput(..), _accounts, getEncompassBound)
import Simeon.Semantics (Input(..)) as S
import Simeon.Slot (secondsDiff, slotToDateTime)
...
import WalletData.State (bccToken, getBcc)
...
actionConfirmationCard assets namedAction state =
  let
    ...
    transactionFeeItem = detailItem [ text "Transaction fee", sup_ [ text "*" ], text ":" ] [ text $ humanizeValue bccToken transactionFee ]
    ...
    -- TODO: when we allow custom currency other than bcc, we won't be able to add the deposit amount
    -- to the transaction fee (always in bcc)
    totalToPay = case namedAction of
      MakeDeposit _ _ _ amount -> amount + transactionFee
      _ -> transactionFee

    hasSufficientFunds = getBcc assets >= totalToPay
  in
    div_
      [ div [ classNames [ "flex", "font-semibold", "justify-between", "bg-lightgray", "p-5" ] ]
          [ span_ [ text "Demo wallet balance:" ]
          , span_ [ text $ humanizeValue bccToken $ getBcc assets ]
          ]
      , div [ classNames [ "px-5", "pb-6", "pt-3", "md:pb-8" ] ]
          ...
          , div
              ...
              [ text $ humanizeValue bccToken totalToPay ]
          ...
          , div [ classNames [ "bg-black", "text-white", "p-4", "mt-4", "rounded" ] ]
              ...
              , p [ classNames [ "pb-4", "border-b-half", "border-lightgray", "text-xs", "text-gray" ] ]
                  ...
                  [ text "In the demo all fees are fixed at 10 entropic, but in the live version the cost will depend on the status of the blockchain at the moment of the transaction." ]
              , div [ classNames [ "pt-4", "flex", "justify-between", "items-center" ] ]
                  [ a
                      -- FIXME: where should this link point to?
                      [ href "https://docs.bcc.org/en/latest/explore-bcc/bcc-fee-structure.html"
...
renderPaymentSummary stepNumber state step =
  let
    ...
    -- TODO: This function is currently hard-coded to BCC, we should rethink how to deal with this
    --       when we support multiple tokens
    paymentToMovement (Payment from (Account to) money) = Transfer from to bccToken $ getBcc money

    paymentToMovement (Payment from (Party to) money) = PayOut from to bccToken $ getBcc money
  ...
  let
    ...
    renderFeesSummary =
      div [ classNames [ "pt-4", "px-3", "border-t", "border-gray", "flex", "items-center", "text-xs" ] ]
        ...
        , span [ classNames [ "flex-grow", "text-right" ] ] [ text $ humanizeValue bccToken transactionFee ]
        ]
    ...
    case currentTab, isContractClosed state, isJust pendingTransaction of
      ...
      Balances, isClosed, _ ->
        let
          ...
          atStart = expandBalances (Set.toUnfoldable $ Map.keys participants) [ bccToken ] balancesAtStart
...
renderBalances stepNumber state stepBalance =
  let
    -- TODO: Right now we only have one type of Token (bcc), but when we support multiple tokens we may want to group by
