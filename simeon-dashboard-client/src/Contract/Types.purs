...
import Simeon.Execution.Types (State) as Execution
import Simeon.Extended.Metadata (MetaData)
import Simeon.PAB (ZerepochAppId)
import Simeon.Semantics (AccountId, Accounts, ChoiceId, ChosenNum, SimeonParams, Party, Payment, Slot, Token(..), TransactionInput, Value)
...
type State
  = { nickname :: String
    ...
    -- Every contract needs SimeonParams, but this is a Maybe because we want to create "placeholder"
    -- contracts when a user creates a contract, to show on the page until the blockchain settles and
    -- we get the SimeonParams back from the PAB (through the SimeonFollower app).
    , mSimeonParams :: Maybe SimeonParams
