...
import Capability.Simeon (class ManageSimeon, applyTransactionInput)
import Capability.SimeonStorage (class ManageSimeonStorage, insertIntoContractNicknames)
import Capability.Toast (class Toast, addToast)
import Contract.Lenses (_executionState, _mSimeonParams, _namedActions, _nickname, _participants, _pendingTransaction, _previousSteps, _selectedStep, _tab, _userParties)
...
import Simeon.Execution.Lenses (_contract, _semanticState, _history, _pendingTimeouts, _previousTransactions)
import Simeon.Execution.State (expandBalances, extractNamedActions, getActionParticipant, isClosed, mkTx, nextState, timeoutState)
import Simeon.Execution.State (mkInitialState) as Execution
import Simeon.Execution.Types (NamedAction(..), PastAction(..))
import Simeon.Execution.Types (PastState, State, TimeoutInfo) as Execution
import Simeon.Extended.Metadata (MetaData, emptyContractMetadata)
import Simeon.HasParties (getParties)
import Simeon.Client (ContractHistory, _chHistory, _chParams)
import Simeon.Semantics (Contract(..), SimeonParams(..), Party(..), Slot, SlotInterval(..), TransactionInput(..), _accounts, _simeonContract, _simeonState, _minSlot, _rolesCurrency)
import Simeon.Semantics (Input(..), State(..)) as Semantic
import Toast.Types (ajaxErrorToast, successToast)
import WalletData.Lenses (_assets, _pubKeyHash, _walletInfo)
import WalletData.State (bccToken)
...
dummyState =
  ...
  emptySimeonData = { simeonContract: contract, simeonState: emptySimeonState }

  emptySimeonState = Semantic.State { accounts: mempty, choices: mempty, boundValues: mempty, minSlot: zero }

-- this is for making a placeholder state for the user who created the contract, used for displaying
-- something before we get the SimeonParams back from the WalletCompanion app
...
-- contract is given a role in it, and gets the SimeonParams at the same time as they hear about
-- everything else
...
mkInitialState walletDetails currentSlot nickname contractHistory =
  ...
  in
    bind chParams \(simeonParams /\ simeonData) ->
      let
        chHistory = view _chHistory contractHistory

        contract = view _simeonContract simeonData

        mTemplate = findTemplate contract

        minSlot = view (_simeonState <<< _minSlot) simeonData
        ...
        flip map mTemplate \template ->
          let
            initialState =
              ...
              , pendingTransaction: Nothing
              , previousSteps: mempty
              , mSimeonParams: Just simeonParams
              , selectedStep: 0
              , metadata: template.metaData
              , participants: getParticipants contract
              , userParties: getUserParties walletDetails simeonParams
              , namedActions: mempty
              }
...
updateState :: WalletDetails -> SimeonParams -> Slot -> Array TransactionInput -> State -> State
updateState walletDetails simeonParams currentSlot transactionInputs state =
  let
    ...
    -- If the `SimeonParams` are `Nothing`, that means this is the first update we've received for
    -- a placeholder contract, and we'll need to set the `SimeonParams` now and also work out the
    -- `userParties` (because these depend on the `SimeonParams`).
    mSetParamsAndParties =
      if isNothing $ state ^. _mSimeonParams then
        set _mSimeonParams (Just simeonParams)
          <<< set _userParties (getUserParties walletDetails simeonParams)
      else
        identity
...
getUserParties :: WalletDetails -> SimeonParams -> Set Party
getUserParties walletDetails simeonParams =
  let
    pubKeyHash = view (_walletInfo <<< _pubKeyHash) walletDetails

    assets = view _assets walletDetails

    rolesCurrency = view _rolesCurrency simeonParams
...
handleAction input@{ currentSlot, walletDetails } (ConfirmAction namedAction) = do
  ...
  mSimeonParams <- use _mSimeonParams
  for_ mSimeonParams \simeonParams -> do
    let
      contractInput = toInput namedAction

      txInput = mkTx currentSlot (executionState ^. _contract) (Unfoldable.fromMaybe contractInput)
    ajaxApplyInputs <- applyTransactionInput walletDetails simeonParams txInput
...
transactionsToStep state { balancesAtStart, balancesAtEnd, txInput, resultingPayments, action } =
  let
    ...
    expandedBalancesAtStart = expandBalances (Set.toUnfoldable $ Map.keys participants) [ bccToken ] balancesAtStart

    expandedBalancesAtEnd = expandBalances (Set.toUnfoldable $ Map.keys participants) [ bccToken ] balancesAtEnd
...
timeoutToStep state { slot, missedActions } =
  let
    ...
    expandedBalances = expandBalances (Set.toUnfoldable $ Map.keys participants) [ bccToken ] balances
  in
    { tab: Tasks
    , expandPayments: false
