...
import Simeon.Extended.Metadata (ContractTemplate, MetaData, NumberFormat(..), _contractName, _metaData, _slotParameterDescriptions, _valueParameterDescription, _valueParameterFormat, _valueParameterInfo)
import Data.Map.Ordered.OMap as OMap
import Simeon.Market (contractTemplates)
import Simeon.PAB (contractCreationFee)
import Simeon.Semantics (Assets, TokenName)
import Simeon.Template (orderContentUsingMetadata)
...
contractSetup walletLibrary state =
  let
    ...
    contractNicknameInputDisplayOptions =
      ...
      , placeholder: "E.g. My Simeon contract"
      , readOnly: false
...
contractReview assets state =
  let
    hasSufficientFunds = getBcc assets >= contractCreationFee
      ...
      , div
          ...
          , div [ classNames [ "px-5", "pb-6", "md:pb-8" ] ]
              ...
              , p
                  ...
                  [ text $ humanizeValue bccToken contractCreationFee ]
              , div
...
-- TODO: This function is also included in the Simeon Playground code. We could/should move it
-- into a shared folder, but it's not obvious where. It could go in the Hint module, but then it
-- would introduce an unnecessary markdown dependency into the Zerepoch Playground. So some more
