...
import ZerepochTx.AssocMap (unionWith)
import ZerepochTx.AssocMap as AssocMap
import Zerepoch.V1.Ledger.Bcc (Bcc(..))
import Zerepoch.V1.Ledger.Interval (Extended(..), Interval, LowerBound(..), UpperBound(..), _Interval)
...
bccToValue :: Bcc -> Value
bccToValue (Entropic bcc) =
  Value
    { getValue:
        AssocMap.Map
          [ JsonTuple
              ( Tuple
                  ...
                  (AssocMap.Map [ JsonTuple (Tuple (TokenName { unTokenName: "" }) bcc.getEntropic) ])
              )
          ]
    }
