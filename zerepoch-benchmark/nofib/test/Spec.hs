{- | Tests for the Zerepoch nofib benchmarks, mostly comparing the result of Zerepoch
evaluation with the result of Haskell evaluation. Lastpiece is currently omitted
because its memory consumption as a Zerepoch program is too great to allow it to
run to completion. -}
...
import           Zerepoch.Benchmark.Prime                   (Result (Composite, Prime))
...
import qualified ZerepochTx                                 as Tx
import qualified UntypedZerepochCore                        as UPLC
import           UntypedZerepochCore.Evaluation.Machine.Cek as UPLC (EvaluationResult (..), unsafeEvaluateCekNoEmit)
...
{- | Evaluate a PLC term using the CEK machine and compare it with an expected
   ...
   comparing the result of a compiled Zerepoch program with the value produced by
   evaluating the program as a Haskell term: this will certainly be safe for
   simple types, but it is not guaranteed that evaluation commutes with Zerepoch
...
-- over 'test' so that we can test both Haskell and Zerepoch evaluation.
...
testPrimalityPlc = mkPrimalityTest "primality test (Zerepoch Core)"
...
allTests :: TestTree
allTests =
  testGroup "zerepoch nofib tests"
    [ testClausify
    , testKnights
    , testPrimalityHs
    , testPrimalityPlc
