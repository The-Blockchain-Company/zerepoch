{-% nofib/spectral/constraints converted to Zerepoch.
...
module Zerepoch.Benchmark.Queens where
...
import           ZerepochTx.Prelude   as TxPrelude
...
{-% This program computes the number of mutually non-attacking arrangements
    ...
    Compiling into Zerepoch, I tried to run all five algorithms one after the other
    ...
    are figures for the time and memory usage for one run of the Zerepoch version
...
-- % Compile a Zerepoch Core term which runs nqueens on given arguments
...
-----------------------------------------------------------
-- % Various standard things reimplemented for Zerepoch
-----------------------------------------------------------
