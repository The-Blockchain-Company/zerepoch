...
module Zerepoch.Benchmark.Prime where
...
import           ZerepochCore.Default (DefaultFun, DefaultUni)
import qualified ZerepochCore.Pretty  as PLC
import qualified ZerepochTx           as Tx
import           ZerepochTx.Builtins  (divideInteger, modInteger)
import           ZerepochTx.Prelude   as Tx hiding (divMod, even)
import           ZerepochTx.Ratio     (divMod)
...
{-% **CAUTION**.  Many functions have to return a pair (x, newState) containing their
   ...
   amount of time and memory required.  This is due to a restriction in the ZerepochTx
