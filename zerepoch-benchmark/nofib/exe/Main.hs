...
import qualified Zerepoch.Benchmark.LastPiece               as LastPiece
...
import           ZerepochTx.Prelude                         as Zerepoch hiding (fmap, mappend, (<$), (<$>), (<*>), (<>))
...
options = hsubparser
  ...
  <> command "runPLC"
     (info (RunPLC <$> progAndArgs)
      (progDesc "compile the program to Zerepoch Core and evaluate it using the CEK machine"))
  <> command "runHaskell"
     (info (RunHaskell <$> progAndArgs)
      (progDesc "run the program directly as Haskell"))
  <> command "dumpPLC"
     (info (DumpPLC <$> progAndArgs)
      (progDesc "print the program (applied to arguments) as Zerepoch Core source on standard output"))
...
description = "This program provides operations on a number of Zerepoch programs "
              ++ "ported from the nofib Haskell test suite.  "
              ++ "The programs are written in Haskell and can be run directly "
              ++ "or compiled into Zerepoch Core and run on the CEK machine.  "
...
footerInfo = text "Every command takes the name of a program and a (possbily empty) list of arguments."
           ...
           <> line <> line
           <> string ("The 'dump' commands construct a Zerepoch Core term applying the program to its\n"
