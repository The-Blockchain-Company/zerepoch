...
import           Paths_zerepoch_benchmark                   (getDataDir, getDataFileName)
...
zerepoch-use-cases:zerepoch-use-cases-scripts, which runs various contracts on the
...
directory here.  Type 'cabal run zerepoch-use-cases:zerepoch-use-cases-scripts
zerepoch-benchmark/validation/data scripts' in the root directory of the Zerepoch
...
in `.stack_work` (and accessed via Paths_zerepoch_benchmark), and if a file in
...
{- Note also that this directory (and any subdirectories) must be included in the
   "data-files" section of the cabal file to ensure that Paths_zerepoch_benchmark
   still works. -}
...
{- | The Criterion configuration returned by `getConfig` will cause an HTML report
   ...
   `zerepoch-benchmark` directory by default.  The -o option can be used to change
...
parserInfo cfg =
    info (helper <*> parseBenchOptions cfg) $ header "Zerepoch Core validation benchmark suite"

{- Run the benchmarks.  You can run groups of benchmarks by typing things like
     `stack bench -- zerepoch-benchmark:validation --ba crowdfunding`
   or
     `cabal bench -- zerepoch-benchmark:validation --benchmark-options crowdfunding`.
-}
main :: IO ()
main = do
