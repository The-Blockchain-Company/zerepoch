...
import           Zerepoch.Contract.Test                  hiding ((.&&.))
import qualified Zerepoch.Contract.Test                  as T
import           Zerepoch.Contract.Types                 (_observableState)
import qualified Zerepoch.Trace.Emulator                 as Trace
import           Zerepoch.Trace.Emulator.Types           (instContractState)
...
zeroCouponBondTest = checkPredicateOptions (defaultCheckOptions & maxSlot .~ 250) "Zero Coupon Bond Contract"
    ...
    T..&&. assertDone simeonZerepochContract (Trace.walletInstanceTag alice) (const True) "contract should close"
    T..&&. assertDone simeonZerepochContract (Trace.walletInstanceTag bob) (const True) "contract should close"
    T..&&. walletFundsChange alice (entropicValueOf 150)
    T..&&. walletFundsChange bob (entropicValueOf (-150))
    T..&&. assertAccumState simeonZerepochContract (Trace.walletInstanceTag alice) ((==) OK) "should be OK"
    T..&&. assertAccumState simeonZerepochContract (Trace.walletInstanceTag bob) ((==) OK) "should be OK"
    ) $ do
    -- Init a contract
    let alicePk = PK (pubKeyHash $ walletPubKey alice)
        bobPk = PK (pubKeyHash $ walletPubKey bob)

    let params = defaultSimeonParams

    let zeroCouponBond = When [ Case
            (Deposit alicePk alicePk bcc (Constant 850))
            (Pay alicePk (Party bobPk) bcc (Constant 850)
                (When
                    [ Case (Deposit alicePk bobPk bcc (Constant 1000)) Close] (Slot 200) Close
                ))] (Slot 100) Close
    ...
    Trace.callEndpoint @"apply-inputs" aliceHdl (params, Nothing, [IDeposit alicePk alicePk bcc 850])
    Trace.waitNSlots 2

    Trace.callEndpoint @"apply-inputs" bobHdl (params, Nothing, [IDeposit alicePk bobPk bcc 1000])
...
errorHandlingTest = checkPredicateOptions (defaultCheckOptions & maxSlot .~ 250) "Error handling"
    (assertAccumState simeonZerepochContract (Trace.walletInstanceTag alice)
    ...
    Trace.callEndpoint @"apply-inputs" aliceHdl (params, Nothing, [IDeposit alicePk alicePk bcc 1000])
    Trace.waitNSlots 2
    pure ()
...
trustFundTest = checkPredicateOptions (defaultCheckOptions & maxSlot .~ 200) "Trust Fund Contract"
    ...
    T..&&. walletFundsChange alice (entropicValueOf (-256) <> Val.singleton (rolesCurrency params) "alice" 1)
    T..&&. walletFundsChange bob (entropicValueOf 256 <> Val.singleton (rolesCurrency params) "bob" 1)
    ...
    -- way.
    --T..&&. assertAccumState simeonFollowContract "bob follow"
    --    (\state@ContractHistory{chParams, chHistory} ->
    --        case chParams of
    --            First (Just (mp, SimeonData{simeonContract})) -> mp == params && simeonContract == contract
    --            _                                               -> False) "follower contract state"
    --        --mp SimeonData{simeonContract} history
    ...
    bobHdl <- Trace.activateContractWallet bob simeonZerepochContract
    aliceHdl <- Trace.activateContractWallet alice simeonZerepochContract
    bobCompanionHdl <- Trace.activateContract bob simeonCompanionContract "bob companion"
    bobFollowHdl <- Trace.activateContract bob simeonFollowContract "bob follow"
    ...
    where
        ...
        contract = When [
            Case (Choice chId [Bound 10 1500])
                (When [Case
                    (Deposit "alice" "alice" bcc (ChoiceValue chId))
                        (When [Case (Notify (SlotIntervalStart `ValueGE` Constant 15))
                            (Pay "alice" (Party "bob") bcc
                                (ChoiceValue chId) Close)]
                        (Slot 40) Close)
                    ] (Slot 30) Close)
            ] (Slot 20) Close
        (params, _ :: TxConstraints SimeonInput SimeonData) =
            let con = setupSimeonParams @SimeonSchema @SimeonError
...
validatorSize :: IO ()
validatorSize = do
    let validator = Scripts.validatorScript $ typedValidator defaultSimeonParams
...
extractContractRolesTest = do
    ...
    extractContractRoles
        (Pay (Role "Alice") (Party (Role "Bob")) bcc (Constant 1) Close)
            @=? Set.fromList ["Alice", "Bob"]
    extractContractRoles
        (When [Case (Deposit (Role "Bob") (Role "Alice") bcc (Constant 10)) Close] 10 Close)
            @=? Set.fromList ["Alice", "Bob"]
    extractContractRoles
...
mulAnalysisTest = do
    let muliply = foldl (\a _ -> MulValue (UseValue $ ValueId "a") a) (Constant 1) [1..100]
        alicePk = PK $ pubKeyHash $ walletPubKey alice
        contract = If (muliply `ValueGE` Constant 10000) Close (Pay alicePk (Party alicePk) bcc (Constant (-100)) Close)
...
transferBetweenAccountsTest = do
    ...
    let simeonData = SimeonData {
                ...
                simeonState = state }
    let Just result@(constraints, SM.State (SimeonData {simeonState=State{accounts}}) balance) =
            mkSimeonStateMachineTransition
                defaultSimeonParams
                (SM.State simeonData (entropicValueOf 100))
                ((20, 30), [])
    balance @=? entropicValueOf 100
...
showWorksForContract contract = unsafePerformIO $ do
  res <- runInterpreter $ setImports ["Language.Simeon"]
