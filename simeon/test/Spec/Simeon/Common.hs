...
module Spec.Simeon.Common where
...
newtype SimeonScenario = SimeonScenario { mlInitialBalances :: Map PubKey Ledger.Value }
...
pangramContract = let
    ...
    in Assert TrueObs $ When
        [ Case (Deposit aliceAcc alicePk bcc valueExpr)
            (Let (ValueId "x") valueExpr
                (Pay aliceAcc (Party bobRole) bcc (UseValue (ValueId "x")) Close))
