...
{-| = Simeon: financial contracts domain specific language for blockchain
...
Semantics is based on <https://github.com/The-Blockchain-Company/simeon/blob/stable/src/Semantics.hs>

Simeon Contract execution is a chain of transactions,
where remaining contract and its state is passed through /Datum/,
and actions (i.e. /Choices/) are passed as
/Redeemer Script/

/Validation Script/ is always the same Simeon interpreter implementation, available below.
-}
...
import           Language.Simeon.Pretty  (Pretty (..))
import           Ledger                   (PubKeyHash (..), Slot (..), ValidatorHash)
import           Ledger.Value             (CurrencySymbol (..), TokenName (..))
import qualified Ledger.Value             as Val
import           ZerepochTx                 (makeIsDataIndexed)
import           ZerepochTx.AssocMap        (Map)
import qualified ZerepochTx.AssocMap        as Map
import           ZerepochTx.Lift            (makeLift)
import           ZerepochTx.Prelude         hiding (encodeUtf8, mapM, (<$>), (<*>), (<>))
import           ZerepochTx.Ratio           (denominator, numerator)
import           Prelude                  (mapM, (<$>))
...
{- Functions that used in Zerepoch Core must be inlineable,
   so their code is available for ZerepochTx compiler -}
{-# INLINABLE inBounds #-}
...
{-| Values include some quantities that change with time,
    including “the slot interval”, “the current balance of an account (in Entropic)”,
...
{-  Zerepoch doesn't support mutually recursive data types yet.
...
{-| Simeon has six ways of building contracts.
...
{-| Simeon contract internal state. Stored in a /Datum/ of a transaction output.
-}
...
{-| Input for a Simeon contract. Correspond to expected 'Action's.
-}
...
data SimeonParams = SimeonParams {
...
-- Lifting data types to Zerepoch Core
makeLift ''Party
...
makeLift ''SimeonData
makeIsDataIndexed ''SimeonData [('SimeonData,0)]
makeLift ''SimeonParams
