...
module Language.Simeon.Pretty where
...
import           Ledger.Bcc              (Bcc, getEntropic)
import           Ledger.Value
...
{-
    Currently, Simeon Playground saves a Haskell contract to a file,
...
instance Pretty Bcc where
    prettyFragment x = prettyFragment (getEntropic x)
