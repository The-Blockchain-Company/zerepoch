...
\label{sec:intro}
The Bcc blockchain~\citep{Bcc, Bcc-ledger-spec} uses a
...
transactions.  In Bcc, scripts will be programs in the Zerepoch Core
language~\citep{Zerepoch-Core-spec}. The Extended UTXO models are largely
...
\paragraph{ETUXO-1 on Bcc.}
The Bcc implementation of EUTXO-1 uses the primitives given in
\cref{fig:eutxo-1-types-bcc}.
...
\begin{ruledfigure}{H}
  ...
  \begin{array}{rll}
    ...
    \script & & \mbox{a Zerepoch Core program}\\
    \scriptAddr : \script \rightarrow \Address &=& s \mapsto \hash{s}\\
    \applyScript{\cdot} : \script \rightarrow \Data \times \cdots \times
    \Data \rightarrow \B & & \mbox{running the Zerepoch Core interpreter with a}\\
                             &&\mbox{script and a number of data objects as input}\\
  \end{array}
  \end{displaymath}
  \caption{Bcc primitives for the EUTXO-1 model}
  \label{fig:eutxo-1-types-bcc}
\end{ruledfigure}

\paragraph{Transaction identifiers.}  We assume that
each transaction has a unique identifier (in Bcc, the hash of a
...
\todokwxm{We may wish to implement a DEX to enable exchange of custom
  ...
  Will the fact that Bcc is treated differently from custom currencies
...
\subsubsection{Remarks}
\paragraph{ETUXO-2 on Bcc.}
The Bcc implementation of EUTXO-2 uses the primitives given in
\cref{fig:eutxo-2-types-bcc}.
Bcc also defines an \emph{native currency} and \emph{native currency token}.
This allows defining a native currency that behaves as a simple \qty{}. This
is used in Fig~\ref{fig:bcc-fee-validity}.
\begin{ruledfigure}{H}
  \begin{displaymath}
    \begin{array}{rll}
      \currency  &=& \H\\
    ...
    \end{array}
  \end{displaymath}
  \caption{Bcc primitives for the EUTXO-2 model}
  \label{fig:eutxo-2-types-bcc}
\end{ruledfigure}
...
\paragraph{Validation on Bcc.}
Bcc adds an additional rule in Fig~\ref{fig:bcc-fee-validity}, which
...
\begin{ruledfigure}{H}
  ...
  \end{displaymath}
  \caption{Validity of a transaction $t$ in the EUTXO-2 model}
  \label{fig:bcc-fee-validity}
\end{ruledfigure}

\subsection{The EUTXO-2 model in practice.}
See~\cite{Zerepoch-book} for examples of contracts which make use of the
...
\textit{script}%
\footnote{In the Bcc setting, scripts are Zerepoch Core
  programs~\citep{Zerepoch-Core-spec}.}
...
amounts of memory.  The precise details of fees in Bcc are outwith
...
transactions.  See~\cite{Zerepoch-book} for examples of this.
...
in~\cite{Zerepoch-book}.  However, it is important not to make too much
