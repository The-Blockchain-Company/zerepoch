...
module Zerepoch.ChainIndex.TxIdState(
    TxIdState(..)
    , TxConfirmedState(..)
    , isConfirmed
    , increaseDepth
...
import           Zerepoch.ChainIndex.Tx        (ChainIndexTx (..), ChainIndexTxOutputs (..), citxOutputs, citxTxId)
import           Zerepoch.ChainIndex.Types     (BlockNumber (..), Depth (..), Point (..), Tip (..), TxStatus (..),
                                              TxValidity (..), pointsToTip)
import           Zerepoch.ChainIndex.UtxoState (RollbackFailed (..), RollbackResult (..), UtxoIndex, UtxoState (..), tip,
                                              viewTip)
