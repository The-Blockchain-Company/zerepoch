...
{-| Handlers for the 'ChainIndexQueryEffect' and the 'ChainIndexControlEffect'
    in the emulator
-}
module Zerepoch.ChainIndex.Emulator.Handlers(
    handleQuery
    , handleControl
...
import           Zerepoch.ChainIndex.Effects            (ChainIndexControlEffect (..), ChainIndexQueryEffect (..))
import           Zerepoch.ChainIndex.Emulator.DiskState (DiskState, addressMap, dataMap, diagnostics, mintingPolicyMap,
                                                       redeemerMap, stakeValidatorMap, txMap, validatorMap)
import qualified Zerepoch.ChainIndex.Emulator.DiskState as DiskState
import           Zerepoch.ChainIndex.Tx                 (ChainIndexTx, _ValidTx, citxOutputs)
import           Zerepoch.ChainIndex.Types              (Tip (..), pageOf)
import           Zerepoch.ChainIndex.UtxoState          (InsertUtxoPosition, InsertUtxoSuccess (..), RollbackResult (..),
                                                       TxUtxoBalance, UtxoIndex, isUnspentOutput, tip)
import qualified Zerepoch.ChainIndex.UtxoState          as UtxoState
import           Zerepoch.Contract.BccAPI           (FromBccError (..))
import           Zerepoch.V1.Ledger.Api                 (Credential (PubKeyCredential, ScriptCredential))
