...
import           Ledger                    (Address, Datum, DatumHash, MintingPolicy, MintingPolicyHash, OnChainTx (..),
                                            Redeemer (..), RedeemerHash, SlotRange, SomeBccApiTx, StakeValidator,
...
data ChainIndexTx = ChainIndexTx {
    ...
    _citxBccTx       :: Maybe SomeBccApiTx -- Might be Nothing if we are in the emulator
    } deriving (Show, Eq, Generic, ToJSON, FromJSON)
