...
{-| Hedgehog generators for types used in zerepoch-chain-index
-}
module Generators(
    genTxOutRef,
    genRandomTxId,
...
import           Zerepoch.ChainIndex.TxIdState (TxIdState)
import qualified Zerepoch.ChainIndex.TxIdState as TxIdState
import           Zerepoch.ChainIndex.Types     (BlockId (..), BlockNumber (..), Tip (..))
import           Zerepoch.ChainIndex.UtxoState (TxUtxoBalance (..), fromTx)
import qualified ZerepochTx.Prelude            as ZerepochTx

-- | Generate a random tx id
genRandomTxId :: MonadGen m => m TxId
genRandomTxId = TxId . ZerepochTx.toBuiltin <$> Gen.bytes (Range.singleton 32)
...
txIdFromInt = TxId . ZerepochTx.toBuiltin . BSL.toStrict . serialise
...
-- | Generate a positive Bcc value
genNonZeroBccValue :: MonadGen m => m Value
genNonZeroBccValue = Bcc.entropicValueOf <$> Gen.integral (Range.linear 1 100_000_000_000)
...
genTx = do
    newOutputs <-
        let outputGen = (,) <$> genAddress <*> genNonZeroBccValue in
    ...
    ChainIndexTx
        ...
        -- 'SomeBccTx', or vis-versa. And then put it here.
        <*> pure Nothing
