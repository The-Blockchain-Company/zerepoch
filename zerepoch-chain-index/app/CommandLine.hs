...
import           Bcc.Api              (NetworkId (..), NetworkMagic (..))
...
data Command =
  ...
  | StartChainIndex
    -- ^ Start the chain index and connect it to a bcc node
    deriving (Show)
...
startChainIndexParser =
  command "start-index" $
  flip info (fullDesc <> progDesc "Start the chain index and connect it to a bcc node") $ do
