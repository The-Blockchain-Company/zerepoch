...
chainSyncHandler trace mState
  (RollForward block _) _ = do
    let ciBlock = fromBccBlock block
    case ciBlock of
      Left err    ->
        logError trace (ConversionFailed err)
      Right txs ->
        runChainIndex trace mState $ appendBlock (tipFromBccBlock block) txs
chainSyncHandler trace mState
  (RollBackward point _) _ = do
    ...
    runChainIndex trace mState $ rollback (fromBccPoint point)
