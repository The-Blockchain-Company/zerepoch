  ...
  mkDeploymentShell =
    ...
    let
      ...
      setupEnvSecrets = env: ''
        ...
        export TF_VAR_simeon_github_client_id=$(echo $SECRETS | ${jq}/bin/jq --raw-output .simeon.githubClientId)
        export TF_VAR_simeon_github_client_secret=$(echo $SECRETS | ${jq}/bin/jq --raw-output .simeon.githubClientSecret)
        export TF_VAR_simeon_jwt_signature=$(echo $SECRETS | ${jq}/bin/jq --raw-output .simeon.jwtSignature)
        export TF_VAR_zerepoch_github_client_id=$(echo $SECRETS | ${jq}/bin/jq --raw-output .zerepoch.githubClientId)
        export TF_VAR_zerepoch_github_client_secret=$(echo $SECRETS | ${jq}/bin/jq --raw-output .zerepoch.githubClientSecret)
        export TF_VAR_zerepoch_jwt_signature=$(echo $SECRETS | ${jq}/bin/jq --raw-output .zerepoch.jwtSignature)
        ...
        export GITHUB_API_USER=$TF_VAR_zerepoch_github_client_id
        export GITHUB_API_PW=$TF_VAR_zerepoch_github_client_secret
      '';
      ...
      waitGitHubStatus = writeShellScriptBin "wait-github-status" ''
        ...
        GITHUB_API_URL=https://api.github.com/repos/The-Blockchain-Company/zerepoch/commits/"$GIT_COMMIT"/status

        fetchCommitStatus() {
            # Request the status from the GitHub API and build an object:
            # { <context>, <status> }
            # Note: the "buildkite/zerepoch" state gets filtered out otherwise we
            # will be stuck in an infinite loop waiting for our own success.
            curl --silent\
             -u "$GITHUB_API_USER:$GITHUB_API_PW" \
             -H "Accept: application/vnd.github.v3+json" \
             "$GITHUB_API_URL" \
                | ${jq}/bin/jq -c '.statuses | map(select(.context != "buildkite/zerepoch")) | map ({(.context): (.state)}) | add'
        }
        ...
        while true; do
            ...
            ALL_CHECKS_PRESENT=$(echo "$GH_STATUS_MAP" | ${jq}/bin/jq 'has("ci/hydra-eval") and has("ci/hydra:Bcc:zerepoch:required") and has("ci/hydra-build:required")')
      ...
      deployNix = writeShellScriptBin "deploy-nix" ''
        ...
        rm -rf ./zerepoch_playground.$DEPLOYMENT_ENV.conf
        rm -rf ./machines.json
        ...
        echo "[deploy-nix]: Writing zerepoch secrets ..."
        zerepoch_tld=$(cat ./machines.json | ${jq}/bin/jq -r '.zerepochTld')
        cat > ./morph/secrets.zerepoch.$DEPLOYMENT_ENV.env <<EOL
        export JWT_SIGNATURE="$(echo $TF_VAR_zerepoch_jwt_signature)"
        export GITHUB_CLIENT_ID="$(echo $TF_VAR_zerepoch_github_client_id)"
        export GITHUB_CLIENT_SECRET="$(echo $TF_VAR_zerepoch_github_client_secret)"
        EOL

        echo "[deploy-nix]: Writing simeon secrets ..."
        simeon_tld=$(cat ./machines.json | ${jq}/bin/jq -r '.simeonTld')
        cat > ./morph/secrets.simeon.$DEPLOYMENT_ENV.env <<EOL
        export JWT_SIGNATURE="$(echo $TF_VAR_simeon_jwt_signature)"
        export GITHUB_CLIENT_ID="$(echo $TF_VAR_simeon_github_client_id)"
        export GITHUB_CLIENT_SECRET="$(echo $TF_VAR_simeon_github_client_secret)"
        EOL
        ...
        export SSH_CONFIG_FILE=./zerepoch_playground.$DEPLOYMENT_ENV.conf
