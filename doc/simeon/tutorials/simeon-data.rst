...
This tutorial formally introduces Simeon as a Haskell data type, as well as
...
be run. It also introduces *Extended Simeon* which we use for describing *contract templates* in Simeon.

The code that we describe here comes from the Haskell modules
`SemanticsTypes.hs <https://github.com/The-Blockchain-Company/simeon/blob/master/src/Language/Simeon/SemanticsTypes.hs>`_
,
`Semantics.hs <https://github.com/The-Blockchain-Company/simeon/blob/master/src/Language/Simeon/Semantics.hs>`_
and
`Util.hs <https://github.com/The-Blockchain-Company/simeon/blob/master/src/Language/Simeon/Util.hs>`_.

Simeon
-------

The Simeon domain-specific language (DSL) is modelled as used above, a collection
...
tutorial <simeon-step-by-step>` what these
...
In modelling basic parts of Simeon we use a combination of Haskell
...
In order to progress a Simeon contract, a party must provide an
...
A Simeon *account* holds amounts of multiple currencies and/or fungible
...
.. code:: haskell
   ...
   bcc = Token bccSymbol bccToken
   
But you are free to create your own currencies and tokens using the native token facility of Bcc.   
You can think
...
This completes our discussion of the types that make up Simeon
contracts.

Extended Simeon
----------------

Extended Simeon adds templating functionality to Simeon language, so that constants need not be “hard wired”
into Simeon contracts, but can be replaced by *parameters*. 
Objects in Extended Simeon are called *templates* or *contract templates*.


Specifically, Extended Simeon extends the ``Value``
...
Extended Simeon is not directly executable, it has to be translated to core Simeon before
execution, deployment, or analysis, through the process of *instantiation*.
The purpose of Extended Simeon is to allow Simeon contracts to be reusable
in different situations without cluttering the code that goes on-chain
(core Simeon). In Simeon Run and the Simeon Playground templates need to be instantiated before
...
As we noted earlier, the semantics of Simeon consist in building
...
This is part of the architecture of Bcc/Zerepoch, which acknowledges
...
over to Simeon: each step of a Simeon contract is processed in the
...
How does this affect the processing of a Simeon contract? Each step is
...
-  It is assumed that cryptographic functions and operations are
   provided by a layer external to Simeon, and so they need not be
   modelled explicitly.
