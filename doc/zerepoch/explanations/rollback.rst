...
The Bcc network is a distributed system with many nodes operating at the same time.
...
To a Zerepoch app whose on-chain state is a set of :term:`unspent transaction outputs <UTXO>`, the effect of a rollback is to undo changes to the state of transaction outputs that were performed by transactions which are now being dropped as part of the rollback.
