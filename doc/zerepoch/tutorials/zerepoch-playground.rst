...
Compiling and testing a Zerepoch app in the Zerepoch Playground
===========================================================

The Zerepoch Playground consists of a code editor and a simulator.

.. figure:: images/playground.png

  Zerepoch Playground (code editor)

You can write Zerepoch apps in the code editor and test them in the simulator.

.. note::

  Zerepoch apps are distributed applications with on-chain and off-chain state.
  Their main task is to build the transactions that produce and spend Zerepoch script outputs.

The Zerepoch Playground includes a number of sample apps.
...
When you first open the Zerepoch Playground, the code editor is populated with the "Vesting" app.
...
.. figure:: images/playground-simulator.png
  ...
  Zerepoch Playground (simulator)

You can use the simulator to define complex scenarios with multiple agents trading and communicating over Bcc.
...
You can see that wallet 1 and wallet 2 each have one hundred entropic.
...
Change the amount to five entropic.
...
.. figure:: images/playground-payment.png
  ...
  Action sequence for a payment of five entropic from wallet 3 to wallet 2.
...
One output with five entropic belonging to wallet 2, and another output with the remaining five entropic belonging to wallet 3.
This second output is called a *change output*.
It is needed because the input that was spent contained ten entropic, and you only wanted to pay five of them to wallet 2.
Therefore the remaining five entropic were given back to wallet 3.
...
The box for the "retrieve funds" endpoint has an input field for an Bcc amount.
In each of the two invocations you retrieve four entropic.

Each endpoint used by an app has its own type (number, text, currency, etc.) that is defined in the source code.
The Zerepoch Playground uses this type to provide the correct set of form elements for creating a value of the type.
...
.. figure:: images/playground-vesting-blockchain.png
  ...
  The highlighted transaction locks eight entropic with the vesting script.

Select the transaction in slot 1.
One of its outputs is a script output.
This is the vesting script that locks eight entropic.

Now select the next transaction (Slot 21, Tx 0).
It spends the script output, paying four entropic to wallet 1 and the remaining four entropic to the vesting script.
...
The final transaction (Slot 61, Tx 0) takes the remaining four entropic from the script and places them in a public-key output belonging to wallet 1.

At the end of the simulation, wallet 1 has eight entropic more than it started out with.
...
The simlation finished with the total of eight entropic still locked by the script.
...
This is another example the different notions of state that you are dealing with in Zerepoch apps.
