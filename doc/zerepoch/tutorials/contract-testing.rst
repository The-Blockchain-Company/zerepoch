...
Property-based testing of Zerepoch contracts
==========================================

Zerepoch comes with a library for testing contracts using
...
.. _here: https://github.com/The-Blockchain-Company/zerepoch/blob/master/zerepoch-use-cases/src/Zerepoch/Contracts/GameStateMachine.hs

 - The first player locks a sum of Bcc in the contract, which is
 ...
 - Any player can now try to guess the password, by submitting a
   ...
   contains enough Bcc, then the guesser receives the withdrawal and
...
the game :term:`token` and Bcc move as they should.
...
of one or more types of :term:`token`. The most common :term:`token` is, of course, the Bcc;
we can import functions manipulating Bcc, and the ``Value`` type
itself, as follows:

.. literalinclude:: GameModel.hs
   :start-after: START import Bcc
   :end-before: END import Bcc

With these imports, we can construct values in the Bcc :term:`currency`:

.. code-block:: text

  > Bcc.entropicValueOf 1
  Value (Map [(,Map [(,1)])])
...
We can even construct a ``Value`` containing an Bcc and a game :term:`token`:

.. code-block:: text

  > Bcc.entropicValueOf 1 <> gameTokenVal
...
the Bcc :term:`token` name is displayed as an empty string, as is the hash of
...
The ``GameModel`` type must be an instance of the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel`
...
In this case we define three actions:
 ...
 - a ``Lock`` action to be performed by the first player when starting
   the game, containing the player's wallet (from which the Bcc will
...
A generated test is called :hsobj:`Zerepoch.Contract.Test.ContractModel.Actions`, and is, as the name suggests, essentially a
sequence of :hsobj:`Zerepoch.Contract.Test.ContractModel.Action` values. We can
run tests by using :hsobj:`Zerepoch.Contract.Test.ContractModel.propRunActions_`:
...
transferred correctly, and contracts didn't crash.
  ...
  .. note::
     ...
     There is also a more general function :hsobj:`Zerepoch.Contract.Test.ContractModel.propRunActions` that allows
     the check at the end of each test to be customized.

But what is the ``instanceSpec`` in the code above? :hsobj:`Zerepoch.Contract.Test.ContractModel.propRunActions_`
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey`, another
associated datatype of the
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class; we talk to a
contract instance by referring to its
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey`. The
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey` type is
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey` is defined as a
GADT.

In this particular case, there is only one type of contract under
test, and so it suffices to define a :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey` type with a single
...
Once this type is defined, we can construct our :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceSpec`:
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey` of the form ``WalletKey w``.
...
specify how to generate an :hsobj:`Zerepoch.Contract.Test.ContractModel.Action`, which we will do next.

Generating actions
^^^^^^^^^^^^^^^^^^

To generate actions, we need to be able to generate wallets, guesses,
and suitable values of Bcc, since these appear as action parameters.
...
correct. We choose Bcc amounts to be non-negative integers, because
negative amounts would be error cases that we choose not to test.

Now we can define a generator for :hsobj:`Zerepoch.Contract.Test.ContractModel.Action`, as a method of the
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class:
...
our generation; if any :hsobj:`Zerepoch.Contract.Test.ContractModel.Action` disappears altogether, or is generated
...
guess, the guesser receives the requested Bcc only if the guess was
correct. An important part of a :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` defines how funds
...
just where all the Bcc are. We need to know:
...
- how much Bcc is currently locked in the contract, so we can
...
type parameter of the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class. (Note that this contract
...
situation before its creation. The locked funds are always in Bcc, so
in the model it suffices to store an integer.

Now we can define the initial state of the model at the start of each
test case, :hsobj:`Zerepoch.Contract.Test.ContractModel.initialState`, and a :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState` function to model the way
we expect each operation to change the state. These are both methods
in the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class.
...
The :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState` function is defined in the :hsobj:`Zerepoch.Contract.Test.ContractModel.Spec` monad
...
contract state (using :hsobj:`Zerepoch.Contract.Test.ContractModel.$=` and generated ``Lens`` operations),
mints the game :term:`token` (using :hsobj:`Zerepoch.Contract.Test.ContractModel.mint`), deposits it in the creator's
wallet, and withdraws the Bcc locked in the contract (using :hsobj:`Zerepoch.Contract.Test.ContractModel.deposit`
and :hsobj:`Zerepoch.Contract.Test.ContractModel.withdraw`):

.. literalinclude:: GameModel.hs
   :start-after: START nextState Lock v1
   :end-before: END nextState Lock v1

A :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` actually tracks not only the contract model state (in
...
test. It is these expectations that are manipulated by :hsobj:`Zerepoch.Contract.Test.ContractModel.mint`,
:hsobj:`Zerepoch.Contract.Test.ContractModel.deposit`, etc... don't confuse them with operations that *actually*
mint or move :term:`tokens<token>` in the implementation. The :hsobj:`Zerepoch.Contract.Test.ContractModel.ModelState` type
contains all of this information.


When making a guess, we need to check parts of the contract state
(which we read using :hsobj:`Zerepoch.Contract.Test.ContractModel.viewContractState`), and then we update the
stored password, game value, and wallet contents appropriately. (Here
:hsobj:`Zerepoch.Contract.Test.ContractModel.$~` applies a function to modify a field of the contract state).

.. literalinclude:: GameModel.hs
   :start-after: START nextState Guess v1
   :end-before: END nextState Guess v1

``GiveToken`` just transfers the game :term:`token` from one wallet to another using :hsobj:`Zerepoch.Contract.Test.ContractModel.transfer`.

.. literalinclude:: GameModel.hs
   :start-after: START nextState GiveToken v1
   :end-before: END nextState GiveToken v1

At the end of each test, the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` framework checks that
every wallet contains the :term:`tokens<token>` that the model says it should.

We can exercise the :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState` function already by generating and
...
not yet been minted--so the ``fromJust`` in the :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState` function
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.precondition` method to our
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` instance.

.. literalinclude:: GameModel.hs
   :start-after: START precondition type
   :end-before: END precondition type

The :hsobj:`Zerepoch.Contract.Test.ContractModel.precondition` is parameterised on the entire model state, which
...
Bcc. Inspecting the error message, we can see that wallet 1 ended up
...
Then we define the :hsobj:`Zerepoch.Contract.Test.ContractModel.perform` method of the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class:

.. literalinclude:: GameModel.hs
   :start-after: START perform type
   :end-before: END perform type

The job of the :hsobj:`Zerepoch.Contract.Test.ContractModel.perform` method in this case is just to invoke the
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.perform` lets us find the contract handle associated with each
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractInstanceKey`.

For the most part, it is good practice to keep the :hsobj:`Zerepoch.Contract.Test.ContractModel.perform` function
...
to :term:`contract endpoints<contract endpoint>` makes interpreting test failures much easier.
 ...
 .. note::
    ...
    **Helping shrinking work better by choosing test case actions well**
    ...
    In the definition of :hsobj:`Zerepoch.Contract.Test.ContractModel.perform` above, the ``GiveToken`` action is a little
    ...
    *from*. So why did we choose to define a ``GiveToken w`` action to
    ...
    would correspond more directly to the code in :hsobj:`Zerepoch.Contract.Test.ContractModel.perform`?
...
specify this shrinking by defining the :hsobj:`Zerepoch.Contract.Test.ContractModel.shrinkAction` operation in the
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class:
...
as replacements for the given :hsobj:`Zerepoch.Contract.Test.ContractModel.Action`, when QuickCheck is simplifying
...
and shrink actions by shrinking the wallet and Bcc parameters.
...
In this test, wallet 1 attempts to lock zero Bcc, and our model predicts
...
1, resulting in a transaction with ID ``2542...``, which pays zero Bcc
...
We add a call to ``delay`` in each branch of :hsobj:`Zerepoch.Contract.Test.ContractModel.perform`:
...
emulator. We do this using corresponding calls to :hsobj:`Zerepoch.Contract.Test.ContractModel.wait` in the
definition of :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState`:
...
outputs had been created. These two outputs are the Bcc amounts
...
now in a broken state. In fact, the Bcc now locked in these :term:`UTXOs<UTXO>`
...
tries to start the game for a second time, the Bcc will be lost (until
the bug is fixed).
...
we see that although the wallet contained 100 million Bcc, it *lacked*
...
In this case, we lock one Bcc, and then wallet 2 makes two guesses,
both with the correct password. The first guess tries to withdraw more
Bcc than are available, which our model predicts should be a
...
and a withdrawal of only one Bcc, ought to succeed. That is why we
expected wallet 2 to end up with the game :term:`token`, and one Bcc. However,
wallet 2 did not receive the Bcc, only the game :term:`token`. Reading the
emulator log reveals why: in slot 4 we called the ``guess`` :term:`endpoint`
to withdraw two Bcc, which would leave -1 Bcc locked by the contract,
...
generated. By default, testing a :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` just displays the
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.monitoring` method in the :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class:

.. literalinclude:: GameModel.hs
   :start-after: START monitoring type
   :end-before: END monitoring type

This function is called for every :hsobj:`Zerepoch.Contract.Test.ContractModel.Action` in a test case,
and given the :hsobj:`Zerepoch.Contract.Test.ContractModel.ModelState` before and after the
:hsobj:`Zerepoch.Contract.Test.ContractModel.Action`. Its result is a function that is applied to the
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.monitoring` as
...
possibilities (and the :hsobj:`Zerepoch.Contract.Test.ContractModel.precondition` for guesses does not depend on
...
previously, the Bcc locked by the contract can always be recovered by
a player who knows the password.

Here we are really identifying desirable 'goal states', namely those
in which all the Bcc have been recovered from the contract, and aiming
...
generators are little programs in the :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` monad, such as
this one:
...
This :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` fragment simply specifies a unit test in terms
of the underlying :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` we have already seen,
using :hsobj:`Zerepoch.Contract.Test.ContractModel.action` to include a specific
:hsobj:`Zerepoch.Contract.Test.ContractModel.Action` in the test. To run such a test, we must specify
...
which uses :hsobj:`Zerepoch.Contract.Test.ContractModel.forAllDL` to generate a test sequence from the
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel`.
...
As well as writing unit tests in the :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` monad, we can add random
generation. For example, if we wanted to generalize the unit test
above a little to lock a random amount of Bcc in the contract, then
...
Here :hsobj:`Zerepoch.Contract.Test.ContractModel.forAllQ` lets us generate a random value using
:hsobj:`Zerepoch.Contract.Test.ContractModel.chooseQ`:

.. literalinclude:: GameModel.hs
   :start-after: START chooseQ type
   :end-before: END chooseQ type

:hsobj:`Zerepoch.Contract.Test.ContractModel.forAllQ` takes a
:hsobj:`Zerepoch.Contract.Test.ContractModel.Quantification`, which resembles a QuickCheck
...
information. Every such test contains a list of :hsobj:`Zerepoch.Contract.Test.ContractModel.Action`, tagged
:hsobj:`Zerepoch.Contract.Test.ContractModel.Do`, and *witnesses*, tagged :hsobj:`Zerepoch.Contract.Test.ContractModel.Witness`. The witnesses record the
results of random choices made by :hsobj:`Zerepoch.Contract.Test.ContractModel.forAllQ`: in this case, the Bcc
value to be locked was chosen to be 1. The test proceeds by
locking the Bcc and giving the game :term:`token` to wallet 2, but the third
action we specified--making the guess--cannot be run, because its
precondition is ``False``. This is what the
:hsobj:`Zerepoch.Contract.Test.ContractModel.BadPrecondition` tells
...
the ``gameValue`` is only 1 Bcc, so of course we cannot withdraw 3.

 .. note::

    We saw earlier that when tests are *generated* from a
    :hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel`, then QuickCheck only generates actions whose
    :hsobj:`Zerepoch.Contract.Test.ContractModel.precondition` is satisfied. On the other hand, when we use dynamic
...
We can rerun the test using :hsobj:`Zerepoch.Contract.Test.ContractModel.withDLTest`, supplying the
:hsobj:`Zerepoch.Contract.Test.ContractModel.DL` ``unitTest`` from which the test case was generated,
...
modified :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` test, so it is not a counterexample to the property we
are testing.
...
Suppose we want to test that no Bcc remain locked in the game contract
for ever. We could try to specify this with a :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` test that requires that *no
Bcc remain locked in the contract after any sequence of actions*. We
can include a random sequence of actions in a :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` test using
:hsobj:`Zerepoch.Contract.Test.ContractModel.anyActions_`, and we can make assertions about the
:hsobj:`Zerepoch.Contract.Test.ContractModel.ModelState` using
:hsobj:`Zerepoch.Contract.Test.ContractModel.assertModel`. Thus we can define
...
locked (:hsobj:`Zerepoch.Contract.Test.ContractModel.lockedValue` extracts the total value locked in contracts from
the :hsobj:`Zerepoch.Contract.Test.ContractModel.ModelState`).
...
If all we do is lock one Bcc, then obviously the locked funds are not
zero. The failed assertion is reported as a
:hsobj:`Zerepoch.Contract.Test.ContractModel.BadPrecondition` (for
the assertion).
...
construct a correct guess that recovers all the locked Bcc to this
...
In this case we locked 1 Bcc in the contract, chose wallet 2 to
...
our model indeed says that there are still 5 Bcc locked in the
contract. This is the effect of the :hsobj:`Zerepoch.Contract.Test.ContractModel.nextState` function in our model,
...
correct password as the new one--so the guess is wrong, and the Bcc
...
strategy. To find out, we can :hsobj:`Zerepoch.Contract.Test.ContractModel.monitor` the contract model during our
tests. As in the :hsobj:`Zerepoch.Contract.Test.ContractModel.monitoring` method of the
:hsobj:`Zerepoch.Contract.Test.ContractModel.ContractModel` class, we
can use any of the QuickCheck operations for analyzing test cases, but
instead of applying the :hsobj:`Zerepoch.Contract.Test.ContractModel.monitoring` at every action in a test case, we
can :hsobj:`Zerepoch.Contract.Test.ContractModel.monitor` at selected points.
...
With the addition of the :hsobj:`Zerepoch.Contract.Test.ContractModel.monitor` line, QuickCheck tells us what
...
We can see that around 30% of generated tests leave some Bcc in the
...
hitherto. The :hsobj:`Zerepoch.Contract.Test.ContractModel.DL` monad is an instance of ``Alternative``, so we can
write tests with random control flow, weight choices suitably, and so
on. For example, :hsobj:`Zerepoch.Contract.Test.ContractModel.anyActions`, which generates a random sequence of
...
alternative is guarded by :hsobj:`Zerepoch.Contract.Test.ContractModel.stopping`, which means it will be chosen
*only* if the test case is 'getting too long'; in this case
:hsobj:`Zerepoch.Contract.Test.ContractModel.anyActions` will generate an empty sequence. We can exercise fine
...
cannot generate? Is it possible, for example, to steal the Bcc locked
...
:hsobj:`Zerepoch.Contract.Test.ContractModel.Action` to complete before performing the next
:hsobj:`Zerepoch.Contract.Test.ContractModel.Action` (because we inserted calls to ``delay`` into
:hsobj:`Zerepoch.Contract.Test.ContractModel.perform`, and
:hsobj:`Zerepoch.Contract.Test.ContractModel.wait` into
:hsobj:`Zerepoch.Contract.Test.ContractModel.nextState`). In reality, different wallets may perform
...
explicit :hsobj:`Zerepoch.Contract.Test.ContractModel.Action` in test cases instead. However,
...
withdraw Bcc from the game contract. So to steal the money, an
...
contracts in the ``zerepoch-use-cases`` project.

.. _GameModel: https://github.com/The-Blockchain-Company/zerepoch/blob/master/zerepoch-use-cases/test/Spec/GameStateMachine.hs
.. _Prism: https://github.com/The-Blockchain-Company/zerepoch/blob/master/zerepoch-use-cases/test/Spec/Prism.hs
.. _Auction: https://github.com/The-Blockchain-Company/zerepoch/blob/master/zerepoch-use-cases/test/Spec/Auction.hs
