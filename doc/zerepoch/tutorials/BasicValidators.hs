...
import           Bcc.Api          (HasTextEnvelope, TextEnvelope, TextEnvelopeDescr, serialiseToTextEnvelope)
...
-- We can use 'compile' to turn a validator function into a compiled Zerepoch Core program.
...
validatePayment _ _ ctx = check $ case fromBuiltinData ctx of
    Just valCtx ->
        ...
        -- than 1 Bcc (more stuff is fine!)
        in fold values `geq` bccValueOf 1
    _ -> False
-- BLOCK5
data DateValidator
