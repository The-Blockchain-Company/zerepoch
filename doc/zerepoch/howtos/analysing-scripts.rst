...
How to analyse the cost and size of Zerepoch scripts
==================================================

Running Zerepoch scripts on a validating node uses CPU time and RAM space, which are paid for by transaction fees.
When building a decentralised application in Zerepoch we need to keep an eye on the size of the transactions that we submit to the network.

The Zerepoch libraries give us some tools for measuring the resource consumption of our scripts.

Resource use of Zerepoch scripts
------------------------------

There are two types of resources used by Zerepoch transactions.
...
The `Zerepoch.Trace.Emulator.Extract` module lets us analyse both types of cost for transactions that are produced by the Zerepoch :term:`emulator`.

:hsobj:`Zerepoch.Trace.Emulator.Extract.writeScriptsTo` is a function that, given an emulator trace, produces a JSON file for each transaction that is created during that trace.

.. literalinclude:: ./WriteScriptsTo.hs
   :start-after: BLOCK0
   :end-before: BLOCK1

The :hsobj:`Zerepoch.Trace.Emulator.Extract.Command` argument selects one of two modes of :hsobj:`Zerepoch.Trace.Emulator.Extract.writeScriptsTo`.
The mode determines what kind of data is written to the folder specified in :hsobj:`Zerepoch.Trace.Emulator.Extract.scPath`.

1. :hsobj:`Zerepoch.Trace.Emulator.Extract.Scripts` writes the validator scripts, one for each script input that is validated as part of the emulator trace. Here we have the choice between fully applied validators and unapplied validators. *Fully applied* means that we get a :term:`Zerepoch Core` (PLC) program that can be evaluated to the unit value (or an error). This is the program that the node actually runs when validating the script input, and it is used for determining the script execution cost. *Unapplied* results in the PLC program of the unapplied validator. This tells us the size of the serialised Zerepoch script that we need to attach to the spending transaction. In both cases the CPU and memory budget in ExUnits will be displayed in the terminal (see sample output below).
2. :hsobj:`Zerepoch.Trace.Emulator.Extract.Transactions` writes all partial transactions that are sent to the (emulated) wallet for balancing before they are submitted to the network. Each partial transaction results in a JSON file. The `transaction` field of the JSON object contains the actual transaction in the text envelope format used by `bcc-api`. Since the transaction body is hex encoded, we can look at the length of the `cborHex` field and divide it by two in order to get the size of the partial transaction in bytes. Note that the final transaction will be slightly larger, because some additional inputs and outputs will be added by the wallet.

Examples
--------

To see :hsobj:`Zerepoch.Trace.Emulator.Extract.writeScriptsTo` in action you can run the `zerepoch-use-cases-scripts` command that is part of the `zerepoch-use-cases` package in our repository.
...
.. code-block:: shell
    ...
    cabal run zerepoch-use-cases-scripts -- ./tmp scripts
...
.. code-block:: shell
    ...
    cabal run zerepoch-use-cases-scripts -- ./tmp scripts --unapplied-validators
...
.. code-block:: shell
    ...
    cabal run zerepoch-use-cases-scripts -- ./tmp transactions -p ./zerepoch-use-cases/scripts/protocol-parameters.json
