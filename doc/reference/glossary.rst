...
.. glossary::
    ...
    Bcc
      The blockchain system upon which the Zerepoch Platform is built.
    ...
    currency
      ...
      policy script. On the Bcc ledger there is a special currency
      called Bcc which can never be minted and which is controlled
      separately.
    ...
    emulator
      ...
      analysing Zerepoch apps.

    endpoint
      A potential request made by a contract application for user input. Every endpoint has a name and a type.

    Extended UTXO Model
      The ledger model which the Zerepoch Platform relies on.

      This is implemented in the Aurum hard fork of the Bcc blockchain.
    ...
    Hydra
      A Layer 2 scalability solution for Bcc. See :cite:t:`chakravarty2020hydra`.
    ...
    PAB client
      ...
      1. Wallet frontends such as Klarity.

      2. Other user software which uses the contract application as part of a wider system.

    Zerepoch Application
      An application written using the Zerepoch Application Framework.

    pab
    Zerepoch Application Backend (PAB)
      The component which manages Zerepoch Applications that run on users' machines.
      It handles:
    ...
    Zerepoch Core
      The programming language in which scripts on the Bcc blockchain
      are written. Zerepoch Core is a small functional programming
      language — a formal specification is available with further details.
      Zerepoch Core is not read or written by humans, it is a compilation
      target for other languages.

      See :ref:`what_is_zerepoch_foundation`.

    Zerepoch IR
      An intermediate language that compiles to Zerepoch Core. Zerepoch IR is
      not used by users, but rather as a compilation target on the way to
      Zerepoch Core. However, it is significantly more human-readable than
      Zerepoch Core, so should be preferred in cases where humans may want to
      inspect the program.
    ...
    Zerepoch Platform
      ...
      1. Zerepoch Foundation, and

      2. The Zerepoch Application Framework

      See :ref:`what_is_the_zerepoch_platform`.
    ...
    Zerepoch Tx
      The libraries and compiler for compiling Haskell into Zerepoch Core to
    ...
    script
      ...
      Bcc blockchain, these are written in Zerepoch Core.
