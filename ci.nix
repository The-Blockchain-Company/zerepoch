    ...
    let
      ...
      _select = _: system: crossSystem:
        ...
        filterAttrsOnlyRecursive (_: drv: isBuildable drv) ({
          ...
          # cache for users
          inherit (zerepoch.haskell.project) roots;
        } // pkgs.lib.optionalAttrs (!rootsOnly) (filterCross {
          # build relevant top level attributes from default.nix
          inherit (packages) docs tests zerepoch-playground simeon-playground simeon-dashboard simeon-dashboard-fake-pab zerepoch-pab zerepoch-use-cases deployment;
          ...
          # on 'zerepoch-core', so this does the job.
          ...
          shell = (import ./shell.nix { inherit packages; }).overrideAttrs (attrs: attrs // {
            disallowedRequisites = [ zerepoch.haskell.packages.zerepoch-core.components.library ];
          });

          # build all haskell packages and tests
          haskell = pkgs.recurseIntoAttrs (mkHaskellDimension pkgs zerepoch.haskell.projectPackages);
        }));
    in
