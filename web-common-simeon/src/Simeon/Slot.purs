...
-- that was introduced when Sophie was launched in mid 2020. Since the launch, each slot number
...
import Simeon.Semantics (Slot(..))
import Partial.Unsafe (unsafePartial)

-- TODO: When we are integrated with the real Bcc node, we will need to
...
-- to know the exact datetime of the Sophie launch, and the slot number at
-- that moment. In the meantime, these are our best guesses based on some
-- quick Googling.  :)
sophieInitialSlot :: Slot
sophieInitialSlot = Slot $ fromInt 4492800

-- Note [Datetime to slot]: The `zerepoch-pab.yaml` config file can specify
-- the datetime of slot zero. To synchronise with the frontend, this should
-- be set to `sophieLaunchDate - (sophieInitialSlot * 1000)` (because there
-- is 1 slot per second). On the current estimates this comes to 1591566291000,
-- which is 2020-06-07 21:44:51 UTC.
sophieLaunchDate :: DateTime
sophieLaunchDate =
  let
...
slotToDateTime slot =
  let
    secondsDiff' = secondsDiff slot sophieInitialSlot
  in
    adjust secondsDiff' sophieLaunchDate

dateTimeToSlot :: DateTime -> Slot
dateTimeToSlot datetime =
  let
    secondsDiff' :: Seconds
    secondsDiff' = diff datetime sophieLaunchDate
  in
    sophieInitialSlot + (Slot $ BigInteger.fromInt $ round $ unwrap secondsDiff')
