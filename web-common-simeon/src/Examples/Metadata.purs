...
import Simeon.Extended.Metadata (NumberFormat(..), MetaData, emptyContractMetadata, entropicFormat, oracleRatioFormat)
...
escrow =
  ...
  , valueParameterInfo:
      ( OMap.fromFoldable
          [ "Price"
              /\ { valueParameterFormat: entropicFormat
...
escrowWithCollateral =
  ...
  , valueParameterInfo:
      ( OMap.fromFoldable
          [ "Collateral amount"
              /\ { valueParameterFormat: entropicFormat
                , valueParameterDescription: "The amount of Entropic to be deposited by both parties at the start of the contract to serve as an incentive for collaboration."
                }
          , "Price"
              /\ { valueParameterFormat: entropicFormat
                , valueParameterDescription: "The amount of Entropic to be paid by the _**Buyer**_ as part of the exchange."
                }
          ]
      )
  }
...
couponBondGuaranteed =
  ...
  , valueParameterInfo:
      ( OMap.fromFoldable
          [ "Interest instalment"
              /\ { valueParameterFormat: entropicFormat
                , valueParameterDescription: "Amount of Entropic that will be paid by the _**Borrower**_ every 30 slots for 3 iterations."
                }
          , "Principal"
              /\ { valueParameterFormat: entropicFormat
                , valueParameterDescription: "Amount of Entropic that will be borrowed by the _**Borrower**_."
                }
          ]
      )
  }
...
swap =
  { contractType: Swap
  , contractName: "Swap of Bcc and dollar tokens"
  , contractShortDescription: "Atomically exchange of Bcc and dollar tokens."
  , contractLongDescription: "Waits until one party deposits Bcc and the other party deposits dollar tokens. If both parties collaborate it carries the exchange atomically, otherwise parties are refunded."
  , choiceInfo: Map.empty
  , roleDescriptions:
      ( Map.fromFoldable
          [ "Bcc provider" /\ "The party that provides the Bcc."
          , "Dollar provider" /\ "The party that provides the dollar tokens."
          ]
      )
  , slotParameterDescriptions:
      ( OMap.fromFoldable
          [ "Timeout for Bcc deposit" /\ "Deadline by which Bcc must be deposited."
          , "Timeout for dollar deposit" /\ "Deadline by which dollar tokens must be deposited (must be after the deadline for Bcc deposit)."
          ]
      )
  , valueParameterInfo:
      ( OMap.fromFoldable
          [ "Amount of Bcc"
              /\ { valueParameterFormat: DecimalFormat 0 "₳"
                , valueParameterDescription: "Amount of Bcc to be exchanged for dollars."
                }
          , "Amount of dollars"
              /\ { valueParameterFormat: DecimalFormat 0 "$"
                , valueParameterDescription: "Amount of dollar tokens to be exchanged for Bcc."
                }
          ]
      )
  }
...
contractForDifferences =
  ...
  , contractName: "CFD"
  , contractShortDescription: "Contract For Differences. Two parties deposit Bcc in a contract and after some time the Bcc is redistributed among them depending on the change in price of an asset as reported by a third party (_**oracle**_)."
  , contractLongDescription: "At the beginning of the contract, _**party**_ and _**counterparty**_ deposit some Bcc in the contract. At the end of the contract, all Bcc deposited is redistributed depending on the change in price in Bcc of an asset (as reported by the _**oracle**_). If the price in Bcc of the asset increases, the difference goes to _**counterparty**_; if it decreases, the difference goes to _**party**_, up to a maximum of the amount deposited at the beginning."
  , choiceInfo:
      ( Map.fromFoldable
          [ "Price in first window"
              /\ { choiceFormat: DecimalFormat 6 "₳"
                , choiceDescription: "Price in BCC of the asset in the first window."
                }
          , "Price in second window"
              /\ { choiceFormat: DecimalFormat 6 "₳"
                , choiceDescription: "Price in BCC of the asset in the second window."
                }
          ]
      )
  , roleDescriptions:
      ( Map.fromFoldable
          ...
          , "Oracle" /\ "The _**oracle**_ provides the price of the asset at the beginning (first window) and at the end (second window) of the contract (in this case the _**oracle**_ provides the conversion rate between Bcc and dollars)."
          ]
      )
  , slotParameterDescriptions:
      ( OMap.fromFoldable
...
contractForDifferencesWithOracle =
  ...
  , contractShortDescription: "Contract For Differences with Oracle. Two parties deposit Bcc in a contract and after some time the Bcc is redistributed among them depending on the change in price of an asset."
  , contractLongDescription: "At the beginning of the contract, _**party**_ and _**counterparty**_ deposit some Bcc in the contract. At the end of the contract, all Bcc deposited is redistributed depending on the change in price in dollars of an asset (as reported by the _**oracle**_). The asset in this contract is an amount of Bcc. If the price in dollars of the asset increases, the difference goes to _**counterparty**_; if it decreases, the difference goes to _**party**_, up to a maximum of the amount deposited at the beginning."
  , choiceInfo:
      ( Map.fromFoldable
          [ "dir-bccusd"
              /\ { choiceFormat: oracleRatioFormat "BCC/USD"
                , choiceDescription: "Exchange rate BCC/USD in the first window."
                }
          , "inv-bccusd"
              /\ { choiceFormat: oracleRatioFormat "USD/BCC"
                , choiceDescription: "Exchange rate USD/BCC in the second window."
                }
          ]
      )
  , roleDescriptions:
      ( Map.fromFoldable
          [ "Counterparty" /\ "The _**counterparty**_ will get the difference in the price of the asset if it increases."
          , "Party" /\ "The _**party**_ will get the difference in the price of the asset if it decreases."
          , "kraken" /\ "The _**oracle**_ provides the price of the asset at the beginning (first window) and at the end (second window) of the contract (in this case the _**oracle**_ provides the conversion rate between Bcc and dollars)."
          ]
      )
  , slotParameterDescriptions:
      ( OMap.fromFoldable
  ...
  , valueParameterInfo:
      ( OMap.fromFoldable
          ...
          , "Amount of Bcc to use as asset"
              /\ { valueParameterFormat: DecimalFormat 6 "₳"
                , valueParameterDescription: "Amount of Bcc whose price in dollars change to monitor."
                }
          ]
      )
  }
