...
import Simeon.Extended (Action(..), Case(..), Contract(..), Observation(..), Payee(..), Timeout(..), Value(..))
import Simeon.Extended.Metadata (MetaData, ContractTemplate)
import Simeon.Semantics (Bound(..), ChoiceId(..), Party(..), Rational(..), Token(..), ValueId(..))
...
priceBeginning = ConstantParam "Amount of Bcc to use as asset"

priceEnd :: ValueId
priceEnd = ValueId "Price in second window"

exchangeBeginning :: ChoiceId
exchangeBeginning = ChoiceId "dir-bccusd" oracle

exchangeEnd :: ChoiceId
exchangeEnd = ChoiceId "inv-bccusd" oracle
...
initialDeposit by deposit timeout timeoutContinuation continuation =
  When [ Case (Deposit by by bcc deposit) continuation ]
    timeout
    timeoutContinuation
...
transferUpToDeposit from payerDeposit to amount = Pay from (Account to) bcc (Cond (ValueLT amount payerDeposit) amount payerDeposit)
