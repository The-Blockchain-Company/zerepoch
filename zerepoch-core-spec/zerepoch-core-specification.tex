% Zerepoch Core Specification
\title{Formal Specification of\\the Zerepoch Core Language (version 2.1)}
\date{6th April 2021}
...
% *** DEFINITIONS FOR ZEREPOCH LANGUAGE ***
...
%\begin{abstract}
%\boldmath
%The Zerepoch Language is outlined, together with the major
...
\section{Zerepoch Core}
Zerepoch Core is a typed, eagerly evaluated $\lambda$-calculus designed
for use as a transaction validation language in blockchain
systems. More precisely, Zerepoch Core is the System $F_\omega$ of
...
of built-in basic types and functions).  Zerepoch Core is meant to be a
compilation target, and this is reflected in the design of the
language: while writing large Zerepoch Core programs by hand is
...
We have tried to keep the Zerepoch Core language small. There is no explicit
...
\paragraph{Implementation.} A compiler and evaluator for Zerepoch Core can be found
in~\citep{Zerepoch-exe-repo}.

\paragraph{Formalisation.} The Agda formalisation of Zerepoch Core mentioned earlier
can be found in~\citep{Zerepoch-metatheory-repo},
and the techniques used are described in ~\citep{System-F-in-Agda}.

\subsection{Blockchain issues}
\label{sec:blockchain-issues}
Zerepoch Core code (and code intended for execution on a blockchain in
...
Zerepoch Core is a typed language which is intended as a compiler
...
define an \textit{untyped} variant of Zerepoch core for deployment to
and execution on the chain (see Section~\ref{sec:untyped-plc}).
Untyped Zerepoch Core is obtained from typed Zerepoch Core by a simple
...
Zerepoch Core is intended to be used for validation of on-chain
transactions on the Bcc blockchain~\citep{Bcc}.  The details
of the validation process are outside the scope of this document, but
are explained in~\citep{EUTXO}. For details of Bcc's smart
contract infrastructure (including examples of the kind of validation
code which is compiled into Zerepoch Core) see the
book~\citep{Zerepoch-book}.



\section{Syntax}

The grammar of Zerepoch Core is given in
Figures~\ref{fig:lexical-grammar} and
\ref{fig:grammar} (but see the note on revised syntax in Section~\ref{sec:syntax-update}).
This grammar describes the abstract syntax trees of Zerepoch Core in a convenient
...
Section~\ref{sec:reduction} below we define reduction rules for Zerepoch
Core terms, and it can be shown (formally, in
Agda (\cite{Zerepoch-metatheory-repo})) that values are precisely those
terms which cannot undergo any further reduction.

\paragraph{Types.} Zerepoch Core contains a copy of the simply typed lambda calculus
...
two types are equal. The type-level structure of Zerepoch Core is
...
\paragraph{Kinds.} In Zerepoch Core we have a copy of the
simply typed lambda calculus at the type level. The types of the
simply typed lambda calculus are lifted to the level of
\textit{kinds} in Zerepoch Core, allowing the type system to talk about
...
Signatures are not used in the concrete syntax of Zerepoch Core, but we
...
Zerepoch Core: \texttt{ifix A B} is a type such that \texttt{ifix A B
...
\paragraph{Programs and Versions.} A complete Zerepoch Core program
consists of a standard version number (1.0.2, for example) indicating
the Zerepoch Core version and a \textit{closed} term (i.e., a term with no
free variables) forming the body of the program.  The version number
is used by the Zerepoch Core evaluator and other tools to check that
they are dealing with code conforming to the correct version of the
Zerepoch Core language.

\subsection{\red{Note: revised syntax}}
\label{sec:syntax-update}
\noindent In the current (April 2021) implementation of Zerepoch Core the syntax of built-in
...
We define for Zerepoch Core a number of typing judgments which explain
...
it means for a type to synthesise a kind. Zerepoch Core is a
...
\textit{reduction semantics}~\citep[\S2]{Felleisen-Hieb}) for Zerepoch
...
Because the Zerepoch Core type system contains a copy of the simply
...
Execution of Zerepoch Core programs is performed by (possibly
...
\subsection{An abstract machine for evaluating Zerepoch Core programs}
\label{sec:typed-ck-machine}
This section contains a description of an abstract machine for
executing Zerepoch Core.  This is based on the CK machine of Felleisen
...
\label{sec:builtins}
Zerepoch Core comes with a predefined set of built-in types and
...
\section{Examples}
\label{sec:examples}
The section contains some simple examples of Zerepoch Core code. The
...
\noindent Note that both branches above will always be evaluated because Zerepoch
...
\paragraph{Other examples.} The Zerepoch Core evaluator in~\citep{Zerepoch-exe-repo}
...
Zerepoch Core together with a type erasure function converting typed
Zerepoch Core terms to untyped ones.
...
\bigskip
\noindent The grammar is essentially that of typed Zerepoch Core with all types
...
As with typed Zerepoch Core, we can show that values (terms which can
...
converts a typed Zerepoch Core term $M$ into an untyped one $\erase{M}$
...
%% removed, and we should talk about untyped Zerepoch Core rather than
%% type-erased Zerepoch Core (captions of CK machine figure etc.)

\subsubsection{Note. Built-in functions in untyped Zerepoch Core}
\label{sec:untyped-ck-builtins}
We assume that untyped Zerepoch Core contains a set of built-in functions
...
is a well-typed Zerepoch Core program then
\begin{itemize}
...
for type-erased Zerepoch Core, analogously to
Figure~\ref{fig:term-reduction}.

\subfile{figures/UntypedTermReduction}

\subsection{An abstract machine for evaluating erased Zerepoch Core programs}
...
type-erased Zerepoch Core programs, implementing the semantics of
...
\section{A CEK machine for typed Zerepoch Core}
\label{appendix:typed-cek-machine}
In Section~\ref{sec:typed-ck-machine} we described the CK machine, an
abstract machine for executing Zerepoch Core programs.  As we pointed
...
\section{A CEK machine for type-erased Zerepoch Core}
\label{appendix:untyped-cek-machine}
As in the typed situation, the CK machine for type-erased Zerepoch Core
...
Zerepoch Core from the CK machine for typed Zerepoch Core.  We will refer
...
which are obtained by erasing types from well-typed Zerepoch Core terms.
...
arbitrary untyped Zerepoch Core terms.
...
bind variables to terms in untyped Zerepoch Core.  Again,
...
\begin{itemize}
\item $A_1, \ldots, A_k$ are Zerepoch Core types.
\item $V_1, \ldots, V_n$ are well-typed typed Zerepoch Core values.
...
hold, and that $P$ is a typed Zerepoch Core program.  Then if $P$ is
...
We use the \texttt{flat} \citep{flat} format to serialise Zerepoch Core terms. The
...
types of untyped Zerepoch Core and how they get serialised.
...
We will serialise the program \verb|(program 11.22.33 (con integer 11))| compiled to untyped zerepoch core, using DeBruijn indices.
...
We define `Program` in the `ZerepochCore.Core.Type` haskell module like this:
...
Let's take a quick look at how we defined untyped zerepoch terms, in Figure~\ref{fig:serialisation-terms}.
...
\begin{verbatim}
> stack repl zerepoch-core:exe:plc
...
\begin{verbatim}
> stack bench zerepoch-benchmark:flat
> cat zerepoch-benchmark/flat-sizes.md
...
\bibliography{zerepoch-core-specification}
