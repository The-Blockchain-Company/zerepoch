...
import Ledger.Extra (bccToValue)
...
import Zerepoch.PAB.Webserver (SPParams_(..))
import Zerepoch.PAB.Webserver.Types (ContractInstanceClientState(..), ContractSignatureResponse(..), CombinedWSStreamToClient, ContractActivationArgs(..))
import Zerepoch.V1.Ledger.Bcc (Bcc(..))
import Zerepoch.V1.Ledger.Value (Value)
...
--   priority is the new Simeon dashboard, however, so in the meantime large chunks of the PAB
--   client are commented out just so that it compiles.
initialValue :: Value
initialValue = bccToValue $ Entropic { getEntropic: zero }
