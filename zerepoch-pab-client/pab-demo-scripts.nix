...
let
  inherit (pab-exes)
    zerepoch-game
    zerepoch-currency
    zerepoch-atomic-swap
    zerepoch-pay-to-wallet
    prism-mirror
    prism-unlock-sto
  ...
  mkSetup = conf:
    ...
    runCommand "pab-setup" { } ''
      ...
      mkdir $out
      cp ${cfg} $out/zerepoch-pab.yaml
      ${pab-exes.zerepoch-pab-examples}/bin/zerepoch-pab-examples --config=$out/zerepoch-pab.yaml migrate
  ...
  pab-setup = "${pab-exes.zerepoch-pab-setup}/bin/zerepoch-pab-setup";
  ...
  runWithContracts = setup: cmd: writeShellScript "run-with-contracts" ''
    WORKDIR=$(mktemp -d)
    CFG_PATH=$WORKDIR/zerepoch-pab.yaml
    ...
    cat ${setup}/zerepoch-pab.yaml | ${yq}/bin/yq -y --arg path $DB_PATH '.dbConfig.dbConfigFile = $path' > $CFG_PATH
    ...
    cat $CFG_PATH
    echo "-----------------------------------------------------------------------------"
    ${pab-exes.zerepoch-pab-examples}/bin/zerepoch-pab-examples --config=$CFG_PATH ${cmd}
  '';
