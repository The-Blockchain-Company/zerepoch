...
import qualified Language.Simeon                                 as S
import qualified Language.Simeon.ACTUS.Definitions.ContractTerms as CT
import           Language.Simeon.Extended
import           Language.Simeon.Semantics                       (State (..))
...
import           Simeon.Contracts                                (contractForDifferences,
                                                                   contractForDifferencesWithOracle,
                                                                   couponBondGuaranteed, escrow, escrowWithCollateral,
                                                                   example, swap, zeroCouponBond)
import qualified Simeon.Symbolic.Server                          as MS
import qualified Simeon.Symbolic.Types.Request                   as MSReq
import qualified Simeon.Symbolic.Types.Response                  as MSRes
...
psContract =
    TypeInfo "simeon-playground-client" "Simeon.Semantics" "Contract" <$>
    psTypeParameters

contractBridge :: BridgePart
contractBridge = do
    typeName ^== "Contract"
    typeModule ^== "Language.Simeon.Semantics"
    psContract

psState :: MonadReader BridgeData m => m PSType
psState =
    TypeInfo "simeon-playground-client" "Simeon.Semantics" "State" <$>
    psTypeParameters
...
psTransactionInput =
    TypeInfo "simeon-playground-client" "Simeon.Semantics" "TransactionInput" <$>
    psTypeParameters
...
psTransactionWarning =
    TypeInfo "simeon-playground-client" "Simeon.Semantics" "TransactionWarning" <$>
    psTypeParameters
...
mySettings :: Settings
mySettings =
    (defaultSettings & set apiModuleName "Simeon")
...
writeUsecases outputDir = do
    ...
    let contractToString = BS8.pack . show . pretty
        ...
        simeonUsecasesModule = psModule "Examples.Simeon.Contracts" simeonUsecases
    createDirectoryIfMissing True (outputDir </> "Examples" </> "Simeon")
    BS.writeFile (outputDir </> "Examples" </> "Simeon" </> "Contracts.purs") simeonUsecasesModule
    putStrLn outputDir
...
writePangramJson outputDir = do
    ...
    let pangram =
            S.Assert S.TrueObs
                (S.When
                    [ S.Case (S.Deposit alicePk alicePk bcc valueExpr)
                        ( S.Let (S.ValueId "x") valueExpr
                            (S.Pay alicePk (S.Party bobRole) bcc (S.Cond S.TrueObs (S.UseValue (S.ValueId "x")) (S.UseValue (S.ValueId "y"))) S.Close)
                        )
