...
module Simeon.Mermaid (toMermaid) where
...
actionShow = escape . showAction
  where
    showAction (Deposit accountId party tkn val) = show party ++ " deposits " ++ valueShow val ++ " entropics into " ++ show accountId ++ " account"
