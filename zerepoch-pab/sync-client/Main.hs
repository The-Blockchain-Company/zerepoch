...
import           Bcc.Api                    (Block (..), BlockHeader (..), BlockInMode (..), ChainPoint (..),
                                                 HasTypeProxy (..), deserialiseFromRawBytesHex,
                                                 serialiseToRawBytesHexText)
import           Bcc.Protocol.Socket.Client (ChainSyncEvent (..), runChainSync)
import           Bcc.Protocol.Socket.Type   (cfgNetworkId)
import           Bcc.Slotting.Slot          (SlotNo (..))
import           Ledger                         (Slot)
import           Ledger.TimeSlot                (SlotConfig (..))

-- | We only need to know the location of the socket.
--   We can get the protocol versions from Bcc.Protocol.Socket.Type
data Configuration = Configuration
  { cSocketPath :: !String
  , cResumeHash :: !ChainPoint
  } deriving (Show)

-- | This is the slot configuration for the sophie network, taken from the PAB
-- configuration file.
