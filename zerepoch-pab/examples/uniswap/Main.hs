...
import qualified Zerepoch.Contracts.Uniswap            as Uniswap
import           Zerepoch.Contracts.Uniswap.Trace      as US
...
import           Zerepoch.PAB.Simulator                (SimulatorEffectHandlers, logString)
import qualified Zerepoch.PAB.Simulator                as Simulator
import qualified Zerepoch.PAB.Webserver.Server         as PAB.Server
...
main = void $ Simulator.runSimulationWith handlers $ do
    ...
    let coins = Map.fromList [(tn, Uniswap.mkCoin cs tn) | tn <- tokenNames]
        bcc   = Uniswap.mkCoin bccSymbol bccToken
    ...
    let cp = Uniswap.CreateParams bcc (coins Map.! "A") 100000 500000
