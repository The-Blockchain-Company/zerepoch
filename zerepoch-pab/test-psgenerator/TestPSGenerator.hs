...
import           Zerepoch.Contracts.Currency                  (SimpleMPS (..))
import           Zerepoch.PAB.Effects.Contract.ContractTest   (TestContracts (Currency, GameStateMachine))
import qualified Zerepoch.PAB.Simulator                       as Simulator
import qualified Zerepoch.PAB.Simulator.Test                  as Simulator
import qualified Zerepoch.PAB.Webserver.Handler               as Webserver
import           Zerepoch.PAB.Webserver.Types                 (ContractSignatureResponse, FullReport)
...
main = getArgs >>= \case
    ...
    _ -> putStrLn "Usage: zerepoch-pab-test-psgenerator FILEPATH"
