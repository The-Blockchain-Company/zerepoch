...
import           Ledger.Bcc                               (bccSymbol, bccToken, entropicValueOf)
import qualified Ledger.Bcc                               as Bcc
import qualified Ledger.AddressMap                        as AM
import           Ledger.Value                             (valueOf)
import           Zerepoch.Contract.State                    (ContractResponse (..))
import           Zerepoch.Contracts.Currency                (OneShotCurrency, SimpleMPS (..))
import qualified Zerepoch.Contracts.GameStateMachine        as Contracts.GameStateMachine
import           Zerepoch.Contracts.PingPong                (PingPongState (..))
import           Zerepoch.PAB.Core                          as Core
import           Zerepoch.PAB.Core.ContractInstance         (ContractInstanceMsg)
import           Zerepoch.PAB.Core.ContractInstance.STM     (BlockchainEnv (..))
import           Zerepoch.PAB.Effects.Contract              (ContractEffect, serialisableState)
import           Zerepoch.PAB.Effects.Contract.Builtin      (Builtin)
...
import           Zerepoch.PAB.Simulator                     (Simulation, TxCounts (..))
import qualified Zerepoch.PAB.Simulator                     as Simulator
import qualified Zerepoch.PAB.Simulator.Test                as Simulator
import           Zerepoch.PAB.Types                         (PABError (..), chainOverviewBlockchain, mkChainOverview)
import qualified Zerepoch.PAB.Webserver.WebSocket           as WS
import           ZerepochTx.Monoid                          (Group (inv))
...
tests :: TestTree
tests = testGroup "Zerepoch.PAB.Core" [activateContractTests, executionTests]
...
walletFundsChangeTest = runScenario $ do
    let initialBalance = entropicValueOf 10_000_000_000
        payment = entropicValueOf 50
        fee     = entropicValueOf 10 -- TODO: Calculate the fee from the tx
          ...
          runScenario $ do
              let openingBalance = 100_000_000_000
                  ...
                  walletFundsChange msg delta = do
                        ...
                        assertEqual msg
                            ...
                            (valueOf (balance <> fees) bccSymbol bccToken)
              initialTxCounts <- Simulator.txCounts
              ...
              lock
                  ...
                  Contracts.GameStateMachine.LockArgs
                      { Contracts.GameStateMachine.lockArgsValue = entropicValueOf lockAmount
              ...
              guess
                  ...
                  Contracts.GameStateMachine.GuessArgs
                      ...
                      , Contracts.GameStateMachine.guessArgsValueTakenOut = entropicValueOf lockAmount
                      }
