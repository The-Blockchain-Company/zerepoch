...
module Bcc.Protocol.Socket.Mock.Server where
...
import           Shardagnostic.Network.Protocol.ChainSync.Server         (ChainSyncServer (..), ServerStIdle (..),
                                                                      ServerStIntersect (..), ServerStNext (..))
import qualified Shardagnostic.Network.Protocol.ChainSync.Server         as ChainSync
import qualified Shardagnostic.Network.Protocol.LocalTxSubmission.Server as TxSubmission
import qualified Shardagnostic.Network.Protocol.LocalTxSubmission.Type   as TxSubmission
import qualified Zerepoch.PAB.Monitoring.Util                          as LM

import           Bcc.Slotting.Slot                               (SlotNo (..), WithOrigin (..))
import           Shardagnostic.Network.Block                             (Point (..), pointSlot)
...
import qualified Shardagnostic.Network.Point                             as OP (Block (..))
...
import           Bcc.Protocol.Socket.Type

import           Bcc.Chain                                       (MockNodeServerChainState (..), addTxToPool,
...
{- This is boilerplate code that sets up the node protocols,
   you can find in:
     shardagnostic-network/shardagnostic-network/demo/chain-sync.hs -}
