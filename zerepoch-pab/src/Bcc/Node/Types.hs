...
import           Bcc.BM.Data.Tracer.Extras       (Tagged (..), mkObjectStr)
import           Bcc.Chain                       (MockNodeServerChainState, fromEmulatorChainState)
import qualified Bcc.Protocol.Socket.Mock.Client as Client
...
import qualified Zerepoch.Contract.Trace               as Trace
...
import           Bcc.Api.NetworkId.Extra         (NetworkIdWrapper (..), testnetNetworkId)
import           Ledger.Fee                          (FeeConfig)
import           Zerepoch.PAB.Arbitrary                ()
...
data NodeMode =
    ...
    | AurumNode -- ^ Connect to an Aurum node
...
data MockServerConfig =
    MockServerConfig
        ...
        -- ^ NetworkId that's used with the BccAPI.
        , mscNodeMode         :: NodeMode
        -- ^ Whether to connect to an Aurum node or a mock node
        }
    deriving stock (Show, Eq, Generic)
...
defaultMockServerConfig =
    MockServerConfig
      -- See Note [pab-ports] in 'test/full/Zerepoch/PAB/CliSpec.hs'.
...
-- | 'AppState' with an initial transaction that pays some Bcc to
--   the wallets.
