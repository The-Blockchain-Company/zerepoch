...
module Bcc.Node.Server
    ( main
    ) where

import           Bcc.BM.Data.Trace               (Trace)
import           Bcc.Node.API                    (API)
import           Bcc.Node.Mock
import           Bcc.Node.Types
import qualified Bcc.Protocol.Socket.Mock.Client as Client
import qualified Bcc.Protocol.Socket.Mock.Server as Server
...
main trace MockServerConfig { mscBaseUrl
    ...
    -- make initial distribution of 1 billion Bcc to all configured wallets
    let dist = Map.fromList $ zip (fromWalletNumber <$> mscInitialTxWallets) (repeat (Bcc.bccValueOf 1000_000_000))
