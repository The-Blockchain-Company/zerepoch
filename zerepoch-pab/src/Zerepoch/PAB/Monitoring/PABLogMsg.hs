...
module Zerepoch.PAB.Monitoring.PABLogMsg(
    PABLogMsg(..),
    ChainIndexServerMsg,
    WalletMsg,
    MockServerLogMsg,
...
import           Bcc.BM.Data.Tracer           (ToObject (..), TracingVerbosity (..))
import           Bcc.BM.Data.Tracer.Extras    (StructuredLog, Tagged (..), mkObjectStr)
import           Bcc.ChainIndex.Types         (ChainIndexServerMsg)
import           Bcc.Node.Types               (MockServerLogMsg)
import           Bcc.Wallet.Types             (WalletMsg)
import           Data.Aeson.Text                  (encodeToLazyText)
import qualified Data.Text                        as T
import           Zerepoch.Contract.Effects          (PABReq, PABResp)
import           Zerepoch.Contract.Resumable        (Response)
import           Zerepoch.Contract.State            (ContractResponse)
import           Zerepoch.PAB.Core.ContractInstance (ContractInstanceMsg (..))
import           Zerepoch.PAB.Effects.Contract      (PABContract (..))
import           Zerepoch.PAB.Events.Contract       (ContractInstanceId)
import           Zerepoch.PAB.Instances             ()
...
instance Pretty (ContractDef t) => Pretty (CoreMsg t) where
    pretty = \case
        ...
        ConnectingToAurumNode -> "Connecting to Aurum node"
        ...
        FoundContract state ->
            mkObjectStr "found contract" $
                case v of
                    ...
                    _                -> Right ()
        ConnectingToAurumNode -> mkObjectStr "Connecting to Aurum node" ()
