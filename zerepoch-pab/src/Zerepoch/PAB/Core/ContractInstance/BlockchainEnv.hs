...
-- |
module Zerepoch.PAB.Core.ContractInstance.BlockchainEnv(
  startNodeClient
  , processMockBlock
...
import           Bcc.Api                            (BlockInMode (..), ChainPoint (..), NetworkId)
import qualified Bcc.Api                            as C
import           Bcc.Node.Types                     (NodeMode (..))
import           Bcc.Protocol.Socket.Client         (ChainSyncEvent (..))
import qualified Bcc.Protocol.Socket.Client         as Client
import qualified Bcc.Protocol.Socket.Mock.Client    as MockClient
...
import           Zerepoch.PAB.Core.ContractInstance.STM   (BlockchainEnv (..), InstanceClientEnv (..), InstancesState,
                                                         OpenTxOutProducedRequest (..), OpenTxOutSpentRequest (..),
                                                         emptyBlockchainEnv)
import qualified Zerepoch.PAB.Core.ContractInstance.STM   as S
import           Zerepoch.Trace.Emulator.ContractInstance (IndexedBlock (..), indexBlock)
import           Zerepoch.V1.Ledger.Api                   (toBuiltin)
...
import           Zerepoch.ChainIndex                      (BlockNumber (..), ChainIndexTx (..), ChainIndexTxOutputs (..),
                                                         InsertUtxoSuccess (..), RollbackResult (..), Tip (..),
                                                         TxConfirmedState (..), TxIdState (..), TxValidity (..),
                                                         UtxoState (..), blockId, citxTxId, fromOnChainTx, insert)
import           Zerepoch.ChainIndex.Compatibility        (fromBccBlockHeader, fromBccPoint)
import           Zerepoch.ChainIndex.TxIdState            (rollback)
...
-- | Process a chain sync event that we receive from the aurum node client
...
runRollback BlockchainEnv{beTxChanges} chainPoint = do
  ...
  let point = fromBccPoint chainPoint
...
-- | Get transaction ID and validity from a bcc transaction in any era
txEvent :: forall era. C.Tx era -> C.EraInMode era C.BccMode -> (TxId, TxValidity)
txEvent (C.Tx body _) _ =
  let txi = fromBccTxId (C.getTxId @era body)
  in (txi, TxValid)  -- TODO: Validity for Aurum transactions (not available in bcc-api yet (?))

fromBccTxId :: C.TxId -> TxId
fromBccTxId = TxId . toBuiltin . C.serialiseToRawBytes
...
-- | Update the blockchain env. with changes from a new block of bcc
--   transactions in any era
processBlock :: forall era. C.BlockHeader -> BlockchainEnv -> [C.Tx era] -> C.EraInMode era C.BccMode -> STM ()
processBlock header env transactions era =
  unless (null transactions) $ do
    let tip = fromBccBlockHeader header
