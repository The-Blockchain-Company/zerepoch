...
import qualified Bcc.Wallet.Mock                            as MockWallet
...
import           Zerepoch.ChainIndex                              (ChainIndexControlEffect, ChainIndexEmulatorState,
                                                                 ...
                                                                 TxStatus, getTip)
import qualified Zerepoch.ChainIndex                              as ChainIndex
...
import           Zerepoch.PAB.Core.ContractInstance.STM           (Activity (..), BlockchainEnv (..), OpenEndpoint)
...
import           Zerepoch.PAB.Effects.Contract.Builtin            (HasDefinitions (..))
import           Zerepoch.PAB.Effects.TimeEffect                  (TimeEffect)
import           Zerepoch.PAB.Monitoring.PABLogMsg                (PABMultiAgentMsg (..))
import           Zerepoch.PAB.Types                               (PABError (ContractInstanceNotFound, WalletError, WalletNotFound))
import           Zerepoch.PAB.Webserver.Types                     (ContractActivationArgs (..))
import           Zerepoch.Trace.Emulator.System                   (appendNewTipBlock)
...
initialState = do
    let initialDistribution = Map.fromList $ fmap (, Bcc.bccValueOf 100_000) knownWallets
...
mkSimulatorHandlers ::
    ...
    => FeeConfig
    -> SlotConfig
    -> SimulatorContractHandler t -- ^ Making calls to the contract (see 'Zerepoch.PAB.Effects.Contract.ContractTest.handleContractTest' for an example)
