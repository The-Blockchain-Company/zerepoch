...
module Zerepoch.PAB.Arbitrary where
...
import           Zerepoch.Contract.StateMachine      (ThreadToken)
...
acceptingValidator = Ledger.mkValidatorScript $$(ZerepochTx.compile [|| (\_ _ _ -> ()) ||])

-- | A minting policy that always succeeds.
acceptingMintingPolicy :: Ledger.MintingPolicy
acceptingMintingPolicy = Ledger.mkMintingPolicyScript $$(ZerepochTx.compile [|| (\_ _ -> ()) ||])

instance Arbitrary ZerepochTx.BuiltinByteString where
    arbitrary = ZerepochTx.toBuiltin <$> (arbitrary :: Gen ByteString)
...
instance Arbitrary ZerepochTx.Data where
    arbitrary = genericArbitrary
    shrink = genericShrink

instance Arbitrary ZerepochTx.BuiltinData where
    arbitrary = ZerepochTx.dataToBuiltinData <$> arbitrary
    shrink d = ZerepochTx.dataToBuiltinData <$> shrink (ZerepochTx.builtinDataToData d)
