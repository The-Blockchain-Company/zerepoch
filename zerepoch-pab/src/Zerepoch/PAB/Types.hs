...
module Zerepoch.PAB.Types where
...
import           Zerepoch.Contract.Types     (ContractError)
...
instance Pretty PABError where
    pretty = \case
        ...
        MigrationNotDoneError msg  -> pretty msg
                                   ...
                                   <+> "(ex. 'zerepoch-pab-migrate' or 'zerepoch-pab-examples --config <CONFIG_FILE> migrate')"
...
newtype RequestProcessingConfig =
    RequestProcessingConfig
        { requestProcessingInterval :: Second -- ^ How many seconds to wait between calls to 'Zerepoch.PAB.Core.ContractInstance.processAllContractOutboxes'
        }
    deriving (Show, Eq, Generic)
