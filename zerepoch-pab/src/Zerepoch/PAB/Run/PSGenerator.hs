...
module Zerepoch.PAB.Run.PSGenerator
    ( HasPSTypes(..)
    , generateDefault
    , generateWith
    , generateAPIModule
...
import           Zerepoch.Contract.Checkpoint                 (CheckpointKey, CheckpointStore, CheckpointStoreItem)
import           Zerepoch.Contract.Resumable                  (Responses)
import           Zerepoch.Contract.StateMachine               (InvalidTransition, SMContractError)
import           Zerepoch.Contract.StateMachine.OnChain       (State)
...
import qualified Zerepoch.PAB.Webserver.API                   as API
import           Zerepoch.PAB.Webserver.Types                 (ChainReport, CombinedWSStreamToClient,
...
mySettings :: Settings
mySettings =
    (defaultSettings & set apiModuleName "Zerepoch.PAB.Webserver")
        {_generateSubscriberAPI = False}

------------------------------------------------------------
-- | Use the Proxy for specifying `a` when generating PS functions for the
-- webserver using a specific Builtin (ex. generate 'Builtin Simeon' instead
-- of 'Builtin a').
generateAPIModule
    :: forall a. Typeable a
    => Proxy a
