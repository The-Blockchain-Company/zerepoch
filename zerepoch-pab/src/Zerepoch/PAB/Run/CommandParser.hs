...
module Zerepoch.PAB.Run.CommandParser (parseOptions, AppOpts(..)) where
...
mockNodeParser =
    command "node-server" $
        info
            (pure StartMockNode)
            (fullDesc <> progDesc "Run a mock version of the Bcc node API server.")
...
mockWalletParser =
    ...
    info
        (pure MockWallet)
        (fullDesc <>
         progDesc "Run a mock version of the Bcc wallet API server.")
