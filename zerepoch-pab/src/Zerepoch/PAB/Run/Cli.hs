...
module Zerepoch.PAB.Run.Cli (ConfigCommandArgs(..), runConfigCommand, runNoConfigCommand) where
...
import qualified Bcc.ChainIndex.Server             as ChainIndex
import qualified Bcc.Node.Server                   as NodeServer
import           Bcc.Node.Types                    (MockServerConfig (..), NodeMode (..))
import qualified Bcc.Wallet.Server                 as WalletServer
import           Bcc.Wallet.Types
...
import           Zerepoch.Contract.Resumable             (responses)
import           Zerepoch.Contract.State                 (State (..))
import qualified Zerepoch.Contract.State                 as State
import qualified Zerepoch.PAB.App                        as App
import qualified Zerepoch.PAB.Core                       as Core
import           Zerepoch.PAB.Core.ContractInstance      (ContractInstanceState (..), updateState)
import           Zerepoch.PAB.Core.ContractInstance.STM  (InstanceState, emptyInstanceState)
import qualified Zerepoch.PAB.Db.Beam                    as Beam
import qualified Zerepoch.PAB.Effects.Contract           as Contract
import           Zerepoch.PAB.Effects.Contract.Builtin   (Builtin, BuiltinHandler, HasDefinitions (..),
                                                        SomeBuiltinState (..), getResponse)
import qualified Zerepoch.PAB.Monitoring.Monitoring      as LM
import           Zerepoch.PAB.Run.Command
import           Zerepoch.PAB.Run.PSGenerator            (HasPSTypes (..))
import qualified Zerepoch.PAB.Run.PSGenerator            as PSGenerator
import           Zerepoch.PAB.Types                      (Config (..), chainIndexConfig, nodeServerConfig,
                                                        walletServerConfig)
import qualified Zerepoch.PAB.Webserver.Server           as PABServer
    ...
    case mscNodeMode nodeServerConfig of
        ...
        AurumNode -> do
            ...
            runM
                ...
                $ LM.PABMsg
                $ LM.SCoreMsg LM.ConnectingToAurumNode
            pure () -- TODO: Log message that we're connecting to the real Aurum node
