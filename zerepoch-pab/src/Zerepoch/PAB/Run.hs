...
module Zerepoch.PAB.Run
( runWith
, runWithOpts
) where

import qualified Bcc.BM.Backend.EKGView          as EKGView
...
import           Zerepoch.PAB.Monitoring.Config        (defaultConfig, loadConfig)
import           Zerepoch.PAB.Monitoring.PABLogMsg     (AppMsg (..))
import           Zerepoch.PAB.Monitoring.Util          (PrettyObject (..), convertLog)
import           Zerepoch.PAB.Run.Cli
import           Zerepoch.PAB.Run.CommandParser
import           Zerepoch.PAB.Run.PSGenerator          (HasPSTypes)
import           Zerepoch.PAB.Types                    (Config (..), PABError (MissingConfigFileOption))
...
runWith :: forall a.
    ...
    => BuiltinHandler a -- ^ Builtin contract handler. Can be created with 'Zerepoch.PAB.Effects.Contract.Builtin.handleBuiltin'.
    -> IO ()
