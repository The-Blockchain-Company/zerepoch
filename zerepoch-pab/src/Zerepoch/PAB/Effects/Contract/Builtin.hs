...
-}
module Zerepoch.PAB.Effects.Contract.Builtin(
    Builtin
    , ContractConstraints
    , SomeBuiltin(..)
    , SomeBuiltinState(..)
...
import           Zerepoch.Contract                                  (ContractInstanceId, EmptySchema, IsContract (..))
import           Zerepoch.Contract.Effects                          (PABReq, PABResp)
import           Zerepoch.Contract.Resumable                        (Response, responses)
import           Zerepoch.Contract.Schema                           (Input, Output)
import           Zerepoch.Contract.State                            (ContractResponse (..), State (..))
import qualified Zerepoch.Contract.State                            as ContractState
import           Zerepoch.Contract.Types                            (ResumableResult (..), SuspendedContract (..))
import           Zerepoch.PAB.Core.ContractInstance.RequestHandlers (ContractInstanceMsg (ContractLog, ProcessFirstInboxMessage))
import           Zerepoch.PAB.Effects.Contract                      (ContractEffect (..), PABContract (..))
import           Zerepoch.PAB.Monitoring.PABLogMsg                  (PABMultiAgentMsg (..))
import           Zerepoch.PAB.Types                                 (PABError (..))
import           Zerepoch.Trace.Emulator.Types                      (ContractInstanceStateInternal (..))
import qualified Zerepoch.Trace.Emulator.Types                      as Emulator
...
-- | Zerepoch contract with all parameters existentially quantified. Can be any contract that satisfies the
--   'ContractConstraints'.
