...
{-# LANGUAGE StrictData  #-}
module Zerepoch.PAB.Events.Contract(
  -- $contract-events
  ContractInstanceId(..)
  , IterationID
  ) where

import           Zerepoch.Contract.Resumable (IterationID)

import           Wallet.Types              (ContractInstanceId (..))

-- $contract-events
-- The events that compiled Zerepoch contracts are concerned with. For each type
