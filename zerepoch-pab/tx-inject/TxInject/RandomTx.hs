...
-- $randomTx
-- Generate a random, valid transaction that moves some bcc
...
generateTx gen slot (UtxoIndex utxo) = do
  ...
  -- outputs at the source address
      sourceOutputs
  -- we restrict ourselves to outputs that contain no currencies other than Bcc,
  ...
  -- We definitely need this for creating multi currency transactions!
        =
          filter
            (\(_, TxOut {txOutValue}) ->
                txOutValue ==
                  Bcc.toValue (Bcc.fromValue txOutValue)) $
          filter
  ...
  then generateTx gen slot (UtxoIndex utxo)
    ...
    -- Total Bcc amount that we want to spend
    let sourceBcc =
          foldMap
            (Bcc.fromValue . txOutValue . snd)
            inputs
        -- inputs of the transaction
        sourceTxIns = Set.fromList $ fmap (Tx.pubKeyTxIn . fst) inputs
    tx <- Gen.sample $
      Generators.genValidTransactionSpending sourceTxIns sourceBcc
