...
import Simeon.Holes (AccountId, Action(..), Bound(..), Case(..), ChoiceId(..), Contract(..), Location(..), Observation(..), Party(..), Payee(..), Term(..), TermWrapper(..), Timeout(SlotParam), Token(..), Value(..), ValueId(..), getLocation, mkHole)
import Simeon.Holes as H
import Simeon.Semantics (Rational(..), Slot(..))
...
locationToRange (BlockId _) = unsafeThrow "Unexpected BlockId location found in SimeonParser"

locationToRange NoLocation = unsafeThrow "Unexpected NoLocation found in SimeonParser"
