...
import Simeon.Holes (Action(..), Bound(..), Case(..), ChoiceId(..), Contract(..), Location(..), Observation(..), Party(..), Payee(..), Term(..), TermWrapper(..), Timeout(..), Token(..), Value(..), ValueId(..))
import Simeon.Semantics (Rational(..))
import Record (merge)
...
-- Bcc Token type
toDefinition blockType@(TokenType BccTokenType) =
  BlockDefinition
    $ merge
        { type: show BccTokenType
        , message0: "bcc"
        , args0: []
        , colour: blockColour blockType
        , output: Just "token"
...
instance blockToTermToken :: BlockToTerm Token where
  blockToTerm b@({ type: "BccTokenType", id }) = pure $ Term (Token "" "") (BlockId id)
...
instance toBlocklyToken :: ToBlockly Token where
  toBlockly newBlock workspace input (Token "" "") = do
    block <- newBlock workspace (show BccTokenType)
    connectToOutput block input
