-- TODO: Rename to Simeon.Term
module Simeon.Holes where
...
import Simeon.Semantics (class HasTimeout, CurrencySymbol, IntervalResult(..), PubKey, Rational(..), Timeouts(..), TokenName, _slotInterval, fixInterval, ivFrom, ivTo, timeouts)
import Simeon.Semantics as S
import Simeon.Template (class Fillable, class Template, Placeholders(..), TemplateContent, fillTemplate, getPlaceholderIds)
...
-- | These are Simeon types that can be represented as holes
data SimeonType
  = ChoiceIdType
  | ValueIdType
  | ActionType
...
derive instance eqSimeonType :: Eq SimeonType

derive instance genericSimeonType :: Generic SimeonType _

instance showSimeonType :: Show SimeonType where
  show = genericShow

derive instance ordSimeonType :: Ord SimeonType

instance enumSimeonType :: Enum SimeonType where
  succ = genericSucc
  pred = genericPred

instance boundedSimeonType :: Bounded SimeonType where
  bottom = genericBottom
  top = genericTop

instance boundedEnumSimeonType :: BoundedEnum SimeonType where
  cardinality = genericCardinality
  toEnum = genericToEnum
  fromEnum = genericFromEnum

allSimeonTypes :: Array SimeonType
allSimeonTypes = upFromIncluding bottom

readSimeonType :: String -> Maybe SimeonType
readSimeonType "ChoiceIdType" = Just ChoiceIdType

readSimeonType "ValueIdType" = Just ValueIdType

readSimeonType "ActionType" = Just ActionType

readSimeonType "PayeeType" = Just PayeeType

readSimeonType "CaseType" = Just CaseType

readSimeonType "ValueType" = Just ValueType

readSimeonType "ObservationType" = Just ObservationType

readSimeonType "ContractType" = Just ContractType

readSimeonType "BoundType" = Just BoundType

readSimeonType "TokenType" = Just TokenType

readSimeonType "PartyType" = Just PartyType

readSimeonType "TimeoutType" = Just TimeoutType

readSimeonType _ = Nothing

-- | To make a name for a hole we show the type, make the first letter lower case and then drop the "Type" at the end
mkArgName :: SimeonType -> String
...
-- | that is what SimpleArgument does. See implementation of `constructSimeonType` function.
data TermGenerator
  = ArgumentArray (Array Argument)
  | SimpleArgument Argument

getSimeonConstructors :: SimeonType -> Map String TermGenerator
getSimeonConstructors ChoiceIdType = Map.singleton "ChoiceId" $ ArgumentArray [ DefaultString "choiceNumber", DataArg PartyType ]

getSimeonConstructors ValueIdType = mempty

getSimeonConstructors ActionType =
  Map.fromFoldable
...
getSimeonConstructors PayeeType =
  Map.fromFoldable
    [ (Tuple "Account" $ ArgumentArray [ DataArg PartyType ])
    , (Tuple "Party" $ ArgumentArray [ DataArg PartyType ])
    ]

getSimeonConstructors CaseType = Map.singleton "Case" $ ArgumentArray [ DataArg ActionType, DataArg ContractType ]

getSimeonConstructors ValueType =
  Map.fromFoldable
...
getSimeonConstructors ObservationType =
  Map.fromFoldable
...
getSimeonConstructors ContractType =
  Map.fromFoldable
...
getSimeonConstructors BoundType = Map.singleton "Bound" $ ArgumentArray [ DefaultNumber zero, DefaultNumber zero ]

getSimeonConstructors TokenType = Map.singleton "Token" $ ArgumentArray [ DefaultString "", DefaultString "" ]

getSimeonConstructors PartyType =
  Map.fromFoldable
...
getSimeonConstructors TimeoutType =
  Map.fromFoldable
    [ (Tuple "Slot" $ SimpleArgument $ DefaultNumber zero)
    , (Tuple "SlotParam" $ ArgumentArray [ DefaultString "slotParameterName" ])
    ]

allSimeonConstructors :: Map String TermGenerator
allSimeonConstructors = foldMap getSimeonConstructors allSimeonTypes

allSimeonConstructorNames :: Map String SimeonType
allSimeonConstructorNames = foldMap f allSimeonTypes
  where
  f :: SimeonType -> Map String SimeonType
  f simeonType' =
    let
      constructors = Map.keys $ getSimeonConstructors simeonType'

      kvs :: Array (Tuple String SimeonType)
      kvs = Set.toUnfoldable $ Set.map (\constructor -> Tuple constructor simeonType') constructors
    in
      Map.fromFoldable kvs

-- Based on a String representation of a constructor get the SimeonType
-- "Close" -> ContractType
-- "PK" -> PartyType
getSimeonTypeFromConstructor :: String -> Maybe SimeonType
getSimeonTypeFromConstructor constructor =
  let
    f t = Set.member constructor (Map.keys (getSimeonConstructors t))
  in
    Array.find f allSimeonTypes

-- | Return a constructor name if there is exactly one constructor for a type
getConstructorFromSimeonType :: SimeonType -> Maybe String
getConstructorFromSimeonType t = case Set.toUnfoldable $ Map.keys (getSimeonConstructors t) of
...
constructSimeonType :: String -> SimeonHole -> Map String TermGenerator -> String
constructSimeonType constructorName (SimeonHole { location }) m = case Map.lookup constructorName m of
  Nothing -> ""
  ...
  showArgument (GenArg simeonType) = case getConstructorFromSimeonType simeonType of
    Nothing -> "?hole"
    Just name ->
      let
        newArgs = getSimeonConstructors simeonType

        hole = SimeonHole { name, simeonType, location: NoLocation }
      in
        constructSimeonType name hole newArgs
  ...
  --   * See if this belongs to LinterMarker rather than Simeon.Holes, as it's relying on the textual
...
--        The other use of ordering is in SimeonHole, to be able to insert them in the `Holes` Set.
...
instance termHasSimeonHoles :: (IsSimeonType a, HasSimeonHoles a) => HasSimeonHoles (Term a) where
...
newtype SimeonHole
  = SimeonHole
  { name :: String
  , simeonType :: SimeonType
  , location :: Location
  }

derive instance genericSimeonHole :: Generic SimeonHole _

derive instance eqSimeonHole :: Eq SimeonHole

instance ordSimeonHole :: Ord SimeonHole where
  compare (SimeonHole { location: a }) (SimeonHole { location: b }) = compareLocation a b

class IsSimeonType a where
  simeonType :: Proxy a -> SimeonType

-- a Monoid for collecting Holes
newtype Holes
  = Holes (Map String (Set SimeonHole))
...
insertHole :: forall a. IsSimeonType a => Term a -> Holes -> Holes
insertHole (Term _ _) m = m

insertHole (Hole name proxy location) (Holes m) = Holes $ Map.alter f name m
  where
  simeonHole = SimeonHole { name, simeonType: (simeonType proxy), location }

  f v = Just (Set.insert simeonHole $ fromMaybe mempty v)

instance arrayHasSimeonHoles :: HasSimeonHoles a => HasSimeonHoles (Array a) where
...
instance boundIsSimeonType :: IsSimeonType Bound where
  simeonType _ = BoundType

instance boundHasSimeonHoles :: HasSimeonHoles Bound where
...
instance timeoutIsSimeonType :: IsSimeonType Timeout where
  simeonType _ = TimeoutType

instance timeoutHasSimeonHoles :: HasSimeonHoles Timeout where
  getHoles (Slot a) m = m
...
instance partyIsSimeonType :: IsSimeonType Party where
  simeonType _ = PartyType

instance partyHasSimeonHoles :: HasSimeonHoles Party where
  getHoles (PK a) m = m
  getHoles (Role a) m = m
...
instance tokenIsSimeonType :: IsSimeonType Token where
  simeonType _ = TokenType

instance tokenHasSimeonHoles :: HasSimeonHoles Token where
...
instance choiceIdIsSimeonType :: IsSimeonType ChoiceId where
  simeonType _ = ChoiceIdType

instance choiceIdHasSimeonHoles :: HasSimeonHoles ChoiceId where
...
instance actionSimeonType :: IsSimeonType Action where
  simeonType _ = ActionType
...
instance payeeSimeonType :: IsSimeonType Payee where
  simeonType _ = PayeeType

instance payeeHasSimeonHoles :: HasSimeonHoles Payee where
...
instance caseSimeonType :: IsSimeonType Case where
  simeonType _ = CaseType
...
instance valueIsSimeonType :: IsSimeonType Value where
  simeonType _ = ValueType
...
instance observationIsSimeonType :: IsSimeonType Observation where
  simeonType _ = ObservationType
...
instance contractIsSimeonType :: IsSimeonType Contract where
  simeonType _ = ContractType
...
instance valueIdIsSimeonType :: IsSimeonType ValueId where
  simeonType _ = ValueIdType

instance valueIdHasSimeonHoles :: HasSimeonHoles ValueId where
  getHoles _ m = m
...
---------------------------------------------------------------------------------------------------
-- TODO: Move to Simeon.Term.Semantic
-- Semantic functions
--
-- These functions and data structures are a trimmed down version from the correspondig Simeon.Semantics
...
--  * Bccpt a full copy-paste of the semantic code:
