...
import Simeon.Extended.Metadata (MetadataHintInfo, _choiceNames, _roles, _slotParameters, _valueParameters)
import Data.Set.Ordered.OSet as OSet
import Simeon.Holes (Action(..), Bound(..), Case(..), ChoiceId(..), Contract(..), Holes, Location(..), Observation(..), Party(..), Payee(..), Term(..), TermWrapper(..), Value(..), compareLocation, fromTerm, getHoles, getLocation, insertHole)
import Simeon.Holes as Holes
import Simeon.Semantics (Rational(..), Slot(..), emptyState, evalValue, makeEnvironment)
...
-- FIXME: This generic Ord requires a lot of instances defined in Simeon.Holes and only for the fallback
...
-- Note on PR https://github.com/The-Blockchain-Company/zerepoch/pull/2560:
--    Before PR 2560 which splitted the Simulation from the Simeon Editor, the lint function was called on each step
--    of the simulation with the state as a parameter. That way, we could take past actions into account. Once we
--    split the two pages, we decided to only lint on the Simeon Editor using the full contract, and for that reason
...
--    want to add the state once again, and the Simeon Editor should probably use the empty state to indicate no past
