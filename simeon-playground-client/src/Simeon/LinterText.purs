module Simeon.LinterText
  ( AdditionalContext
  , suggestions
  , markers
  , provideCodeActions
  , format
  , simeonHoleToSuggestionText
  ) where
...
import Simeon.Holes (Contract, Holes(..), Location(..), SimeonHole(..), SimeonType, Term, TermGenerator, constructSimeonType, getSimeonConstructors, readSimeonType)
import Simeon.Linter (Warning(..), WarningDetail(..), _holes, _metadataHints, _warnings, lint)
import Simeon.Parser (ContractParseError(..), parseContract)
...
-- FIXME: We have multiple model markers, 1 per quick fix. This is wrong though, we need only 1 but in SimeonCodeActionProvider we want to run the code
...
holeToMarker :: SimeonHole -> Map String TermGenerator -> String -> IMarkerData
holeToMarker hole@(SimeonHole { name, simeonType, location }) m constructorName =
  let
    ...
    -- in 3. General linter that does not care about Location, SimeonLinter that deals with `Warning IRange`
  ...
  in
    ...
    , endColumn
    , endLineNumber
    , message: holeText simeonType
    , severity: markerSeverity "Warning"
    , code: ""
    , source: "Hole: " <> (dropEnd 4 $ show simeonType)
    }

holeToMarkers :: SimeonHole -> Array IMarkerData
holeToMarkers hole@(SimeonHole { name, simeonType }) =
  let
    m = getSimeonConstructors simeonType

    constructors = take 1 $ Set.toUnfoldable $ Map.keys m
  in
    map (holeToMarker hole m) constructors

markerToHole :: IMarkerData -> SimeonType -> SimeonHole
markerToHole markerData simeonType = SimeonHole { name: "unknown", simeonType, location: Range (Monaco.getRange markerData) }
...
provideCodeActions uri markers' additionalContext =
  provideLintCodeActions uri additionalContext
    <> (flip foldMap) markers' \(marker@{ source, startLineNumber, startColumn, endLineNumber, endColumn }) -> case regex "Hole: (\\w+)" noFlags of
        Left _ -> []
        Right r -> case readSimeonType =<< (join <<< (flip index 1)) =<< match r (source <> "Type") of
          Nothing -> []
          Just simeonType ->
            let
              m = getSimeonConstructors simeonType

              hole = markerToHole marker simeonType
              ...
              actions =
                mapFlipped constructors \constructorName ->
                  let
                    text = constructSimeonType constructorName hole m
...
simeonHoleToSuggestionText :: Boolean -> SimeonHole -> String -> String
simeonHoleToSuggestionText stripParens firstHole@(SimeonHole { simeonType }) constructorName =
  let
    m = getSimeonConstructors simeonType

    fullInsertText = constructSimeonType constructorName firstHole m
  in
    if stripParens then
...
simeonHoleToSuggestion :: String -> Boolean -> IRange -> SimeonHole -> String -> CompletionItem
simeonHoleToSuggestion original stripParens range simeonHole@(SimeonHole { simeonType }) constructorName =
  let
    kind = completionItemKind "Constructor"

    insertText = simeonHoleToSuggestionText stripParens simeonHole constructorName
...
holeSuggestions :: String -> Boolean -> IRange -> SimeonHole -> Array CompletionItem
holeSuggestions original stripParens range simeonHole@(SimeonHole { name, simeonType }) =
  let
    simeonHoles = getSimeonConstructors simeonType
  in
    map (simeonHoleToSuggestion original stripParens range simeonHole) $ Set.toUnfoldable $ Map.keys simeonHoles
...
suggestions original stripParens contractString range { contract } =
  fromMaybe [] do
    ...
    -- with contractString, which is a contract that comes from the SimeonCompletionItemProvider with
