...
import BlocklyEditor.Types (Action(..), BottomPanelView(..), State, _bottomPanelState, _hasHoles, _simeonCode, _warnings)
...
otherActions state =
  div [ classes [ group ] ]
    [ button
        [ onClick $ const $ Just ViewAsSimeon
        , enabled hasCode
        , classNames [ "btn" ]
        ]
        [ text "View as Simeon" ]
  ...
  where
  hasCode = isJust $ state ^. _simeonCode
