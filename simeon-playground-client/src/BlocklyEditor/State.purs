-- TODO: rename modules from BlocklyEditor -> SimeonBlocklyEditor
module BlocklyEditor.State where

import Prelude
import BlocklyComponent.Types as Blockly
import BlocklyEditor.Types (Action(..), BottomPanelView, State, _bottomPanelState, _errorMessage, _hasHoles, _simeonCode, _metadataHintInfo, _warnings)
...
handleAction (InitBlocklyProject code) = do
  ...
  liftEffect $ SessionStorage.setItem simeonBufferLocalStorageKey code
...
handleAction ViewAsSimeon = pure unit
...
-- This function reads the Simeon code from blockly and, process it and updates the component state
processBlocklyCode ::
  forall m.
  MonadAff m =>
  HalogenM State Action ChildSlots Void m Unit
processBlocklyCode = do
    ...
    Right holesContract -> do
      ...
      let
        ...
        maybeUpdateTemplateContent = case Holes.fromTerm holesContract of
          ...
          Nothing -> identity
      liftEffect $ SessionStorage.setItem simeonBufferLocalStorageKey prettyContract
      modify_
        ( set _errorMessage Nothing
            <<< set _simeonCode (Just $ prettyContract)
  ...
  where
  unexpected s = "An unexpected error has occurred, please raise a support issue at https://github.com/The-Blockchain-Company/zerepoch/issues/new: " <> s
