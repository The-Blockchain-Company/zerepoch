...
import Simeon.Extended.Metadata (NumberFormat(..), MetaData)
import Simeon.Semantics (Party(..), Payee(..), Token(..))

renderPrettyToken :: forall p i. Token -> HTML p i
renderPrettyToken (Token "" "") = text "BCC"

renderPrettyToken (Token cur tok) = abbr [ title cur ] [ text tok ]

showPrettyToken :: Token -> String
showPrettyToken (Token "" "") = "BCC"
