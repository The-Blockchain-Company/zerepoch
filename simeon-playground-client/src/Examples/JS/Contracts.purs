...
escrow =
  """
    ...
    function deposit(timeout: Timeout, timeoutContinuation: Contract, continuation: Contract): Contract {
        return When([Case(Deposit(seller, buyer, bcc, price), continuation)],
            timeout,
            timeoutContinuation);
    }
    ...
    function sellerToBuyer(continuation: Contract): Contract {
        return Pay(seller, Account(buyer), bcc, price, continuation);
    }

    function paySeller(continuation: Contract): Contract {
        return Pay(buyer, Party(seller), bcc, price, continuation);
    }

    const refundBuyer: Contract = explicitRefunds ? Pay(buyer, Party(buyer), bcc, price, Close) : Close;

    const refundSeller: Contract = explicitRefunds ? Pay(seller, Party(seller), bcc, price, Close) : Close;
...
escrowWithCollateral =
  """
    ...
    function depositCollateral(party: Party, timeout: Timeout, timeoutContinuation: Contract, continuation: Contract): Contract {
        return When([Case(Deposit(party, party, bcc, collateral), continuation)],
            timeout,
            timeoutContinuation);
    }

    function burnCollaterals(continuation: Contract): Contract {
        return Pay(seller, Party(burnAddress), bcc, collateral,
    ...
    function refundSellerCollateral(continuation: Contract): Contract {
        if (explicitRefunds) {
            return Pay(seller, Party(seller), bcc, collateral, continuation);
        } else {
            return continuation;
        }
    }
    ...
    function refundBuyerCollateral(continuation: Contract): Contract {
        if (explicitRefunds) {
            return Pay(buyer, Party(buyer), bcc, collateral, continuation);
        } else {
            return continuation;
        }
    }
...
zeroCouponBond =
  """
    ...
    function transfer(timeout: Timeout, from: Party, to: Party, amount: Value, continuation: Contract): Contract {
        return When([Case(Deposit(from, from, bcc, amount),
...
couponBondGuaranteed =
  """
    ...
    function deposit(amount: Value, by: Party, toAccount: Party,
        ...
        return When([Case(Deposit(toAccount, by, bcc, amount), continuation)],
            timeout,
            timeoutContinuation);
    }

    function refundGuarantor(amount: Value, continuation: Contract): Contract {
        return Pay(investor, Party(guarantor), bcc, amount, continuation)
    }
    ...
    function giveCollateralToLender(amount: Value): Contract {
        if (explicitRefunds) {
            return Pay(investor, Party(investor), bcc, amount,
                Close);
        } else {
            return Close;
        }
...
swap =
  """
    ...
    const entropicPerBcc: Value = Constant(1000000n);
    const amountOfBcc: Value = ConstantParam("Amount of Bcc");
    const amountOfEntropic: Value = MulValue(entropicPerBcc, amountOfBcc);
    const amountOfDollars: Value = ConstantParam("Amount of dollars");

    const bccDepositTimeout: Timeout = SlotParam("Timeout for Bcc deposit");
    ...
    const contract: Contract = makeDeposit(bccProvider, bccDepositTimeout, Close,
...
contractForDifferences =
  """
    ...
    function initialDeposit(by: Party, deposit: Value, timeout: ETimeout, timeoutContinuation: Contract,
        continuation: Contract): Contract {
        return When([Case(Deposit(by, by, bcc, deposit), continuation)],
            timeout,
            timeoutContinuation);
    }
    ...
    function transferUpToDeposit(from: Party, payerDeposit: Value, to: Party, amount: Value, continuation: Contract): Contract {
        return Pay(from, Account(to), bcc, Cond(ValueLT(amount, payerDeposit), amount, payerDeposit), continuation);
    }

    function refund(who: Party, amount: Value, continuation: Contract): Contract {
        if (explicitRefunds) {
            return Pay(who, Party(who), bcc, amount,
                continuation);
        } else {
            return continuation;
        }
...
contractForDifferencesWithOracle =
  """
    ...
    const priceBeginning: Value = ConstantParam("Amount of Bcc to use as asset");
    const priceEnd: ValueId = ValueId("Price in second window");

    const exchangeBeginning: ChoiceId = ChoiceId("dir-bccusd", oracle);
    const exchangeEnd: ChoiceId = ChoiceId("inv-bccusd", oracle);
