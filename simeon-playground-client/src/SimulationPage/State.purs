...
import Simeon.Holes (Location(..), getLocation)
import Simeon.Monaco as MM
import Simeon.Parser (parseContract)
import Simeon.Semantics (ChoiceId(..), Input(..), Party(..), inBounds)
import Simeon.Template (fillTemplate, typeToLens)
...
import Simulator.Lenses (_SimulationNotStarted, _SimulationRunning, _currentContract, _currentSimeonState, _executionState, _initialSlot, _simeonState, _moveToAction, _possibleActions, _templateContent, _termContract)
import Simulator.State (applyInput, emptySimeonState, inFuture, moveToSlot, startSimulation, updateChoice)
...
mkState =
  { showRightPanel: true
  , simeonState: NEL.singleton (emptySimeonState Nothing)
  , helpContext: SimeonHelp
...
handleAction (SetInitialSlot initialSlot) = do
  assign (_currentSimeonState <<< _executionState <<< _SimulationNotStarted <<< _initialSlot) initialSlot
  setOraclePrice

handleAction (SetIntegerTemplateParam templateType key value) = do
  modifying (_currentSimeonState <<< _executionState <<< _SimulationNotStarted <<< _templateContent <<< typeToLens templateType) (Map.insert key value)
  setOraclePrice

handleAction StartSimulation =
  void
    {- The simeonState is a non empty list of an object that includes the ExecutionState (SimulationRunning | SimulationNotStarted)
    ...
    $ runMaybeT do
        initialSlot <- MaybeT $ peruse (_currentSimeonState <<< _executionState <<< _SimulationNotStarted <<< _initialSlot)
        termContract <- MaybeT $ peruse (_currentSimeonState <<< _executionState <<< _SimulationNotStarted <<< _termContract <<< _Just)
        templateContent <- MaybeT $ peruse (_currentSimeonState <<< _executionState <<< _SimulationNotStarted <<< _templateContent)
        let
...
handleAction (SetSlot slot) = do
  assign (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _possibleActions <<< _moveToAction) (Just $ MoveToSlot slot)
  setOraclePrice
...
handleAction ResetSimulator = do
  modifying _simeonState (NEL.singleton <<< last)
  updateOracleAndContractEditor

handleAction Undo = do
  modifying _simeonState tailIfNotEmpty
  updateOracleAndContractEditor
...
handleAction (LoadContract contents) = do
  ...
  assign _simeonState $ NEL.singleton $ emptySimeonState mTermContract
...
setOraclePrice ::
  ...
  HalogenM State Action ChildSlots Void m Unit
setOraclePrice = do
  execState <- use (_currentSimeonState <<< _executionState)
  case execState of
