...
import Simeon.Extended.Metadata (MetaData, NumberFormat(..), getChoiceFormat)
import Data.Map.Ordered.OMap as OMap
import Data.Set.Ordered.OSet (OSet)
import Simeon.Monaco as MM
import Simeon.Semantics (AccountId, Assets(..), Bound(..), ChoiceId(..), Input(..), Party(..), Payee(..), Payment(..), PubKey, Slot, SlotInterval(..), Token(..), TransactionInput(..), inBounds, timeouts)
import Simeon.Template (IntegerTemplateType(..), orderContentUsingMetadata)
...
import Simulator.Lenses (_SimulationRunning, _currentContract, _currentSimeonState, _executionState, _log, _simeonState, _possibleActions, _slot, _transactionError, _transactionWarnings)
...
render metadata state =
  ...
  runtimeWarnings = view (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _transactionWarnings) state

  hasRuntimeError :: Boolean
  hasRuntimeError = has (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _transactionError <<< to isJust <<< only true) state
...
sendToBlocklyButton state =
  ...
  where
  isBlocklyEnabled = view (_simeonState <<< _Head <<< _editorErrors <<< to Array.null) state
...
simeonEditor ::
  ...
  State ->
  ComponentHTML Action ChildSlots m
simeonEditor state = slot _simulatorEditorSlot unit component unit (Just <<< HandleEditorMessage)
  where
...
sidebar metadata state = case view (_simeonState <<< _Head <<< _executionState) state of
  ...
  cardWidget "Simulation has not started yet"
    $ div_
        [ div [ classes [ ClassName "slot-input", ClassName "initial-slot-input" ] ]
            ...
            , simeonActionInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (SetInitialSlot <<< wrap) (unwrap initialSlot)
            ]
        , div_
                      ...
                      ( ( div [ class_ (ClassName "template-fields") ]
                            ( [ div_
                                ...
                                <> [ case lookupFormat key of
                                      Just (currencyLabel /\ numDecimals) -> simeonCurrencyInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (actionGen typeName key) currencyLabel numDecimals value
                                      Nothing -> simeonActionInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (actionGen typeName key) value
                                  ]
                            )
                        )
                      )
...
simulationStateWidget state =
  let
    currentSlot = state ^. (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _slot <<< to show)
...
actionWidget metadata state =
  ...
  where
  possibleActions = view (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _possibleActions <<< _Newtype) state
    ...
    div
      ...
      ( [ div [ classes [ ClassName "action-label" ] ]
            [ div [ class_ (ClassName "choice-input") ]
                ...
                , case mChoiceInfo of
                    Just { choiceFormat: DecimalFormat numDecimals currencyLabel } -> simeonCurrencyInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (SetChoice choiceId) currencyLabel numDecimals chosenNum
                    _ -> simeonActionInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (SetChoice choiceId) chosenNum
                ]
...
inputItem _ state person (MoveToSlot slot) =
  div
    ...
    ( [ div [ classes [ ClassName "action" ] ]
          [ p [ class_ (ClassName "slot-input") ]
              ...
              , simeonActionInput [ "mx-2", "flex-grow", "flex-shrink-0" ] (SetSlot <<< wrap) (unwrap slot)
              ]
  ...
  boundsError = "The slot must be more than the current slot " <> (state ^. (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _slot <<< to show))

simeonCurrencyInput :: forall p action. Array String -> (BigInteger -> action) -> String -> Int -> BigInteger -> HTML p action
simeonCurrencyInput classes f currencyLabel numDecimals current = currencyInput classes current currencyLabel numDecimals (Just <<< f <<< fromMaybe zero)

simeonActionInput :: forall p action. Array String -> (BigInteger -> action) -> BigInteger -> HTML p action
simeonActionInput classes f current = simeonCurrencyInput classes f "" 0 current
...
logWidget metadata state =
  ...
  where
  inputLines = state ^. (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _log <<< to (concatMap (logToLines metadata) <<< reverse))
