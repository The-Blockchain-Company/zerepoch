...
import Simeon.Semantics (ChoiceId(..), Party, Token, TransactionError, TransactionWarning, ValueId(..), _accounts, _boundValues, _choices)
import Simeon.ViewPartials (displayWarningList)
import Pretty (renderPrettyParty, renderPrettyToken, showPrettyMoney)
import SimulationPage.Types (BottomPanelView(..), State)
import Simulator.Lenses (_SimulationNotStarted, _SimulationRunning, _executionState, _initialSlot, _simeonState, _slot, _state, _transactionError, _transactionWarnings)
...
panelContents metadata state WarningsAndErrorsView =
  let
    ...
    mRuntimeError = join $ preview (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _transactionError) state
  in
...
currentStateView metadata state =
  ...
  where
  slotText = case previewOn state (_simeonState <<< _Head <<< _executionState <<< _SimulationNotStarted <<< _initialSlot) of
    Just initialSlot -> Right $ show initialSlot
    Nothing -> case previewOn state (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _slot) of
      Just slot -> Right $ show slot
      Nothing -> Left "Slot number not defined"

  accountsData =
    let
      (accounts :: Array (Tuple (Tuple Party Token) BigInteger)) = state ^. (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _state <<< _accounts <<< to Map.toUnfoldable)
  ...
  choicesData =
    let
      (choices :: Array (Tuple ChoiceId BigInteger)) = state ^. (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _state <<< _choices <<< to Map.toUnfoldable)
  ...
  bindingsData =
    let
      (bindings :: Array (Tuple ValueId BigInteger)) = state ^. (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning <<< _state <<< _boundValues <<< to Map.toUnfoldable)
