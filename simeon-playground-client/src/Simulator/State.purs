...
import Simeon.Holes (Contract(..), Term(..), TransactionOutput(..), computeTransaction, fromTerm, reduceContractUntilQuiescent)
import Simeon.Holes as T
import Simeon.Semantics (Action(..), Bound(..), ChoiceId(..), ChosenNum, Environment(..), Input, IntervalResult(..), Observation, Party, Slot, SlotInterval(..), State, Timeouts(..), TransactionError(..), TransactionInput(..), _minSlot, boundFrom, emptyState, evalValue, fixInterval, moneyInContract, timeouts)
import Simeon.Semantics as S
import Simeon.Template (getPlaceholderIds, initializeTemplateContent)
import Simulator.Lenses (_SimulationRunning, _contract, _currentSimeonState, _executionState, _log, _simeonState, _moneyInContract, _moveToAction, _pendingInputs, _possibleActions, _slot, _state, _transactionError, _transactionWarnings)
import Simulator.Types (ActionInput(..), ActionInputId(..), ExecutionState(..), ExecutionStateRecord, LogEntry(..), SimeonState, Parties(..), otherActionsParty)
...
emptySimeonState :: Maybe (Term T.Contract) -> SimeonState
emptySimeonState mContract =
  { editorErrors: mempty
...
inFuture :: forall r. { simeonState :: NonEmptyList SimeonState | r } -> Slot -> Boolean
inFuture state slot = has (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _slot <<< nearly zero ((>) slot)) state

updatePossibleActions :: SimeonState -> SimeonState
...
applyPendingInputs :: SimeonState -> SimeonState
...
updateSlot :: Slot -> SimeonState -> SimeonState
...
updateSimeonState ::
  forall s m.
  MonadState { simeonState :: NonEmptyList SimeonState | s } m =>
  (SimeonState -> SimeonState) ->
  m Unit
updateSimeonState f = modifying _simeonState (extendWith (updatePossibleActions <<< f))
...
updateChoice ::
  ...
  ChosenNum ->
  m Unit
updateChoice choiceId chosenNum = updateSimeonState (over (_executionState <<< _SimulationRunning <<< _possibleActions) (mapPartiesActionInput (doUpdate choiceId)))
  where
...
moveToSlot ::
  ...
  m Unit
moveToSlot slot = do
  mSignificantSlot <- use (_simeonState <<< _Head <<< to nextTimeout)
  let
    ...
    mApplyPendingInputs =
      if slot >= (fromMaybe zero mSignificantSlot) then
        ...
        identity
  updateSimeonState (mApplyPendingInputs <<< updateSlot slot)

hasHistory :: forall s. { simeonState :: NonEmptyList SimeonState | s } -> Boolean
hasHistory state = case state ^. (_simeonState <<< _Tail) of
  Nil -> false
  Cons _ _ -> true

evalObservation :: SimeonState -> Observation -> Boolean
...
nextTimeout :: SimeonState -> Maybe Slot
