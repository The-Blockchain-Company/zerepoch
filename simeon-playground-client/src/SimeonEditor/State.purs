...
import Env (Env)
import Examples.Simeon.Contracts (example) as ME
import Halogen (HalogenM, liftEffect, modify_, query)
import Halogen as H
import Halogen.Extra (mapSubmodule)
import Halogen.Monaco (Message(..), Query(..)) as Monaco
import MainFrame.Types (ChildSlots, _simeonEditorPageSlot)
import Simeon.Extended as Extended
import Simeon.Extended.Metadata (MetadataHintInfo)
import Simeon.Holes as Holes
import Simeon.LinterText as Linter
import Simeon.Monaco (updateAdditionalContext)
import Simeon.Monaco as MM
import Simeon.Parser (parseContract)
import Simeon.Template (TemplateContent)
import Simeon.Template as Template
import SimeonEditor.Types (Action(..), BottomPanelView, State, _bottomPanelState, _editorErrors, _editorReady, _editorWarnings, _hasHoles, _keybindings, _metadataHintInfo, _selectedHole, _showErrorDetail)
...
import StaticData (simeonBufferLocalStorageKey)
...
handleAction (ChangeKeyBindings bindings) = do
  ...
  void $ query _simeonEditorPageSlot unit (Monaco.SetKeyBindings bindings unit)

handleAction (HandleEditorMessage Monaco.EditorReady) = do
  editorSetTheme
  mContents <- liftEffect $ SessionStorage.getItem simeonBufferLocalStorageKey
  editorSetValue $ fromMaybe ME.example mContents
  for_ mContents processSimeonCode
...
handleAction (HandleEditorMessage (Monaco.TextChanged text)) = do
  ...
  -- that it doesn't break the zerepoch playground.
  editorReady <- use _editorReady
  when editorReady do
    assign _selectedHole Nothing
    liftEffect $ SessionStorage.setItem simeonBufferLocalStorageKey text
    processSimeonCode text
...
handleAction (MoveToPosition lineNumber column) = do
  void $ query _simeonEditorPageSlot unit (Monaco.SetPosition { column, lineNumber } unit)
...
handleAction (LoadScript key) = do
  for_ (preview (ix key) StaticData.simeonContracts) \contents -> do
    ...
    liftEffect $ SessionStorage.setItem simeonBufferLocalStorageKey prettyContents
...
handleAction (InitSimeonProject contents) = do
  editorSetValue contents
  liftEffect $ SessionStorage.setItem simeonBufferLocalStorageKey contents
...
processSimeonCode text = do
  ...
  let
    ...
    -- displaying it.
    -- see https://github.com/The-Blockchain-Company/zerepoch/pull/2560#discussion_r550252989
    warningMarkers =
      markerData
  ...
  void $ query _simeonEditorPageSlot unit $ H.request $ Monaco.SetModelMarkers markerData
  modify_
  ...
  -}
  mProviders <- query _simeonEditorPageSlot unit (Monaco.GetObjects identity)
  case mProviders of
...
editorSetTheme = void $ query _simeonEditorPageSlot unit (Monaco.SetTheme MM.daylightTheme.name unit)

editorSetValue :: forall state action msg m. String -> HalogenM state action ChildSlots msg m Unit
editorSetValue contents = void $ query _simeonEditorPageSlot unit (Monaco.SetText contents unit)

editorGetValue :: forall state action msg m. HalogenM state action ChildSlots msg m (Maybe String)
editorGetValue = query _simeonEditorPageSlot unit (Monaco.GetText identity)
