module SimeonEditor.View where
...
import SimeonEditor.BottomPanel (panelContents)
import SimeonEditor.Types (Action(..), BottomPanelView(..), State, _bottomPanelState, _editorErrors, _editorWarnings, _keybindings, contractHasErrors, contractHasHoles)
...
render metadata state =
  ...
  panelTitles =
    ...
    , { title: warningsTitle, view: SimeonWarningsView, classes: [] }
    , { title: errorsTitle, view: SimeonErrorsView, classes: [] }
    ]
...
sendToSimulatorButton state =
  div [ id_ "simeonSendToSimulator" ]
    ...
    , tooltip tooltipMessage (RefId "simeonSendToSimulator") Bottom
    ]
  where
  ...
  tooltipMessage =
    if disabled' then
      ...
      "Execute this contract in the Simeon simulator"
...
simeonEditor ::
  ...
  State ->
  ComponentHTML Action ChildSlots m
simeonEditor state = slot _simeonEditorPageSlot unit component unit (Just <<< HandleEditorMessage)
  where
