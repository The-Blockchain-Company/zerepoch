...
import Simeon.Holes (SimeonType(..))
import Simeon.Holes as Holes
...
toHTML helpType =
  ...
  where
  headerText SimeonHelp = text "Simulation and analysis"
  ...
  headerText EditorHelp = text "Simeon Code Editor"

  bodyText SimeonHelp =
    div []
      [ p_ [ text "In this pane you can create, simulate and analyse Simeon contracts." ]
  ...
  bodyText EditorHelp =
    div []
      [ p_ [ text "Simeon code can be edited and analysed in the code editor. To make things easier to read, you can format code at any time by right-clicking in the editor and selecting \"Format Document\"." ]
      , p_
          [ text
...
holeText :: SimeonType -> String
holeText simeonType = "Found a hole of type " <> dropEnd 4 (show simeonType) <> "\n" <> text <> "\nClick on Quick Fix below to see the options."
  where
  text = simeonTypeMarkerText simeonType

  dropEnd :: Int -> String -> String
  dropEnd n = fromCodePointArray <<< Array.dropEnd n <<< toCodePointArray

simeonTypeMarkerText :: SimeonType -> String
simeonTypeMarkerText ChoiceIdType =
  """
...
simeonTypeMarkerText ValueIdType =
  """
Values defined using Let are identified by a ValueID
"""

simeonTypeMarkerText ActionType =
  """
...
simeonTypeMarkerText ValueType =
  """
A Value encompasses Bcc, fungible tokens (think currencies), non-fungible tokens (a custom token that is not interchangeable with other tokens), and more exotic mixed cases.
"""

simeonTypeMarkerText ObservationType =
  """
...
simeonTypeMarkerText ContractType =
  """
Simeon has six ways of building contracts. Five of these – Pay, Let, If, When and Assert – build a complex contract from simpler contracts, and the sixth, Close, is a simple contract. At each step of execution, as well as returning a new state and continuation contract, it is possible that effects – payments – and warnings can be generated too.
"""

simeonTypeMarkerText BoundType =
  """
...
helpForConstructor constructor =
  let
    constructors = Holes.allSimeonConstructorNames

    simeonType = Map.lookup constructor constructors
  in
    simeonTypeMarkerText <$> simeonType
