...
import BlocklyEditor.Types (_simeonCode)
...
import MainFrame.Types (Action(..), ChildSlots, ModalView(..), Query(..), State, View(..), _actusBlocklySlot, _authStatus, _blocklyEditorState, _contractMetadata, _createGistResult, _gistId, _hasUnsavedChanges, _haskellState, _javascriptState, _loadGistResult, _simeonEditorState, _newProject, _projectName, _projects, _rename, _saveAs, _showBottomPanel, _showModal, _simulationState, _view, _workflow, sessionToState, stateToSession)
import MainFrame.View (render)
import Simeon (getApiGistsByGistId)
import Simeon as Server
import Simeon.ActusBlockly as AMB
import Simeon.Extended.Metadata (emptyContractMetadata, getHintsFromMetadata)
import Simeon.Gists (PlaygroundFiles, mkNewGist, playgroundFiles)
import SimeonEditor.State as SimeonEditor
import SimeonEditor.Types as ME
...
initialState =
  ...
  , simeonEditorState: ME.initialState
...
toSimeonEditor = mapSubmodule _simeonEditorState SimeonEditorAction
...
handleSubRoute Router.SimeonEditor = selectView SimeonEditor
...
handleAction (SimeonEditorAction action) = do
  toSimeonEditor (SimeonEditor.handleAction action)
  case action of
    ME.SendToSimulator -> do
      mContents <- SimeonEditor.editorGetValue
      for_ mContents \contents ->
        sendToSimulation contents
    ME.ViewAsBlockly -> do
      mSource <- SimeonEditor.editorGetValue
    ...
    ME.HandleEditorMessage (Monaco.TextChanged _) -> setUnsavedChangesForLanguage Simeon true
    ME.InitSimeonProject _ -> setUnsavedChangesForLanguage Simeon false
...
handleAction (BlocklyEditorAction action) = do
  ...
  case action of
    BE.SendToSimulator -> do
      mCode <- use (_blocklyEditorState <<< _simeonCode)
    ...
    BE.ViewAsSimeon -> do
      ...
      -- into something like this: https://github.com/The-Blockchain-Company/zerepoch/pull/2560#discussion_r549892291
      mCode <- use (_blocklyEditorState <<< _simeonCode)
      for_ mCode \code -> do
        selectView SimeonEditor
        assign _workflow (Just Simeon)
        toSimeonEditor $ SimeonEditor.handleAction $ ME.InitSimeonProject code
...
-- https://github.com/The-Blockchain-Company/zerepoch/pull/2498#discussion_r533478042
...
handleAction (NewProjectAction (NewProject.CreateProject lang)) = do
  ...
  case lang of
    ...
    Simeon ->
      for_ (Map.lookup "Example" StaticData.simeonContracts) \contents -> do
        toSimeonEditor $ SimeonEditor.handleAction $ ME.InitSimeonProject contents
    Blockly ->
      for_ (Map.lookup "Example" StaticData.simeonContracts) \contents -> do
...
handleAction (DemosAction action@(Demos.LoadDemo lang (Demos.Demo key))) = do
  ...
  case lang of
    ...
    Simeon -> do
      for_ (preview (ix key) StaticData.simeonContracts) \contents -> do
        toSimeonEditor $ SimeonEditor.handleAction $ ME.InitSimeonProject contents
    Blockly -> do
      for_ (preview (ix key) StaticData.simeonContracts) \contents -> do
        toBlocklyEditor $ BlocklyEditor.handleAction $ BE.InitBlocklyProject contents
...
routeToView { subroute } = case subroute of
  ...
  Router.SimeonEditor -> Just SimeonEditor
...
viewToRoute = case _ of
  ...
  SimeonEditor -> Router.SimeonEditor
...
createFiles = do
  let
    ...
    -- playground is a meta-data file that we currently just use as a tag to check if a gist is a simeon playground gist
    playground = "{}"
  ...
  case workflow of
    Just Simeon -> do
      simeon <- pruneEmpty <$> SimeonEditor.editorGetValue
      pure $ emptyFiles { simeon = simeon }
    Just Blockly -> do
...
-- https://<base_url>/#/simeon?gistid=<gist_id>
...
loadGist gist = do
  ...
  toSimeonEditor $ SimeonEditor.handleAction $ ME.InitSimeonProject $ fromMaybe mempty simeon
