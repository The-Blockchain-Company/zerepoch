...
import Halogen.Classes (simeonPlayLogo)
...
import MainFrame.Types (Action(..), ChildSlots, ModalView(..), State, View(..), _actusBlocklySlot, _authStatus, _blocklyEditorState, _contractMetadata, _createGistResult, _gistId, _hasUnsavedChanges, _haskellState, _javascriptState, _simeonEditorState, _projectName, _simulationState, _view, hasGlobalLoading)
import Simeon.ActusBlockly as AMB
import SimeonEditor.View as SimeonEditor
...
render state =
  div [ classNames [ "site-wrap" ] ]
    ( [ header [ classNames [ "no-margins", "flex", "flex-col" ] ]
          [ div [ classNames [ "flex", "items-center", "justify-between", "bg-gray-dark", "px-medium", "py-small" ] ]
              ...
              , projectTitle
              , div_
                  [ a [ href "./doc/simeon/tutorials/index.html", target "_blank", classNames [ "font-semibold" ] ] [ text "Tutorial" ]
                  -- Link disabled as the Actus labs is not working properly. Future plans might include moving this functionality to Simeon run
          ...
          [ section [ id_ "main-panel" ] case state ^. _view of
              ...
              SimeonEditor ->
                [ renderSubmodule
                    _simeonEditorState
                    SimeonEditorAction
                    (SimeonEditor.render (state ^. _contractMetadata))
                    state
                ]
              HaskellEditor ->
                [ renderSubmodule
      ...
      , footer [ classNames [ "flex", "justify-between", "px-medium", "py-small", "bg-gray-dark", "font-semibold" ] ]
          ...
          , div_ [ text (copyright <> " 2021 GodXCoin Ltd") ]
          , div [ classNames [ "flex" ] ]
              [ a [ href "https://t.me/GodXCoin_Simeon", target "_blank", classNames [ "pr-small" ] ] [ text "Telegram" ]
              , a [ href "https://twitter.com/hashtag/Simeon", target "_blank", classNames [ "pl-small" ] ] [ text "Twitter" ]
              ]
          ]
      ]
    )
  ...
  otherActions SimeonEditor = [ renderSubmodule _simeonEditorState SimeonEditorAction SimeonEditor.otherActions state ]
