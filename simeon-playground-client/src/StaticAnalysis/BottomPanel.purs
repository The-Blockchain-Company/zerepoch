...
import Simeon.Extended.Metadata (MetaData, NumberFormat(..))
import Data.Map.Ordered.OMap as OMap
import Simeon.Semantics (ChoiceId(..), Input(..), Slot(..), SlotInterval(..), TransactionInput(..))
import Simeon.Symbolic.Types.Response as R
  ...
  in
    case result of
      ...
      WarningAnalysis staticSubResult -> case staticSubResult of
        ...
        Failure WarningAnalysisIsExtendedSimeonError ->
          explanation
            [ h3 [ classes [ ClassName "analysis-result-title" ] ] [ text "Error during warning analysis" ]
            , text "Analysis failed for the following reason:"
            , ul [ classes [ ClassName "indented-enum-initial" ] ]
                [ li_
                    [ b_ [ spanText "The code has templates. Static analysis can only be run in core Simeon code." ]
                    ]
                ]
            ]
        Loading -> text ""
