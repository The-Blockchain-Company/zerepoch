...
import Simeon.Template (fillTemplate)
import Simeon.Semantics (Case(..), Contract(..), Observation(..), emptyState)
import Simeon.Semantics as S
import Simeon.Symbolic.Types.Request as MSReq
import Simeon.Symbolic.Types.Response (Response(..), Result(..))
...
analyseContract extendedContract = do
  ...
  case toCore $ fillTemplate templateContent extendedContract of
    ...
    Nothing -> assign (_analysisState <<< _analysisExecutionState) $ WarningAnalysis $ Failure WarningAnalysisIsExtendedSimeonError
  where
  emptySemanticState = emptyState zero

  checkContractForWarnings settings state contract = traverse logAndStripDuration =<< (runAjax $ (flip runReaderT) settings (Server.postApiSimeonanalysis (MSReq.Request { onlyAssertions: false, contract, state })))
...
checkContractForFailedAssertions contract state = do
  settings <- asks _.ajaxSettings
  traverse logAndStripDuration =<< (runAjax $ (flip runReaderT) settings (Server.postApiSimeonanalysis (MSReq.Request { onlyAssertions: true, contract: contract, state: state })))
