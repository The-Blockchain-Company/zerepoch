...
import Simeon.Semantics (AccountId, Contract(..), Observation(..), Payee(..), Token, Value(..), emptyState)
...
analyseClose extendedContract = do
  ...
  case toCore $ fillTemplate templateContent extendedContract of
    ...
    Nothing -> assign (_analysisState <<< _analysisExecutionState) (CloseAnalysis $ AnalysisFailure "The code has templates. Static analysis can only be run in core Simeon code.")
