...
import Simeon.Semantics (Contract(..), Observation(..), emptyState)
...
analyseReachability extendedContract = do
  ...
  case toCore $ fillTemplate templateContent extendedContract of
    ...
    Nothing -> assign (_analysisState <<< _analysisExecutionState) (ReachabilityAnalysis $ AnalysisFailure "The code has templates. Static analysis can only be run in core Simeon code.")
