...
Check the [backend documentation](../simeon-playground-server/README.md) for more information on how to setup the Github OAuth application.

Now we will build and run the front end:
```bash
cd simeon-playground-client
# Generate the purescript bridge files
$(nix-build ../default.nix -A simeon-playground.generate-purescript)/bin/simeon-playground-generate-purs
...
- Outside of the nix-shell environment (from the client directory): `$(nix-build -A zerepoch.updateClientDeps ../)/bin/update-client-deps`
