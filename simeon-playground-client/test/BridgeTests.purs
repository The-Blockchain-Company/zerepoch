...
import Simeon.Semantics (Action(..), Bound(..), Case(..), ChoiceId(..), Contract(..), Observation(..), Party(..), Payee(..), Rational(..), Slot(..), State(..), Token(..), Value(..), ValueId(..))
  ...
  test "Contract Serialization" do
    ...
    let
      ...
      contract =
        Assert TrueObs
          ( When
              [ Case (Deposit alicePk alicePk bcc valueExpr)
                  ( Let (ValueId "x") valueExpr
                      (Pay alicePk (Party bobRole) bcc (Cond TrueObs (UseValue (ValueId "x")) (UseValue (ValueId "y"))) Close)
                  )
