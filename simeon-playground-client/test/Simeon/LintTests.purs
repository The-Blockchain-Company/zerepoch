...
import Simeon.Linter (State(..), WarningDetail(..), lint)
import Simeon.Parser (parseContract)
import Simeon.Semantics as S
import StaticData (simeonContracts)
import Test.Unit (TestSuite, Test, suite, test, failure)
import Test.Unit.Assert as Assert

all :: TestSuite
all = do
  suite "Simeon.Linter reports simplification" do
  ...
  suite "Simeon.Linter reports bad pratices" do
    test "Let shadowing" letShadowing
    test "Non-increasing timeouts" nonIncreasingTimeouts
  suite "Simeon.Linter reports unreachable code" do
  ...
  suite "Simeon.Linter reports bad contracts" do
  ...
  suite "Simeon.Linter does not report good contracts" do
  ...
  suite "All examples pass Simeon.Linter" examplesPassLinter
...
payInsufficientDeposit = testWarningSimple (depositAndThenDo "(Constant 9)" continuation) "The contract makes a payment of 10 BCC from account \"role\" but the account only has 9"
  where
  continuation = "(Pay (Role \"role\") (Party (Role \"role\")) (Token \"\" \"\") (Constant 10) Close)"

payTwiceInsufficientDeposit :: Test
payTwiceInsufficientDeposit = testWarningSimple (depositAndThenDo "(Constant 9)" continuation) "The contract makes a payment of 5 BCC from account \"role\" but the account only has 4"
  where
  continuation =
...
examplesPassLinter = sequence_ (map examplePassesLinter (Map.toUnfoldable simeonContracts) :: Array TestSuite)
  where
  examplePassesLinter :: String /\ String -> TestSuite
  examplePassesLinter (name /\ contract) = test (show name <> " example passes Simeon.Linter") $ testNoWarning contract
