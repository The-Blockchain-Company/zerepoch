-- TODO: Move these tests to simeon-commons
module Simeon.ContractTests where
...
import Examples.Simeon.Contracts as Contracts
...
import Simeon.Semantics (Bound(..), ChoiceId(..), Contract(..), Input(..), Party(..), Token(..), TransactionError, TransactionWarning)
...
import Simulator.Lenses (_SimulationRunning, _currentContract, _currentSimeonState, _currentPossibleActions, _executionState, _simeonState, _transactionError, _transactionWarnings)
...
contractToExtended contract = case parseContract contract of -- We reuse the extended Simeon parser for now since it is a superset
  ...
  toTerm
    ( fillTemplate
        ( TemplateContent
            ...
            , valueContent:
                Map.fromFoldable
                  [ "Amount of Bcc" /\ BigInteger.fromInt 1500000
        ...
        ( TemplateContent
            ...
            , valueContent:
                Map.fromFoldable
                  [ "Amount paid by party" /\ BigInteger.fromInt 100000000
                  , "Amount paid by counterparty" /\ BigInteger.fromInt 100000000
                  , "Amount of Bcc to use as asset" /\ BigInteger.fromInt 100000000
                  ]
            }
        )
...
-- TODO:  We should combine this test with the ones defined in Simeon.Holes.SemanticTest
...
escrowSimpleFlow =
  test "Escrow" do
    ...
    let
      deposit = IDeposit seller buyer bcc (BigInteger.fromInt 450)
      ...
      txError = do
        executionState <- preview (_simeonState <<< _Head <<< _executionState <<< _SimulationRunning) finalState
  ...
  runContract (Just _) = do
    ...
    case mPossibleActions of
      ...
      Just possibleActions -> do
        ...
        (mError :: Maybe TransactionError) <- gets $ preview (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _transactionError <<< _Just)
        (mWarnings :: Maybe (Array TransactionWarning)) <- gets $ preview (_currentSimeonState <<< _executionState <<< _SimulationRunning <<< _transactionWarnings)
