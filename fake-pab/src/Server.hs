...
import           Language.Simeon                  (ChoiceId (ChoiceId), Contract (..), Input (..), Party (..),
...
import           Zerepoch.V1.Ledger.Value            (CurrencySymbol (..), TokenName (..), Value (..), currencySymbol,
                                                    tokenName)
...
fromCurrencySymbol =  Text.unpack . TE.decodeUtf8 . B16.encode . ZerepochTx.fromBuiltin . unCurrencySymbol
...
fromPubKeyHash =  Text.unpack . TE.decodeUtf8 . B16.encode . ZerepochTx.fromBuiltin . getPubKeyHash

toPubKeyHash :: PublicKey -> PubKeyHash
toPubKeyHash = PubKeyHash . ZerepochTx.toBuiltin . fromRight mempty . B16.decode . TE.encodeUtf8 . Text.pack
...
-- Creates a Wallet with 1000 BCC
      ...
      execute conn
        [sql|WITH source_container AS (SELECT get_or_create_money_container_id_from_pubkey(?) AS id),
              ...
              VALUES ((SELECT id FROM main_transaction), 1, (SELECT id FROM source_container), ?, ?, ?),
                   ...
                   (srcPubKey,
                    destPubKey,
                    -amount, curr, ZerepochTx.fromBuiltin tok, amount, curr, ZerepochTx.fromBuiltin tok)
      return ())
  where
  ...
  then
    liftIO . withResource conns $ \conn -> withTransaction conn (do
      ...
      executeMany conn [sql| INSERT INTO token (currency_symbol, token_name)
                             VALUES (?, ?)
                       |] ([(currencySymbol, BSU.toString (ZerepochTx.fromBuiltin tns)) | tns <- ownerTokenNamesStr] :: [(String, String)])
      sequence_ [execute conn [sql| INSERT INTO currency_amount (amount, currency_symbol, token_name, money_container_id)
                              ...
                              |] (1 :: Integer, currencySymbol, BSU.toString (ZerepochTx.fromBuiltin tns), pk) | (TokenName tns, pk) <- Map.toList owners ]
...
fromTokenName = BSU.toString . ZerepochTx.fromBuiltin . unTokenName
