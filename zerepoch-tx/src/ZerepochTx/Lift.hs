...
import           ZerepochTx.Lift.Class             (makeLift)
import qualified ZerepochTx.Lift.Class             as Lift
import           ZerepochTx.Lift.Instances         ()

import           Data.Bifunctor
import           ZerepochIR
import qualified ZerepochIR                        as PIR
import           ZerepochIR.Compiler
import           ZerepochIR.Compiler.Definitions
import qualified ZerepochIR.Error                  as PIR
import qualified ZerepochIR.MkPir                  as PIR

import qualified ZerepochCore                      as PLC
import           ZerepochCore.Pretty               (PrettyConst)
import           ZerepochCore.Quote
import qualified ZerepochCore.StdLib.Data.Function as PLC
...
-- | Get a Zerepoch Core term corresponding to the given value.
safeLift
    :: (Lift.Lift uni a
...
-- | Get a Zerepoch Core program corresponding to the given value.
safeLiftProgram
    :: (Lift.Lift uni a
       , PIR.AsTypeError e (PIR.Term TyName Name uni fun ()) uni fun (Provenance ()),  PLC.GEq uni
       , PIR.AsTypeErrorExt e uni (Provenance ())
...
-- | Get a Zerepoch Core term corresponding to the given value, throwing any errors that occur as exceptions and ignoring fresh names.
lift
...
-- | Get a Zerepoch Core program corresponding to the given value, throwing any errors that occur as exceptions and ignoring fresh names.
liftProgram
    :: (Lift.Lift uni a, Throwable uni fun, PLC.Typecheckable uni fun)
    => a -> UPLC.Program UPLC.NamedDeBruijn uni fun ()
liftProgram x = UPLC.Program () (PLC.defaultVersion ()) $ lift x

-- | Get a Zerepoch Core program in the default universe corresponding to the given value, throwing any errors that occur as exceptions and ignoring fresh names.
liftProgramDef
    :: Lift.Lift PLC.DefaultUni a
    => a -> UPLC.Program UPLC.NamedDeBruijn PLC.DefaultUni PLC.DefaultFun ()
liftProgramDef = liftProgram

-- | Get a Zerepoch Core program corresponding to the given value as a 'CompiledCodeIn', throwing any errors that occur as exceptions and ignoring fresh names.
liftCode
    :: (Lift.Lift uni a, Throwable uni fun, PLC.Typecheckable uni fun)
