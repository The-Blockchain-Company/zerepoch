...
import qualified ZerepochTx.Bool     as P
import qualified ZerepochTx.Eq       as P
import qualified ZerepochTx.IsData   as P
import qualified ZerepochTx.Lift     as P
import qualified ZerepochTx.Numeric  as P
import qualified ZerepochTx.Ord      as P
import qualified ZerepochTx.Trace    as P
...
instance  Show a => Show (Ratio a) where
    -- Bccpted from Data.Ratio in the base library
...
Zerepoch Core provides built-in functions 'divideInteger', 'modInteger',
...
--   Zerepoch-compatible 'ZerepochTx.Ratio.Rational'
fromGHC :: Ratio.Rational -> Ratio Integer
fromGHC (n Ratio.:% d) = n :% d

-- | Convert a 'ZerepochTx.Ratio.Rational' to a
--   'Data.Ratio.Rational'
...
{-# INLINABLE divMod #-}
-- TODO. This is doing twice as much work as it needs to: the Zerepoch Core
...
{-# INLINABLE quotRem #-}
-- TODO.  Provide a Zerepoch Core built-in function for this: see the comment for
-- 'divMod'.
