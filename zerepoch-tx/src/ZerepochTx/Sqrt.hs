...
module ZerepochTx.Sqrt(
    Sqrt (..)
    , rsqrt
    , isqrt
    ) where

import           ZerepochTx.IsData  (makeIsDataIndexed)
import           ZerepochTx.Lift    (makeLift)
import           ZerepochTx.Prelude (divide, negate, otherwise, ($), (*), (+), (<), (<=), (==))
import           ZerepochTx.Ratio   (Rational, denominator, numerator, (%))
