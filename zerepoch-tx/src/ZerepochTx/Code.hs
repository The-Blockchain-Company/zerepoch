...
-- | A compiled Zerepoch Tx program. The last type parameter indicates
...
-- | Get the actual Zerepoch Core program out of a 'CompiledCodeIn'.
getPlc
    :: (PLC.Closed uni, uni `PLC.Everywhere` Flat, Flat fun, uni `PLC.Everywhere` PLC.PrettyConst, PLC.GShow uni, PLC.Pretty fun)
...
-- | Get the Zerepoch IR program, if there is one, out of a 'CompiledCodeIn'.
getPir
