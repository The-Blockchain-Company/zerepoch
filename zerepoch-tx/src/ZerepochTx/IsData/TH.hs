...
module ZerepochTx.IsData.TH (unstableMakeIsData, makeIsDataIndexed) where
...
import qualified ZerepochTx.Builtins.Internal   as BI
...
reconstructCase (TH.ConstructorInfo{TH.constructorName=name, TH.constructorFields=argTys}, index) ixExpr argsExpr kont = do
    ...
    let app = foldl' (\h v -> [| $h ZerepochTx.<*> fromBuiltinData $(TH.varE v) |]) [| ZerepochTx.pure $(TH.conE name) |] argNames
