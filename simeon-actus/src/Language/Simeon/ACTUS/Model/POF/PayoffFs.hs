...
module Language.Simeon.ACTUS.Model.POF.PayoffFs where

import           Data.Time                                         (Day)
import           Language.Simeon                                  (Observation, Value)
...
import           Language.Simeon.ACTUS.SimeonCompat              (RiskFactorsSimeon, constnt, enum, useval)
import           Language.Simeon.ACTUS.Model.POF.PayoffModel
import           Language.Simeon.ACTUS.Ops                        (ActusNum (..), YearFractionOps (_y),
                                                                    simeonFixedPoint)
import           Prelude                                           hiding (Fractional, Num, (*), (+), (-), (/))

payoffFs :: EventType -> RiskFactorsSimeon -> ContractTerms -> Integer -> Day -> Day -> Maybe (Value Observation)
payoffFs
  ev
  RiskFactorsPoly {..}
  ContractTerms
  ...
  curDate =
    let pof = case contractType of
          ...
          ANN -> case ev of
            ...
            IP  -> Just $ _POF_IP_LAM o_rf_CURS isc ipac ipnr ipcb y_sd_t
            _   -> Nothing
     in (\x -> x / (constnt $ fromIntegral simeonFixedPoint)) <$> pof
    where
