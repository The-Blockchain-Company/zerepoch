...
module Language.Simeon.ACTUS.Model.STF.StateTransitionFs (stateTransitionFs) where

import           Data.Maybe                                             (maybeToList)
import           Data.Time                                              (Day)
import           Language.Simeon.ACTUS.Definitions.ContractTerms       (CT (..), ContractTerms (..))
import           Language.Simeon.ACTUS.Definitions.Schedule            (ShiftedDay (calculationDay))
import           Language.Simeon.ACTUS.Model.SCHED.ContractSchedule    (maturity, schedule)
import           Language.Simeon.ACTUS.Model.STF.StateTransitionModel
import           Language.Simeon.ACTUS.Model.Utility.ScheduleGenerator (inf, sup)
import           Language.Simeon.ACTUS.Ops                             (YearFractionOps (_y))

import           Language.Simeon                                       (Contract)
import           Language.Simeon.ACTUS.Definitions.BusinessEvents      (EventType (..), RiskFactorsPoly (..))
import           Language.Simeon.ACTUS.SimeonCompat                   (ContractStateSimeon, RiskFactorsSimeon,
                                                                         constnt, enum, letval, simeonDate,
                                                                         stateTransitionSimeon)

stateTransitionFs :: EventType -> RiskFactorsSimeon -> ContractTerms -> Integer -> Day -> Day -> Contract -> Contract
stateTransitionFs
  ev
  RiskFactorsPoly{..}
  ct@ContractTerms
  ...
  prevDate
  curDate
  continue = addComment $ stateTransitionSimeon ev t continue $ stf ct
    where
      stf :: ContractTerms -> EventType -> ContractStateSimeon -> ContractStateSimeon
      ...
      interestPaymentAnchor = simeonDate <$> Just ipanx
