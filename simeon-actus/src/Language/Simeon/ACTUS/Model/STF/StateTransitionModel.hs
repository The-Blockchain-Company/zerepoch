...
module Language.Simeon.ACTUS.Model.STF.StateTransitionModel where

import           Data.Maybe                                       (fromMaybe)
import           Language.Simeon.ACTUS.Definitions.ContractState (ContractStatePoly (ContractStatePoly, feac, ipac, ipcb, ipnr, isc, nsc, nt, prf, prnxt, sd, tmd))
import           Language.Simeon.ACTUS.Definitions.ContractTerms (CR, FEB (FEB_N), IPCB (..),
                                                                   SCEF (SE_00M, SE_0N0, SE_0NM, SE_I00))
import           Language.Simeon.ACTUS.Model.Utility.ANN.Annuity (annuity)
import           Language.Simeon.ACTUS.Ops                       (ActusNum (..), ActusOps (..), DateOps (_lt),
                                                                   RoleSignOps (_r))
