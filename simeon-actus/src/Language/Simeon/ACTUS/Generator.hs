...
This module contains templates for Simeon constructs required by ACTUS logic

-}

module Language.Simeon.ACTUS.Generator
    ( genStaticContract
    , genFsContract
    )
where
...
import           Language.Simeon                                           (Action (..), Bound (..), Case (..),
                                                                             ChoiceId (..), Contract (..),
                                                                             Observation (..), Party (..), Payee (..),
                                                                             Slot (..), Value (..), ValueId (ValueId),
                                                                             bcc)
import           Language.Simeon.ACTUS.Analysis                            (genProjectedCashflows)
import           Language.Simeon.ACTUS.Definitions.BusinessEvents          (EventType (..), RiskFactors,
                                                                             RiskFactorsPoly (..))
import           Language.Simeon.ACTUS.Definitions.ContractTerms           (Assertion (..), AssertionContext (..),
                                                                             Assertions (..), ContractTerms (..),
                                                                             TermValidationError (..),
                                                                             setDefaultContractTermValues)
import           Language.Simeon.ACTUS.Definitions.Schedule                (CashFlow (..))
import           Language.Simeon.ACTUS.SimeonCompat                       (constnt, dayToSlotNumber,
                                                                             stateInitialisation, toSimeonFixedPoint,
                                                                             useval)
import           Language.Simeon.ACTUS.Model.APPLICABILITY.Applicability   (validateTerms)
import           Language.Simeon.ACTUS.Model.INIT.StateInitializationModel (initialize)
import           Language.Simeon.ACTUS.Model.POF.PayoffFs                  (payoffFs)
import           Language.Simeon.ACTUS.Model.STF.StateTransitionFs         (stateTransitionFs)
import           Language.Simeon.ACTUS.Ops                                 as O (ActusNum (..), YearFractionOps (_y))
...
--            in  When
--                    [ Case
--                        (Deposit party party bcc (Constant amount))
--                            continue
--                    ]
--                    (Slot timeout)
--                    Close
...
invoice from to amount timeout continue =
    ...
    in  When
            [ Case
                    (Deposit party party bcc amount)
                    (Pay party
                        (Party counterparty)
                        bcc
                        amount
    ...
    let
        ...
        inferBounds name ctx = case (name, ctx) of
            ("o_rf_RRMO", Just AssertionContext{..}) ->
                [Bound (toSimeonFixedPoint rrmoMin) (toSimeonFixedPoint rrmoMax)]
