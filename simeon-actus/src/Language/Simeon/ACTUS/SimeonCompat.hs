...
{- This module provides compatibility to Simeon DSL -}

module Language.Simeon.ACTUS.SimeonCompat where

import           Data.String                                       (IsString (fromString))
import           Data.Time                                         (Day, UTCTime (UTCTime))
import           Data.Time.Clock.System                            (SystemTime (MkSystemTime), utcToSystemTime)
import           Language.Simeon                                  (Contract (Let), Observation,
                                                                    Value (Constant, UseValue), ValueId (ValueId))
import           Language.Simeon.ACTUS.Definitions.BusinessEvents (EventType, RiskFactorsPoly (..))
import           Language.Simeon.ACTUS.Definitions.ContractState  (ContractState, ContractStatePoly (..))
import           Language.Simeon.ACTUS.Ops                        (simeonFixedPoint)

type EventHandlerSTF = EventType -> ContractStateSimeon -> ContractStateSimeon
type ContractStateSimeon = ContractStatePoly (Value Observation) (Value Observation)
type RiskFactorsSimeon = RiskFactorsPoly (Value Observation)
...
toSimeonFixedPoint :: Double -> Integer
toSimeonFixedPoint = round <$> (fromIntegral simeonFixedPoint *)

constnt :: Double -> Value Observation
constnt = Constant . toSimeonFixedPoint

enum :: a -> a
enum = id

stateTransitionSimeon :: EventType -> Integer -> Contract -> EventHandlerSTF -> Contract
stateTransitionSimeon ev t continue handler =
  let inputState =
        ContractStatePoly
          { tmd = useval "tmd" $ t - 1,
            nt = useval "nt" $ t - 1,
...
dayToSlotNumber d =
  let (MkSystemTime secs _) = utcToSystemTime (UTCTime d 0)
   in fromIntegral secs - bccEpochStart

simeonDate :: Day -> Value Observation
simeonDate = Constant . fromInteger . dayToSlotNumber
