...
import           ZerepochTx.Prelude      hiding (pure, (<$>))
import qualified Prelude               as Haskell

------------------------------------------------------------

newtype HashedString = HashedString BuiltinByteString deriving newtype (ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)

ZerepochTx.makeLift ''HashedString

newtype ClearString = ClearString BuiltinByteString deriving newtype (ZerepochTx.ToData, ZerepochTx.FromData, ZerepochTx.UnsafeFromData)

ZerepochTx.makeLift ''ClearString
...
gameInstance = Scripts.mkTypedValidator @Game
    $$(ZerepochTx.compile [|| validateGuess ||])
...
guess = endpoint @"guess" @GuessParams $ \(GuessParams theGuess) -> do
    -- Wait for script to have a UTxO of a least 1 entropic
    logInfo @Haskell.String "Waiting for script to have a UTxO of at least 1 entropic"
    utxos <- fundsAtAddressGeq gameAddress (Bcc.entropicValueOf 1)
...
causes the Haskell compiler to generate a schema for the endpoint. The Zerepoch
