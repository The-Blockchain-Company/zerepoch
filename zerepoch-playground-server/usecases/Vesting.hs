...
import           ZerepochTx.Prelude         hiding (Semigroup (..), fold)
...
ZerepochTx.makeLift ''VestingTranche
...
ZerepochTx.makeLift ''VestingParams
...
endpoints = vestingContract vestingParams
  where
    ...
    vestingTranche1 =
        VestingTranche
            {vestingTrancheDate = TimeSlot.scSlotZeroTime def + 20000, vestingTrancheAmount = Bcc.entropicValueOf 50_000_000}
    vestingTranche2 =
        VestingTranche
            {vestingTrancheDate = TimeSlot.scSlotZeroTime def + 40000, vestingTrancheAmount = Bcc.entropicValueOf 30_000_000}
