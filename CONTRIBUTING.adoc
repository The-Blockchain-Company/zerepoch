...
. Check that you can run `nix build -f default.nix zerepoch.haskell.projectPackages.<package name>` successfully.
...
== Integrating with other Bcc projects

=== The Bcc ledger (`bcc-ledger-specs`)

We have a cyclic dependency with `bcc-ledger-specs`:
1. `bcc-ledger-aurum` depends on `zerepoch-ledger-api`.
2. `zerepoch-ledger`/`zerepoch-contract` etc. depend on `bcc-api`, which depends on `bcc-node`, which depends on `cardo-ledger-aurum`.

This is possible because the cycle is not between *packages*, merely between the respositories.
Still, it causes some problems: if we make a change to `zerepoch-ledger-api` or below that causes `bcc-ledger-aurum` to not compile, we cannot immediately merge that change!
To do that, we would need to change our `source-repository-package` pin to point to a version of `bcc-ledger-specs` that works with that change.
But we can't easily make such a PR to `bcc-ledger-specs` either: that would require them to pin a version of `zerepoch` where everything worked.

To make this less painful, we have a (slightly complicated) workflow for such changes.

==== Staging branch

Firstly, there is a branch `zerepoch-staging` in `bcc-ledger-specs`.
The rules for `zerepoch-staging` are:
1. It should pass the `bcc-ledger-specs` CI, so it is always mergeable. 
This means that you *will* need to update the `zerepoch` pin when you make changes to `zerepoch-staging`.
2. It is allowed for the `zerepoch` pin in `zerepoch-staging` to point to a _non-master_ commit, such as a commit in an unmerged PR.
3. The commit pointed to by the `zerepoch` pin in `zerepoch-staging` must continue to exist: we should not squash or rebase it away.
3. When merging `zerepoch-staging` into `master`, the `zerepoch` pin should be updated to point to a _master_ commit.

==== PR workflow

1. Create a `zerepoch` PR (#1) with your changes.
2. Create a `bcc-ledger-specs` PR (#2) targeting `zerepoch-staging` with the necessary changes in `bcc-ledger-specs`. 
The `zerepoch` pin should point to a commit on the PR #1 branch.
While you are working, you can point the `bcc-ledger-specs` pin in your PR #1 branch to the tip of PR #2, so that everything compiles.
3. Once the changes to `bcc-ledger-specs` have settled down, we can merge PR #2.
After this be careful not to remove the commit from PR #1 which you used in `zerepoch-staging`!
4. Update the `bcc-ledger-specs` pin in PR #1 to point to `zerepoch-staging`.
5. Once the changes overall settle down, we can merge PR #1.

==== Merging the staging branches

We can do several PRs that touch `bcc-ledger-specs` before we merge `zerepoch-staging` into master, but it's best not to let it diverge too much.
The process is:

1. Open a PR in `bcc-ledger-specs` merging `zerepoch-staging` into master, and updating the `zerepoch` pin to point to master.
2. You may need to rerun the `zerepoch-preprocessor` in `bcc-ledger-specs` before doing this.
It generates some test scripts, and isn't done automatically.
3. Open a PR in `zerepoch` updating the `bcc-ledger-specs` pin to point to master.
