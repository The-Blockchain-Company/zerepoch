...
import           Zerepoch.Trace                          (ContractConstraints, ContractInstanceTag)
import           Zerepoch.Trace.Emulator.Types           (EmulatorRuntimeError (EmulatorJSONDecodingError), _ContractLog,
                                                        _ReceiveEndpointCall, cilMessage)
import           Zerepoch.Trace.Playground               (PlaygroundTrace, runPlaygroundStream, walletInstanceTag)
import qualified Zerepoch.Trace.Playground
import qualified Zerepoch.Trace.Playground               as Trace
...
stage contract programJson simulatorWalletsJson = do
    ...
    let config = Zerepoch.Trace.Playground.EmulatorConfig (Left $ toInitialDistribution simulatorWallets) def def
