...
import           GHC.Generics                             (C1, Constructor, D1, Generic, K1 (K1), M1 (M1), Rec0, Rep,
                                                           ...
                                                           (:*:) ((:*:)), (:+:) (L1, R1))
import           Ledger                                   (Bcc, AssetClass, CurrencySymbol, DatumHash, Interval,
                                                           POSIXTime, POSIXTimeRange, PubKey, PubKeyHash, RedeemerHash,
                                                           Signature, TokenName, TxId, TxOutRef, ValidatorHash, Value)
import           Ledger.Bytes                             (LedgerBytes)
import           Zerepoch.Contract.StateMachine.ThreadToken (ThreadToken)
import qualified ZerepochTx.AssocMap
import qualified ZerepochTx.Prelude                         as P
import qualified ZerepochTx.Ratio                           as P
...
-- describes all the obvious primitives, plus some Zerepoch types
...
instance (ToSchema k, ToSchema v) =>
         ToSchema (ZerepochTx.AssocMap.Map k v)
...
deriving anyclass instance ToSchema Bcc
...
deriving anyclass instance ToArgument Bcc
